...
public class ModPotions extends Potion {
    ...
    /**
     * If true, the following potion which is an instance of EtFuturumPotion will have a packet sent to all players
     * nearby.
     ...
     * Let's send a packet to all entities within the standard entity tracking distance (80 blocks) notifying them of
     * the potion change.
     ...
     * The potion is removed from the map at this point, but the packet just uses the ID, so we don't need the map
     * anyways.
     ...
     */
    @Override
    public void removeAttributesModifiersFromEntity(EntityLivingBase entity, BaseAttributeMap attributeMap,
        int potency) {
