...
public class TileEntityNewBrewingStand extends TileEntityBrewingStand {
    ...
    public void updateEntity() {
        if (fuel <= 0 && inventory[4] != null) {
            ...
            if (--inventory[4].stackSize <= 0) {
                inventory[4] = inventory[4].getItem()
                    .hasContainerItem(inventory[4])
                        ? inventory[4].getItem()
                            .getContainerItem(inventory[4])
                        : null;
            }
            markDirty();
        }
    ...
    private boolean canBrew() {
        if (fuel > 0 && inventory[3] != null && inventory[3].stackSize > 0) {
            ...
            if (!itemstack.getItem()
                .isPotionIngredient(itemstack)) return false;
            else if (itemstack.getItem() == ModItems.DRAGON_BREATH.get()) {
                for (int i = 0; i < 3; i++) if (inventory[i] != null && inventory[i].getItem() == Items.potionitem)
                    if (ItemPotion.isSplash(inventory[i].getItemDamage())) return true;
                return false;
            } else {
                boolean flag = false;

                for (int i = 0; i < 3; i++) if (inventory[i] != null && inventory[i].getItem() instanceof ItemPotion) {
    ...
    private void brewPotions() {
        if (ForgeEventFactory
            .onPotionAttemptBreaw(new ItemStack[] { inventory[0], inventory[1], inventory[2], inventory[3] })) return;
        if (canBrew()) {
            for (int i = 0; i < 3; i++) if (inventory[i] != null && inventory[i].getItem() instanceof ItemPotion) {
                int j = inventory[i].getItemDamage();
                if (ItemPotion.isSplash(j) && inventory[3].getItem() == ModItems.DRAGON_BREATH.get())
                    inventory[i] = ModItems.LINGERING_POTION
                        .newItemStack(inventory[i].stackSize, inventory[i].getItemDamage());
                else {
                    ...
                    if ((j <= 0 || list != list1) && (list == null || !list.equals(list1) && list1 != null)) {
                        if (j != k) inventory[i].setItemDamage(k);
                    } else if (!ItemPotion.isSplash(j) && ItemPotion.isSplash(k)) inventory[i].setItemDamage(k);
                }
            }

            boolean hasContainerItem = inventory[3].getItem()
                .hasContainerItem(inventory[3]);
            if (--inventory[3].stackSize <= 0) inventory[3] = hasContainerItem ? inventory[3].getItem()
                .getContainerItem(inventory[3]) : null;
            ...
            fuel--;
            ForgeEventFactory
                .onPotionBrewed(new ItemStack[] { inventory[0], inventory[1], inventory[2], inventory[3] });
            worldObj.playSound(
                xCoord,
                yCoord,
                zCoord,
                Reference.MCAssetVer + ":block.brewing_stand.brew",
                1.0F,
                1.0F,
                true);
        }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        else if (slot == 3) return stack.getItem()
            .isPotionIngredient(stack);
        else return stack.getItem() instanceof ItemPotion || stack.getItem() == Items.glass_bottle;
    }
    ...
    public int getFilledSlots() {
        ...
        for (int j = 0; j < 3; j++) if (inventory[j] != null) i |= 1 << j;
