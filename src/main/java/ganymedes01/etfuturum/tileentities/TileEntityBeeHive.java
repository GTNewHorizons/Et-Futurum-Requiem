...
public class TileEntityBeeHive extends TileEntity {
    ...
    public boolean isNearFire() {
        if (this.getWorldObj() != null) {
            for (BlockPos blockpos : BlockPos
                .iterate(xCoord - 1, yCoord - 1, zCoord - 1, xCoord + 1, yCoord + 1, zCoord + 1)) {
                if (blockpos.getBlock(getWorldObj())
    ...
    public static boolean isLitCampfireBelow(World world, int x, int y, int z, int spacing) {
        for (int i = 1; i <= spacing; i++) {
            ...
            if ((TEMPORARY_FIRES_LIST.isEmpty() && block.getMaterial() == Material.fire)
                || TEMPORARY_FIRES_LIST.contains(RegistryMapping.getKeyFor(block, meta))) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Will be replaced when HogUtils exists and has tags. Has an adder func so people can add fires, will redirect to
     * using the tag when HogUtils gets them.
    ...
    public void tryEnterHive(Entity p_226962_1_, boolean p_226962_2_, int p_226962_3_) {
        if (getBeeCount() < maxHiveSize() && p_226962_1_ instanceof EntityBee) {
            ...
            ((EntityBee) p_226962_1_).clearLeashed(
                true,
            ...
            // writeToNBT doesn't save the entity ID in its own NBT, it's probably saved somewhere else. So we manually
            // add it to the tags for the beehive
            ...
            if (this.getWorldObj() != null) {
                ...
                // We want to make sure the entity is spawned, before playing the noise. This is because hives can
                // generate naturally or with saplings.
                // Natural generation uses a dummy entity to generate bee data for the hive and then calls this function
                // to add them, but doesn't actually spawn them.
                // We don't want trees to make a loud bee pop, and since "fake" bees never spawn addedToChunk will be
                // false for them, so that's why we do this.
                if (beeentity.addedToChunk) {
                    this.getWorldObj()
                        .playSoundEffect(
                            ...
                            yCoord + 0.5,
                            zCoord + 0.5,
                            Reference.MCAssetVer + ":block.beehive.enter",
                            1.0F,
                            1.0F);
                }
            }
    ...
    private boolean releaseBee(NBTTagCompound p_226967_2_, List<Entity> p_226967_3_,
        TileEntityBeeHive.State p_226967_4_) {
        if ((this.getWorldObj()
            .isDaytime()
            && !this.getWorldObj()
                .isRaining())
            || p_226967_4_ == State.EMERGENCY) {
            EnumFacing direction = EnumFacing.getFront(getWorldObj().getBlockMetadata(xCoord, yCoord, zCoord) % 6);
            Block front = getWorldObj()
                .getBlock(xCoord + direction.getFrontOffsetX(), yCoord, zCoord + direction.getFrontOffsetZ());
            boolean flag = front.isOpaqueCube() || front.getMaterial()
                .isLiquid();
                ...
                if (entity instanceof EntityBee) {
                    ...
                    if (p_226967_4_ == State.HONEY_DELIVERED) {
                        ...
                        if (getWorldObj().getBlock(xCoord, yCoord, zCoord) instanceof BlockBeeHive) {
                            ...
                            if (i < 5) {
                                ...
                                setHoneyLevel(i + j);
                                BlockBeeHive
                                    .updateHiveState(getWorldObj(), xCoord, yCoord, zCoord, getHoneyLevel() == 5);
                            }
                        }
                    }
                    ...
                    this.getWorldObj()
                        .playSoundEffect(
                            ...
                            yCoord + 0.5,
                            zCoord + 0.5,
                            Reference.MCAssetVer + ":block.beehive.exit",
                            1.0F,
                            1.0F);
    ...
    private void tickBees() {
        ...
        while (iterator.hasNext()) {
            ...
            if (beehivetileentity$bee.ticksInHive > beehivetileentity$bee.minOccupationTicks) {
                ...
                TileEntityBeeHive.State beehivetileentity$state = compoundnbt.getBoolean("HasNectar")
                    ? TileEntityBeeHive.State.HONEY_DELIVERED
                    : TileEntityBeeHive.State.BEE_RELEASED;
            ...
            if (getBeeCount() > 0 && this.getWorldObj().rand.nextDouble() < 0.005D) {
                this.getWorldObj()
                    .playSoundEffect(
                        ...
                        yCoord + 0.5,
                        zCoord + 0.5,
                        Reference.MCAssetVer + ":block.beehive.work",
                        1.0F,
                        1.0F);
            }
        }
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < listnbt.tagCount(); ++i) {
            ...
            TileEntityBeeHive.Bee beehivetileentity$bee = new TileEntityBeeHive.Bee(
                compoundnbt.getCompoundTag("EntityData"),
                compoundnbt.getInteger("TicksInHive"),
                compoundnbt.getInteger("MinOccupationTicks"));
