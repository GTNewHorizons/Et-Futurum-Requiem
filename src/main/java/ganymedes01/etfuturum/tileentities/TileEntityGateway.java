...
public class TileEntityGateway extends TileEntity {
        ...
        if (!this.worldObj.isRemote && !this.isCoolingDown()) {
            ...
            if (this.exitPortal != null) {
                ...
                if (entityIn instanceof EntityLivingBase) {
                    ((EntityLivingBase) entityIn).setPositionAndUpdate(
    ...
    private void findExitPortal() {
        ...
        for (int i = 16; getChunk(this.worldObj, vec3d1).getTopFilledSegment() > 0 && i-- > 0; vec3d1 = vec3d1
            .addVector(vec3d.xCoord * -16.0D, vec3d.yCoord * -16.0D, vec3d.zCoord * -16.0D)) {
        ...
        for (int j = 16; getChunk(this.worldObj, vec3d1).getTopFilledSegment() == 0
            && j-- > 0; vec3d1 = vec3d1.addVector(vec3d.xCoord * 16.0D, vec3d.yCoord * 16.0D, vec3d.zCoord * 16.0D)) {
        ...
        if (this.exitPortal == null) {
            ...
            (new WorldGenEndIsland()).generate(
                this.worldObj,
                new Random(this.exitPortal.toLong()),
                exitPortal.getX(),
                exitPortal.getY(),
                exitPortal.getZ());
        } else {
    ...
    private static Chunk getChunk(World worldIn, Vec3 vec3) {
        return worldIn.getChunkFromChunkCoords(
            MathHelper.floor_double(vec3.xCoord / 16.0D),
            ...
            for (int y = yfrom; y <= yto; y++) {
                for (int z = zfrom; z <= zto; z++) {
                    if (chunkIn.worldObj.getBlock(x, y, z) == Blocks.end_stone
                        && !chunkIn.worldObj.getBlock(x, y + 1, z)
                            .isNormalCube()
                        && !chunkIn.worldObj.getBlock(x, y + 2, z)
                            .isNormalCube()) {
                        double dis1 = x + 0.5D;
                        double dis2 = y + 0.5D;
                        double dis3 = z + 0.5D;
    ...
    public boolean shouldRenderFace(EnumFacing p_184313_1_) {
        return this.getBlockType()
            .shouldSideBeRendered(this.worldObj, xCoord, yCoord, zCoord, p_184313_1_.ordinal());
    }
    ...
    public int getParticleAmount() {
        ...
        for (EnumFacing enumfacing : Utils.ENUM_FACING_VALUES) {
            if (this.shouldRenderFace(enumfacing)) i++;
        }
