...
public class TileEntityBarrel extends TileEntity implements IInventory {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("Type")) { // compat with barrels placed before iron barrels were added
            this.type = ConfigModCompat.barrelIronChest ? BarrelType.VALUES[compound.getByte("Type")]
                : BarrelType.VANILLA;
        } else {
    ...
    public void updateEntity() {
        ...
        if (this.numPlayersUsing > 0 && this.soundTimer <= 0.0F) {
            ...
            this.worldObj.setBlockMetadataWithNotify(
                ...
                yCoord,
                zCoord,
                this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord) % 8 + 8,
                2);
            this.worldObj.playSoundEffect(
                d1,
                this.yCoord + 0.5D,
                d2,
                Reference.MCAssetVer + ":block.barrel.open",
                0.5F,
        ...
        if (this.numPlayersUsing == 0 && this.soundTimer > 0.0F || this.numPlayersUsing > 0 && this.soundTimer < 1.0F) {
            ...
            if (this.soundTimer < f1 && worldObj.getBlockMetadata(xCoord, yCoord, zCoord) > 7) {
                this.worldObj.setBlockMetadataWithNotify(
                    ...
                    yCoord,
                    zCoord,
                    this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord) % 8,
                    2);
                this.worldObj.playSoundEffect(
                    this.xCoord + 0.5D,
                    this.yCoord + 0.5D,
                    this.zCoord + 0.5D,
                    Reference.MCAssetVer + ":block.barrel.close",
                    0.5F,
                    this.worldObj.rand.nextFloat() * 0.1F + 0.9F);
            }
