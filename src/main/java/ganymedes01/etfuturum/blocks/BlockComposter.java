...
public class BlockComposter extends Block implements FakeTileEntityProvider {
    ...
    public static boolean addToComposter(World world, int x, int y, int z, ItemStack stack) {
        ...
        if (chance > 0) {
            /*
             * Takes the chance (which can be up to 600) and multiplies it by 0.01F (same as dividing by 100) to see how
             * many times it passes 100
             * Then I wrap it by 100 and check that against the chance to see if we should add one more to the fill
             * level.
             */
            ...
            if (fillAmount > 0) {
                ...
                world.playSoundEffect(
                    ...
                    y + 0.5D,
                    z + 0.5D,
                    Reference.MCAssetVer + ":block.composter.fill_success",
                    1,
                    1);
                ...
                world.playSoundEffect(
                    ...
                    y + 0.5D,
                    z + 0.5D,
                    Reference.MCAssetVer + ":block.composter.fill",
                    1,
                    1);
            }
            return true;
        ...
        if (meta < FULL_META) {
            ...
            if (chance > 0) {
                ...
                for (int i = 0; i <= 5; i++) {
                    world.spawnParticle(
                        "happyVillager",
                        ((world.rand.nextDouble() * 0.875D) + 0.125D) + x,
                        0.25D + (0.125D * meta) + y,
                        ((world.rand.nextDouble() * 0.875D) + 0.125D) + z,
                        0,
                        0,
                        0);
                }
            ...
            if (!world.isRemote) {
                EntityItem item = new EntityItem(
                    ...
                    y + 1,
                    z + 0.5D,
                    EtFuturumLootTables.COMPOSTER_LOOT.getOneItem(world.rand));
    ...
    public TileEntity getFakeTileEntity(World world, int x, int y, int z) {
        ...
        if (meta < FULL_META) targetInv = new InventoryEmptyComposter(world, x, y, z);
        else if (meta == HARVESTABLE_META) targetInv = new InventoryFullComposter(world, x, y, z);
        else targetInv = new InventoryDummy(world, x, y, z);
        ...
        public boolean canExtractItem(int p_102008_1_, ItemStack p_102008_2_, int p_102008_3_) {
            if (p_102008_3_ == 0) {
                world.playSound(
                    ...
                    y + 0.5D,
                    z + 0.5D,
                    Reference.MCAssetVer + ":block.composter.empty",
                    1,
                    1,
                    true);
                world.setBlockMetadataWithNotify(x, y, z, 0, 3);
