...
public class BlockChorusPlant extends Block {
    ...
    public boolean canSurviveAt(World world, int x, int y, int z) {
        ...
        for (EnumFacing enumfacing : Utils.ENUM_FACING_VALUES) {
            ...
            if (block == this) {
                ...
                Block block1 = world
                    .getBlock(x + enumfacing.getFrontOffsetX(), y - 1, z + enumfacing.getFrontOffsetZ());
    ...
    public static boolean canPlaceOn(Block block) {
        return block == Blocks.end_stone || block == ExternalContent.Blocks.ENDERLICIOUS_END_ROCK.get()
            || block == ExternalContent.Blocks.HEE_END_STONE.get();
    }
