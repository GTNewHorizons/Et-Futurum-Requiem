...
public class BlockGeneralModdedDeepslateOre extends BaseSubtypesBlock implements IInitAction {
    ...
    // Uses BaseSubtypesBlock instead of BaseSubtypesDeepslateOre as the latter is designed for individual mod support
    // rather than the way this is used.
    ...
    public BlockGeneralModdedDeepslateOre(String... names) {
        ...
        for (int i = 0; i < names.length; i++) {
            ores[i] = "ore" + StringUtils.capitalize(
                names[i].replaceFirst("^deepslate_", "")
                    .replace("_ore", ""));
        }
    ...
    public void getSubBlocks(Item item, CreativeTabs tabs, List<ItemStack> list) {
        for (int i = 0; i < ores.length; i++) {
            ...
            if (Utils.listGeneralModdedDeepslateOre(ores[i])
                || Utils.listGeneralModdedDeepslateOre(ores[i].replace("Mythril", "Mithril"))) {
                list.add(stack);
            }
        }
    }
    ...
    public void onLoadAction() {
        ...
        for (int i = 0; i < ores.length; i++) {
            ...
            // We do this cursed shit because these functions have no meta input. So we have to place it to "simulate"
            // the method to get an accurate output for that specific meta.
            // Since many mod ores are based on meta we need to do this to ensure the returned resistance of the block
            // is accurate. There is probably a better way to do this.
            // This also won't be "accurate" if the block returns different stuff depending on the input functions, but
            // we can document and make special code for such cases as they come along.
            ...
            try {
                ...
                hardnesses[i] = ConfigFunctions.useStoneHardnessForDeepslate ? block.getBlockHardness(world, 0, 0, 0)
                    : block.getBlockHardness(world, 0, 0, 0) * 1.5F;
                resistances[i] = block.getExplosionResistance(null, world, 0, 0, 0, 0, 0, 0); // We don't need to divide
                                                                                              // because the base method
                                                                                              // we overrode won't be
                                                                                              // dividing
            } catch (Exception e) {
                setHarvestLevel("pickaxe", 1, i);
                hardnesses[i] = ConfigFunctions.useStoneHardnessForDeepslate ? Blocks.iron_ore.blockHardness
                    : Blocks.iron_ore.blockHardness * 1.5F;
