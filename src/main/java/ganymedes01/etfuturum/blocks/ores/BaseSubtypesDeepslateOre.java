...
public abstract class BaseSubtypesDeepslateOre extends BaseSubtypesBlock implements IInitAction {
    ...
    public Item getItemDropped(int meta, Random rand, int fortune) {
        return checkDrop(
            new ItemStack(getBase(meta).getItemDropped(getBaseMeta(meta), rand, fortune), 1, fortune),
            meta).getItem();
    }
    ...
    // @Override //TODO: Figure out how to capture meta in case a mod overrides these. Although it's unlikely this is
    // needed so it's "FINE" for now
     ...
     * If it drops the base ore block, replace it with the base deepslate block (eg iron ore drop is swapped for
     * deepslate iron ore drop)
     */
    ...
    public void onLoadAction() {
        ...
        for (int i = 0; i < getTypes().length; i++) {
            ...
            try {
                ...
                setHardnessValues(
                    ConfigFunctions.useStoneHardnessForDeepslate ? block.getBlockHardness(world, 0, 0, 0)
                ...
                setHardnessValues(
                    ConfigFunctions.useStoneHardnessForDeepslate ? Blocks.iron_ore.blockHardness
