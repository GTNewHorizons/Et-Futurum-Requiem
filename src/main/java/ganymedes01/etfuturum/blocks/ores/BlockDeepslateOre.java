 ...
 * So referencing other modded blocks would cause crashes, use BaseDeepslateOre for registering deepslate ores for other
 * mods.
 */
public class BlockDeepslateOre extends BaseDeepslateOre {
    ...
    public BlockDeepslateOre(Block block, boolean defaultMapping) {
        ...
        copyAttribs(block);
        base = block;
        // We don't do this stuff in the constructor normally since modded blocks might not be loaded right now, but
        // vanilla blocks are here so it's fine.
        if (getClass().getName()
            .startsWith("ganymedes01.etfuturum")) { // We only want to do this on my own stuff, not mods that extend it.
