...
public class BlockDeepslateCopperOre extends BlockDeepslateOre implements IInitAction {
    ...
    public void onLoadAction() {
        if (!ModBlocks.COPPER_ORE.isEnabled() && !OreDictionary.getOres("oreCopper")
            .isEmpty()) {
            // If EFR copper ore isn't enabled, find another mod copper to pull stats from. Fallback to EFR copper stats
            ...
            try {
                ...
                blockResistance = block.getExplosionResistance(null, world, 0, 0, 0, 0, 0, 0); // Because the game
                                                                                               // divides it by 5 for
                                                                                               // some reason
            } catch (Exception ignored) {
                setHarvestLevel("pickaxe", 1);

                blockHardness = ConfigFunctions.useStoneHardnessForDeepslate ? ModBlocks.COPPER_ORE.get().blockHardness
                    : ModBlocks.COPPER_ORE.get().blockHardness * 1.5F;
