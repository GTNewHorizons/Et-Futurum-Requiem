...
public abstract class BaseDeepslateOre extends BaseBlock implements IInitAction {
    ...
    public void postInitAction() {
        if (!getBase().getClass()
            .getName()
            .contains("net.minecraft.block") && getBase() != ModBlocks.COPPER_ORE.get()) {
            loaded.add(this);
        }
    }
    ...
    public void onLoadAction() {
        ...
        try {
            ...
            blockHardness = ConfigFunctions.useStoneHardnessForDeepslate ? block.getBlockHardness(world, 0, 0, 0)
                : block.getBlockHardness(world, 0, 0, 0) * 1.5F;
            blockResistance = block.getExplosionResistance(null, world, 0, 0, 0, 0, 0, 0) * 5; // Because the game
                                                                                               // divides it by 5 for
                                                                                               // some reason
        } catch (Exception e) {
            setHarvestLevel("pickaxe", 1);
            blockHardness = ConfigFunctions.useStoneHardnessForDeepslate ? Blocks.iron_ore.blockHardness
