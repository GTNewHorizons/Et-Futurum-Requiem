...
public class BlockShulkerBox extends BlockContainer {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack) {
        ...
        if (stack.hasTagCompound()) {
            box.type = ShulkerBoxType.VALUES[stack.getTagCompound()
                .getByte("Type")];
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float subX,
        ...
        if (world.getTileEntity(x, y, z) instanceof TileEntityShulkerBox shulker) {
            ...
            if (!player.isSneaking() && player.getHeldItem() != null
                && player.getHeldItem()
                    .getItem() == ModItems.SHULKER_BOX_UPGRADE.get()) {
                ItemStack stack = player.getHeldItem();
                String[] upgrades = ((BaseSubtypesItem) player.getHeldItem()
                    .getItem()).types[stack.getItemDamage()].split("_");
                if (upgrades[0].equals(
                    shulker.type.toString()
                        .toLowerCase())) {
                    ItemStack[] tempCopy = shulker.chestContents == null ? new ItemStack[shulker.getSizeInventory()]
                        : ArrayUtils.clone(shulker.chestContents);
    ...
    //
    // if(tileentity instanceof TileEntityShulkerBox && ((TileEntityShulkerBox)tileentity).getAnimationStatus() ==
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return tileentity instanceof TileEntityShulkerBox
            ? ((TileEntityShulkerBox) tileentity).getBoundingBox(((TileEntityShulkerBox) tileentity).facing)
                .offset(x, y, z)
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int meta, EntityPlayer player) {
        ...
        if (ForgeEventFactory.fireBlockHarvesting(
            drops,
            world,
            this,
            x,
    ...
    public boolean addHitEffects(World world, MovingObjectPosition target, EffectRenderer effectRenderer) {
        ...
        if (block.getMaterial() != Material.air && TE != null) {
            ...
            effectRenderer.addEffect(
                (dig).applyColourMultiplier(x, y, z)
