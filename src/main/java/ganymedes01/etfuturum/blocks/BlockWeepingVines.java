...
public class BlockWeepingVines extends BaseBlock implements IShearable {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (rand.nextFloat() <= .1F && world.getBlockMetadata(x, y, z) == 0
    ...
    public int quantityDropped(int meta, int fortune, Random random) {
        // We do this because we don't want the block to use this logic if it's broken by shears. Without BugTorch
        // installed this will run even when shearing them.
        // This is because Forge made a huge sin and shearing does not cancel the normal block drops. This causes
        // duping.
        if (harvesters.get() == null || (harvesters.get()
            .getHeldItem() != null
            && !(harvesters.get()
                .getHeldItem()
    ...
    public int quantityDropped(Random random) {
        // Just to have all the logic in one spot. This should only fire when not broken by a player, so 0 for the args
        // should be fine.
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        return world.getBlock(x, y + 1, z)
            .isOpaqueCube() || world.getBlock(x, y + 1, z) == this;
    }
