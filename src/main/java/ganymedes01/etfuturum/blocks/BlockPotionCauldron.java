...
public class BlockPotionCauldron extends BlockCauldronTileEntity {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random random) {
        if (random.nextInt(30) == 0) {
            ...
            // Slightly decrease the bounds in which the particles can spawn so they don't go through the inside of the
            // cauldron walls
            float min = 0.1875F;
            float max = 0.8125F;
            ...
            world.spawnParticle(
                "mobSpell",
                d0,
                y + BlockCauldron.getRenderLiquidLevel(world.getBlockMetadata(x, y, z) + 1),
                d1,
                r,
                g,
                b);
    ...
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer entityPlayer, int side, float subX,
        ...
        if (stack != null) {
            ...
            if (item == Items.potionitem) {
                ...
                if (effects == null || !effects
                    .equals(((ItemPotion) potionCauldron.potion.getItem()).getEffects(potionCauldron.potion))) {
                    EnumCauldronFillAction.EVAPORATE.getAction(world, x, y, z, false);// TODO make this evaporate with
                                                                                      // buckets too
                ...
                ItemStack tippedArrow = ModItems.TIPPED_ARROW
                    .newItemStack(Math.min(stack.stackSize, tipmax), potionCauldron.potion.getItemDamage());
                final int setMeta = tippedArrow.stackSize >= tipmax ? -1 : meta - (tippedArrow.stackSize < 32 ? 1 : 2);

                if (!Items.potionitem.getEffects(potionCauldron.potion)
                    .isEmpty() && potionCauldron.potion.hasTagCompound()
                    && potionCauldron.potion.getTagCompound()
                        .hasKey("CustomPotionEffects", 9)) {
    ...
    public void registerBlockIcons(IIconRegister reg) {
        blockIcon = reg
            .registerIcon(GrayscaleWaterResourcePack.createGrayscaleName("water_still", GrayscaleType.TINT_INVERSE));
    }
            ...
            public void getAction(World world, int x, int y, int z, boolean filling) {
                ...
                if (ConfigSounds.fluidInteract) {
                    world.playSoundEffect(
                        ...
                        liquidLevel,
                        z + 0.5D,
                        Reference.MCAssetVer + ":item.bottle." + (filling ? "fill" : "empty"),
                        1,
                        1);
                }
                for (int i = 0; i < world.rand.nextInt(4) + 4; i++) {
                    world.spawnParticle(
                        "mobSpell",
                        this.getParticleXYCoord(x, world.rand),
                        liquidLevel,
                        this.getParticleXYCoord(z, world.rand),
                        r,
                        g,
                        b);
                }
        ...
        EVAPORATE {
            ...
            public void getAction(World world, int x, int y, int z, boolean filling) {
                ...
                world.playSound(
                    ...
                    "random.fizz",
                    0.3F,
                    (world.rand.nextFloat() * 0.6F) + 0.4F,
                    false);
                for (int i = 0; i < world.rand.nextInt(4) + 4; i++) {
                    world.spawnParticle(
                        "explode",
