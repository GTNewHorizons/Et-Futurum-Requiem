...
public class ItemBlockShulkerBox extends ItemBlock {
    ...
    public String getUnlocalizedName(ItemStack stack) {
        int type = stack.hasTagCompound() ? stack.getTagCompound()
            .getByte("Type") : 0;
        int color = stack.hasTagCompound() ? stack.getTagCompound()
            .getByte("Color") : 0;

        String string = field_150939_a/* blockInstance */.getUnlocalizedName()
            .substring(15);
        if (type > 0 && type - 1 < TileEntityShulkerBox.tiers.length) {
            string = TileEntityShulkerBox.tiers[type - 1] + "_" + string;
        }
        if (color > 0) {
            String dye = ModRecipes.dye_names[stack.getTagCompound()
                .getByte("Color") - 1 % ModRecipes.dye_names.length];
            string = dye + "_" + string;
        }
    ...
    public boolean onItemUse(ItemStack p_77648_1_, EntityPlayer p_77648_2_, World p_77648_3_, int p_77648_4_,
        ...
        if (block == Blocks.cauldron && meta > 0
            && p_77648_1_.hasTagCompound()
            && p_77648_1_.getTagCompound()
                .hasKey("Color")) {
            p_77648_1_.getTagCompound()
                .removeTag("Color");
            if (p_77648_1_.getTagCompound()
                .hasNoTags()) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> lore, boolean f3h) {
        if (ConfigFunctions.shulkerBoxTooltipLines > 0 && stack.getTagCompound() != null
            && stack.getTagCompound()
                .hasKey("Items")) {
            NBTTagList tag = stack.getTagCompound()
                .getTagList("Items", 10);
            int items = 0;
