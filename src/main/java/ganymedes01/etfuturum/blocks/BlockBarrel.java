...
public class BlockBarrel extends BlockContainer {
    ...
    public BlockBarrel(TileEntityBarrel.BarrelType type) {
        ...
        if (type == TileEntityBarrel.BarrelType.VANILLA) {
            ...
            this.setBlockName(
                Utils.getUnlocalisedName(
                    type.name()
                        .toLowerCase() + "_barrel"));
            this.setBlockTextureName(
                "metalbarrels:" + type.name()
                    .toLowerCase() + "_barrel");
        }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float subX,
        ...
        if (player.isSneaking() || barrel.numPlayersUsing != 0
            || player.getHeldItem() == null
            || player.getHeldItem()
                .getItem() != ModItems.BARREL_UPGRADE.get()) {
            player.openGui(EtFuturum.instance, GUIIDs.BARREL, world, x, y, z);
            return true;
        }

        ItemStack upgradeStack = player.getHeldItem();
        String[] upgradeStrings = ((BaseSubtypesItem) player.getHeldItem()
            .getItem()).types[upgradeStack.getItemDamage()].split("_");
        if (upgradeStrings.length < 3 || !upgradeStrings[0].equals(
            barrel.type.toString()
                .toLowerCase())) {
            return false;
        }

        barrel.upgrading = true;
        ItemStack[] tempCopy = barrel.chestContents == null ? new ItemStack[barrel.getSizeInventory()]
            : ArrayUtils.clone(barrel.chestContents);
        TileEntityBarrel newTE = (TileEntityBarrel) TileEntityBarrel.BarrelType.valueOf(upgradeStrings[1].toUpperCase())
            .getBlock()
            .createTileEntity(world, barrel.getBlockMetadata());
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        ...
        if (tileEntityBarrel != null && !tileEntityBarrel.upgrading) {
            for (int i1 = 0; i1 < tileEntityBarrel.getSizeInventory(); ++i1) {
                ...
                for (float zOffset = world.rand.nextFloat() * 0.8F + 0.1F; itemstack.stackSize > 0; world
                    ...
                    entityitem = new EntityItem(
                        ...
                        y + yOffset,
                        z + zOffset,
                        new ItemStack(itemstack.getItem(), itemCount, itemstack.getItemDamage()));
