...
public class BlockChorusFlower extends Block {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (!canBlockStay(world, x, y, z)) world.func_147480_a(x, y, z, true); // breakBlock
        else if (world.isAirBlock(x, y + 1, z)) {
            boolean canGrowUp = false;
            boolean isSegmentOnEndstone = false;
            Block lowerBlock = world.getBlock(x, y - 1, z);
            if (BlockChorusPlant.canPlaceOn(lowerBlock)) canGrowUp = true;
                ...
                for (height = 0; height < 4; height++) {
                    ...
                    if (b != ModBlocks.CHORUS_PLANT.get()) {
                        if (BlockChorusPlant.canPlaceOn(b)) isSegmentOnEndstone = true;
                        break;
                    }
                    par8++;
                }
                ...
                height = 4;
                if (isSegmentOnEndstone) height++;

                if (par8 < 2 || rand.nextInt(height) >= par8) canGrowUp = true;
            } else if (lowerBlock.isAir(world, x, y - 1, z)) canGrowUp = true;

            if (canGrowUp && areAllNeighborsEmpty(world, x, y + 1, z, ForgeDirection.DOWN)
                ...
                boolean grew = false;
                if (isSegmentOnEndstone) tries++;
                for (int i = 0; i < tries; i++) {
                    ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[rand
                ...
                if (grew) world.setBlock(x, y, z, ModBlocks.CHORUS_PLANT.get(), 0, 3);
                else setFlower(world, x, y, z, 5);
            } else if (meta == 4) setFlower(world, x, y, z, 5);
        }
    }
    ...
    private void setFlower(World world, int x, int y, int z, int age) {
        ...
        world.playSoundEffect(
            ...
            y + 0.5D,
            z + 0.5D,
            Reference.MCAssetVer + ":block.chorus_flower." + (age < 5 ? "grow" : "death"),
            1,
            1);
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighbour) {
        if (!canBlockStay(world, x, y, z)) world.func_147480_a(x, y, z, false); // breakBlock
    }
        ...
        if (block != ModBlocks.CHORUS_PLANT.get() && !BlockChorusPlant.canPlaceOn(block)) {
            if (block.isAir(world, x, y - 1, z)) {
                ...
                for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if (adjecentBlock == ModBlocks.CHORUS_PLANT.get()) adjecentCount++;
    ...
    public static boolean areAllNeighborsEmpty(World world, int x, int y, int z, ForgeDirection skip) {
        ...
        for (ForgeDirection enumfacing : horizontal) {
            if (enumfacing != skip
                && !world.isAirBlock(x + enumfacing.offsetX, y + enumfacing.offsetY, z + enumfacing.offsetZ)) {
                return false;
            }
        }
    ...
    private static void growTreeRecursive(World worldIn, int x, int y, int z, int x1, int y1, int z1, Random rand,
        ...
        if (p_185601_5_ < 4) {
            ...
            for (int k = 0; k < l; ++k) {
                ...
                if (Math.abs(bp1x - x1) < p_185601_4_ && Math.abs(bp1z - z1) < p_185601_4_
                    && worldIn.isAirBlock(bp1x, bp1y, bp1z)
                    && worldIn.isAirBlock(bp1x, bp1y - 1, bp1z)
                    && areAllNeighborsEmpty(worldIn, bp1x, bp1y, bp1z, ForgeDirection.getOpposite())) {
                    flag = true;
