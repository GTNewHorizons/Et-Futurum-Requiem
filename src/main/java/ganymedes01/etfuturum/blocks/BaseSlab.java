...
public class BaseSlab extends BlockSlab implements ISubBlocksBlock {
    ...
    /**
     * Used to know the previous slab registered, so the slab knows what the double slab version is without passing it
     * as a constructor argument every time
    ...
    public BaseSlab(boolean isDouble, Material material, String... names) {
        ...
        if (names.length > 8) {
            throw new IllegalArgumentException(
                "Slabs can't have more than 8 subtypes! Tried to register a slab with " + names.length);
        }
        types = names;
    ...
    public String func_150002_b(int meta) {
        ...
        type = ("".equals(type) ? getUnlocalizedName().replace("tile.", "")
            .replace("etfuturum.", "") : type);
        return type.replace("bricks", "brick")
