...
public class BlockBeeHive extends BlockContainer {
    ...
    public IIcon getIcon(int side, int meta) {
        switch (side) {
            ...
            case 1:
                return topIcon;
            default:
                return (side == meta % 6) || (side == 3 && meta == 0) ? meta > 5 ? frontIconHoney : frontIcon
                    : blockIcon;
        }
    }
            ...
            if (!world.getBlock(x, y - 1, z)
                ...
                for (int i = 0; i < random.nextInt(1) + 1; ++i) {
                    if (random.nextFloat() > 0.3F) {
                        CustomParticles.spawnDrippingParticleWithSound(
                            world,
                            world.rand.nextDouble() + x,
                            y,
                            world.rand.nextDouble() + z,
                            Reference.MCAssetVer + ":block.beehive.drip",
                            0xFF8E6D14);
                    }
                }
            }
        ...
        if (!world.isRemote) {
            ...
            if (tileentity instanceof TileEntityBeeHive beehivetileentity) {
                ...
                if ((!empty && player.capabilities.isCreativeMode) || silk) {
                    ...
                    ForgeEventFactory
                        .fireBlockHarvesting(itemStacks, world, this, x, y, z, meta, fortune, 1.0F, silk, player);
                ...
                beehivetileentity.destroyAllBees(); // Deletes all bees from the hive so breakBlock doesn't release
                                                    // duplicate bees
            }
        }
    ...
    public void onPostBlockPlaced(World world, int x, int y, int z, int meta) {
        ...
        if (tileentity instanceof TileEntityBeeHive) {
            ...
            if (world.getBlock(x + facing.getFrontOffsetX(), y, z + facing.getFrontOffsetZ())
