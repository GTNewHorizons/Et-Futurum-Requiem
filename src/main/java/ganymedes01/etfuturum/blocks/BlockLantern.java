...
public class BlockLantern extends Block {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess worldIn, int x, int y, int z) {
        ...
        else this.setBlockBounds(0.5F - f1, r * 1, 0.5F - f1, 0.5F + f1, r * 8, 0.5F + f1);
    }
    ...
    private boolean canPlaceAbove(World world, int x, int y, int z) {
        ...
        return world.isSideSolid(x, y + 1, z, ForgeDirection.DOWN)
            || (above == ModBlocks.CHAIN.get() && world.getBlockMetadata(x, y + 1, z) == 0)
            || above instanceof BlockFence
            || above instanceof BlockWall
            || (above instanceof BlockSlab && aboveMeta < 8)
            || (above instanceof BlockStairs && aboveMeta < 4);
    }

    private boolean canPlaceBelow(World world, int x, int y, int z) {
        return world.getBlock(x, y - 1, z)
            .canPlaceTorchOnTop(world, x, y - 1, z) || World.doesBlockHaveSolidTopSurface(world, x, y - 1, z);
    }
