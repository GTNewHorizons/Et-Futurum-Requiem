...
public class BlockBubbleColumn extends BaseBlock implements IInitAction {
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        for (char a = 'a'; i < largest && a < 'z'; a++, i++) {
            if (i < innerIconCount()) {
                inner_icons[i] = reg
                    .registerIcon(getTextureName() + "_inner_" + (char) (a + (isUp ? innerIconCount() : 0)));
            }
            if (i < outerIconCount()) {
                outer_icons[i] = reg
                    .registerIcon(getTextureName() + "_outer_" + (char) (a + (isUp ? outerIconCount() : 0)));
            }
            if (i < topIconCount()) {
    ...
    public IIcon getIcon(IBlockAccess worldIn, int x, int y, int z, int side) {
        ...
        if (side < 2) {
            ...
            return renderingInner.get() ? inner_icons[pseudoRand % inner_icons.length]
                : outer_icons[pseudoRand % outer_icons.length];
        }
    }
    ...
    public void randomDisplayTick(World worldIn, int x, int y, int z, Random random) {
        ...
        if (random.nextInt(256) == 0) {
            worldIn.playSound(
                ...
                y + random.nextFloat(),
                z + random.nextFloat(),
                getBubblingNoise(worldIn, x, y, z, random),
                1,
                1,
                false);
        }
    ...
    public void postInitAction() {
        if (ModRecipes.validateItems(this)) {
            supportBlocks.stream()
                .filter(ModRecipes::validateItems)
                .forEach(block -> EtFuturumWorldListener.bubbleColumnMap.put(block, this));
        }
    }
