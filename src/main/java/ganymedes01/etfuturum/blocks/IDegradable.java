...
public interface IDegradable {
     ...
     * @param worldMeta World meta (Is always the meta value from before the copper changed and is the actual block
     *                  meta, not the copper ID)
    ...
    default void tryDegrade(World world, int x, int y, int z, Random random) {
        ...
        if (i < 7 && i % 4 != 3) {
            for (int x1 = -CHECK_RANGE; x1 <= CHECK_RANGE; x1++) {
                for (int y1 = -CHECK_RANGE; y1 <= CHECK_RANGE; y1++) {
                    for (int z1 = -CHECK_RANGE; z1 <= CHECK_RANGE; z1++) {
                        ...
                        if (block instanceof IDegradable && (x1 != 0 || y1 != 0 || z1 != 0)
                            && Math.abs(x1) + Math.abs(y1) + Math.abs(z1) <= CHECK_RANGE) {
                            int m = ((IDegradable) block)
                                .getCopperMeta(world, x, y, z, world.getBlockMetadata(x1 + x, y1 + y, z1 + z));
            ...
            if (random.nextFloat() < g) {
                setCopperBlock(
                    getCopperBlockFromMeta(i + 1),
                    getFinalCopperMeta(world, x, y, z, i + 1, world.getBlockMetadata(x, y, z)),
                    world,
                    x,
                    y,
                    z);
    ...
    default boolean tryWaxOnWaxOff(World world, int x, int y, int z, EntityPlayer entityPlayer) {
        ...
        if (entityPlayer.getCurrentEquippedItem() != null) {
            ...
            if (flag && !flag2) {
                ...
                setCopperBlock(
                    block,
                    getFinalCopperMeta(world, x, y, z, waxMeta, world.getBlockMetadata(x, y, z)),
                    world,
                    x,
                    y,
                    z);
                ...
                setCopperBlock(
                    block,
                    getFinalCopperMeta(world, x, y, z, meta - 1, world.getBlockMetadata(x, y, z)),
                    world,
                    x,
                    y,
                    z);
    ...
    default void playSound(World world, double x, double y, double z, int type) {
        ...
        world.playSound(
            ...
            y,
            z,
            Reference.MCAssetVer + ":item."
                + (type == 0 ? "honeycomb.wax_on" : type == 1 ? "axe.wax_off" : "axe.scrape"),
            1F,
            (float) ((pitch == 0 ? 0 : ((double) pitch / 10D)) + 0.9D),
            false);
    }
