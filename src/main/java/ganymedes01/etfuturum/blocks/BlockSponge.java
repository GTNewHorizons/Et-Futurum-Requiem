...
public class BlockSponge extends BaseSubtypesBlock {
    ...
    protected void tryAbsorb(World worldIn, int x, int y, int z, boolean wet) {
        if (!ArrayUtils.contains(
            BiomeDictionary.getTypesForBiome(worldIn.getBiomeGenForCoords(x, z)),
            ...
            if (!wet && absorb(worldIn, x, y, z)) {
                ...
                if (ConfigSounds.newBlockSounds) {
                    worldIn.playSoundEffect(
                        ...
                        y + .5D,
                        z + .5D,
                        Reference.MCAssetVer + ":block.sponge.absorb",
                        1,
                        1);
                }
            }
    ...
    public Item getItem(World world, int x, int y, int z) {
        ...
        return Item.getItemFromBlock(
            ConfigWorld.tileReplacementMode == -1 || meta == 1 ? ModBlocks.SPONGE.get() : Blocks.sponge);
    }
