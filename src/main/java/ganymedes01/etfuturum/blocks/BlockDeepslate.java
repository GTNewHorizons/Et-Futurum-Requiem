...
public class BlockDeepslate extends BlockRotatedPillar {
    ...
    @Override
    /**
     * We need to do this in order to make sure deepslate generates, as veins crossing a chunk border may not get
     * replaced fully.
    ...
    public static void doDeepslateRedoCheck(World world, int x, int y, int z) {
        if (!EtFuturumLateWorldGenerator.stopRecording && !world.getChunkFromBlockCoords(x, z).sendUpdates) {
            Map<Long, List<Integer>> map = EtFuturumLateWorldGenerator.deepslateRedoCache
                .computeIfAbsent(world.provider.dimensionId, k -> Maps.newConcurrentMap());
            List<Integer> posSet = map
                .computeIfAbsent(ChunkCoordIntPair.chunkXZ2Int(x >> 4, z >> 4), k -> Lists.newLinkedList());
