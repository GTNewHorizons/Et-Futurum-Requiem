...
public class BlockPointedDripstone extends Block {
    ...
    private void setState(World world, int x, int y, int z) {
        ...
        for (meta = 0; meta < 2; meta++) {
            if (world.getBlockMetadata(x, y + (up ? 1 : -1) + (up ? meta : -meta), z) > 4 != up
                || world.getBlock(x, y + (up ? 1 : -1) + (up ? meta : -meta), z) != this) {
                break;
            }
        }
        if (meta % 5 == 0 && world.getBlock(x, y + (up ? 1 : -1), z) == this
            && world.getBlockMetadata(x, y + (up ? 1 : -1), z) > 4 != up) {
            meta = 4;
        }
        if (meta % 5 == 2 && world.getBlock(x, y + (up ? 1 : -1), z) == this
            && world.getBlockMetadata(x, y + (up ? 1 : -1), z) % 5 == 2
            && world.getBlock(x, y + (up ? -1 : 1), z)
                .isSideSolid(world, x, y + (up ? -1 : 1), z, up ? ForgeDirection.DOWN : ForgeDirection.UP)) {
            meta = 3;
        }
        if (up) meta += states;
    ...
    private boolean canDripstoneStayHere(World world, int x, int y, int z, int offset) {
        return world.isBlockNormalCubeDefault(x, y + offset, z, false)
            || (offset == (world.getBlockMetadata(x, y + offset, z) < states ? 1 : -1)
                && world.getBlock(x, y + offset, z) == this);
    }
