...
public class BlockGeneralModdedRawOre extends BaseSubtypesBlock implements IInitAction {
    ...
    public BlockGeneralModdedRawOre(String... names) {
        ...
        for (int i = 0; i < names.length; i++) {
            ores[i] = "block" + StringUtils.capitalize(
                names[i].replaceFirst("^raw_", "")
                    .replace("_block", ""));
        }
        ...
        setHardness(5);
        setResistance(6);
        setHarvestLevel("pickaxe", 1); // TODO: Config for this since some mods use different harvest levels for these
                                       // materials
    }

    @Override
    public void getSubBlocks(Item item, CreativeTabs tab, List<ItemStack> list) {
        ModItems.MODDED_RAW_ORE.get()
    ...
    public void onLoadAction() {
        ...
        for (int i = 0; i < ores.length; i++) {
            ...
            try {
                ...
                resistances[i] = block.getExplosionResistance(null, world, 0, 0, 0, 0, 0, 0) * 5; // Because the game
                                                                                                  // divides it by 5 for
                                                                                                  // some reason
            } catch (Exception e) {
