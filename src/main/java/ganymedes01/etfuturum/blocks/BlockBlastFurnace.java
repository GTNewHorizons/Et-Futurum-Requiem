...
public class BlockBlastFurnace extends BlockFurnace {
    ...
    public IIcon getIcon(int side, int meta) {
        return side == 1 ? this.blockTop
            : (side == 0 ? this.blockTop
                : (side != meta ? (side == 3 && meta == 0 ? this.blockFront : this.blockIcon) : this.blockFront));
    }
        ...
        if (this.isCooking) {
            if (ConfigSounds.furnaceCrackling && random.nextDouble() < 0.1D) {
                worldIn.playSound(
                    ...
                    y + .5D,
                    z + .5D,
                    Reference.MCAssetVer + ":block.blastfurnace.fire_crackle",
                    1,
