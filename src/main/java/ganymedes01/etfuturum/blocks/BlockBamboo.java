...
public class BlockBamboo extends BaseBlock implements IPlantable, IGrowable {
    ...
    public float getPlayerRelativeBlockHardness(EntityPlayer player, World worldIn, int x, int y, int z) {
        if (player.getCurrentEquippedItem() != null) {
            Set<String> classes = player.getCurrentEquippedItem()
                .getItem()
                .getToolClasses(player.getCurrentEquippedItem());
            if (player.getCurrentEquippedItem()
                .getItem() instanceof ItemSword || classes.contains("sword")) {
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return ModBlocks.BAMBOO_SAPLING.get()
            .canPlaceBlockAt(world, x, y, z) || block == ModBlocks.BAMBOO_SAPLING.get() || block == this;
    }
    ...
    public boolean canPlaceBlockOnSide(World world, int x, int y, int z, int side) {
        Block block = world.getBlock(x, y - 1, z);
        return ModBlocks.BAMBOO_SAPLING.get()
            .canPlaceBlockOnSide(world, x, y, z, side) || block == ModBlocks.BAMBOO_SAPLING.get() || block == this;
    }
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        Block block = world.getBlock(x, y - 1, z);
        return ModBlocks.BAMBOO_SAPLING.get()
            .canBlockStay(world, x, y, z) || block == ModBlocks.BAMBOO_SAPLING.get() || block == this;
    }
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        leaves = new IIcon[] { reg.registerIcon("bamboo_singleleaf"), reg.registerIcon("bamboo_small_leaves"),
            reg.registerIcon("bamboo_large_leaves") };
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side) {
        return side > 1
            || (!(world.getBlock(x, y, z) instanceof BlockBamboo) && super.shouldSideBeRendered(world, x, y, z, side));
    }
    ...
    public static Vec3 getOffset(int x, int z) {
        OFFSET_VEC.xCoord = 0.1875D
            + ((double) (Math.abs(Utils.cantor(x * 2654435761L, z * -2654435761L)) % 41) * 0.015625D);
        OFFSET_VEC.zCoord = 0.1875D
            + ((double) (Math.abs(Utils.cantor(z * 2654435761L, x * -2654435761L)) % 41) * 0.015625D);
        // 0.8125
        return OFFSET_VEC;
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        ...
        setBlockBounds(
            (float) (offsetBox.xCoord - size),
            0,
            (float) (offsetBox.zCoord - size),
            (float) (offsetBox.xCoord + size),
            1,
            (float) (offsetBox.zCoord + size));
    }
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return AxisAlignedBB.getBoundingBox(
            x + offsetBox.xCoord - size,
            y,
            z + offsetBox.zCoord - size,
            x + offsetBox.xCoord + size,
            y + 1,
            z + offsetBox.zCoord + size);
    }
    ...
    protected void growBamboo(World world, int x, int y, int z, Random rand, int height) {
        ...
        if (origin == this) {
            ...
            world.setBlock(
                ...
                z,
                this,
                setLeavesSize(setStage(setStalkSize(0, stalkSize == 1), complete == 1), bambooleaves),
                3);
        }
    }
