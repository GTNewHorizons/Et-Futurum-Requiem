...
public class BlockObserver extends Block implements IBlockObserver {
    ...
    public IIcon getIcon(int side, int meta) {
        ...
        if (side == k) return this.observerFront;
        else if (side == Facing.oppositeSide[k]) return powered ? this.observerBackLit : this.observerBack;
        else {
            int topSide;
            switch (k) {
                case 2:
    ...
    @Override
    public void observedNeighborChange(World world, int observerX, int observerY, int observerZ, Block changedBlock,
        int changedX, int changedY, int changedZ) {
        ...
        if (observedX == changedX && observedY == changedY && observedZ == changedZ) {
            if ((myMeta & 8) == 0
                && !((IObserverWorldExtension) world).etfu$hasScheduledUpdate(observerX, observerY, observerZ, this)) {
    ...
    /**
     * <p>
     * Disable notification of observers about chunk updates on the current thread. Observer notifications add
     * some overhead to chunk updates, so they should be disabled if it's reasonable to assume that no observers will
     * see them (e.g. during world gen).
     * </p>
     *
     * <p>
     * Calling this method will increase the disablification stack by one level. Each call should be followed by a
     * {@link BlockObserver#enableNotifications()} call to restore the stack.
     * </p>
     */
