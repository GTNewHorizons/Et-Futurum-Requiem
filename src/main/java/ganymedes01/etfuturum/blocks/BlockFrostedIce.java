...
public class BlockFrostedIce extends BlockIce {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (block == this || block == Blocks.ice || block == Blocks.packed_ice)
                if (++surroundingBlockCount >= 4) break;
        }

        if (surroundingBlockCount < 4 || rand.nextInt(100) <= 33) {
            int meta = world.getBlockMetadata(x, y, z);
            if (meta < 3) world.setBlockMetadataWithNotify(x, y, z, meta + 1, 2);
            else world.setBlock(x, y, z, Blocks.flowing_water, 0, 3);
        }
    ...
    public void registerBlockIcons(IIconRegister reg) {
        icons = new IIcon[4];
        for (int i = 0; i < icons.length; i++) icons[i] = reg.registerIcon(getTextureName() + "_" + i);
    }
