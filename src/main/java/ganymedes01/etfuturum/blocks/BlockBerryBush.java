...
public class BlockBerryBush extends BlockBush implements IGrowable {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (world.getBlockMetadata(x, y, z) >= 3) return;
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity instanceof EntityLivingBase) {
            ...
            if (!world.isRemote && world.getBlockMetadata(x, y, z) > 0
                && (entity.lastTickPosX != entity.posX || entity.lastTickPosZ != entity.posZ)) {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float subX,
        ...
        int i = world.getBlockMetadata(x, y, z);
        if (i > 1 && (i != 2 || player.getHeldItem() == null
            || player.getHeldItem()
                .getItem() != Items.dye
            || player.getHeldItem()
                .getItemDamage() != 15)) {
            // We check for if the plant is in the first berried stage (meta 2, the plant has berries on meta 2 and 3)
            // and if we're holding bone meal.
            ...
            world.playSound(
                ...
                y,
                z,
                Reference.MCAssetVer + ":block.sweet_berry_bush.pick_berries",
                1.0F,
