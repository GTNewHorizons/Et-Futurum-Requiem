        ...
        while (l < 128) {
            ...
            while (true) {
                if (l1 < l / 16) {
                    ...
                    if ((blockBelow != Blocks.grass && blockBelow != Blocks.stone
                        && blockBelow != Blocks.dirt
                        && blockBelow != ModBlocks.MOSS_BLOCK.get()) && !blockAtPos.isNormalCube()) {
                        ++l1;
                        continue;
                    } else {
                        ...
                        if (blockAtPos == Blocks.air) {
                            ...
                            int randomOption = rand.nextInt(
                                AZALEA_BUSH_WEIGHT + FLOWERING_AZALEA_WEIGHT
                                    + DOUBLE_TALL_GRASS_WEIGHT
                                    + MOSS_CARPET_WEIGHT
                                    + TALL_GRASS_WEIGHT
                                    + AIR_WEIGHT);

                            if (randomOption < AZALEA_BUSH_WEIGHT) {
                                if (ModBlocks.AZALEA.get()
                                    .canBlockStay(world, i1, j1 + 1, k1)) {
