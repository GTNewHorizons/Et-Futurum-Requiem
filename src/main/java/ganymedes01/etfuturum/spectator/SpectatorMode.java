...
public class SpectatorMode {
    ...
    // TODO: DO NOT MAKE THIS A LAMBDA! INTELLIJ SUGGESTS IT BUT FOR SOME REASON IT CAUSES AN INITIALIZER EXCEPTION
    // SPECIFICALLY IN THE LIVE GAME AND NOT IN DEV!
    ...
    public static boolean isSpectator(EntityPlayer player) {
        if (player == null || player instanceof FakePlayer || player.worldObj == null) return false;
        if (player.worldObj.isRemote) {
            if (player == FMLClientHandler.instance()
                .getClient().thePlayer
                && FMLClientHandler.instance()
                    .getClient().playerController != null)
                return FMLClientHandler.instance()
                    .getClient().playerController.currentGameType == SPECTATOR_GAMETYPE;
            return false;
        }
        return player instanceof EntityPlayerMP
            && ((EntityPlayerMP) player).theItemInWorldManager.getGameType() == SPECTATOR_GAMETYPE;
    }
        ...
        if (isSpectator(event.entityPlayer)) {
            if (event.action != PlayerInteractEvent.Action.RIGHT_CLICK_BLOCK) {
                ...
                if (!canSpectatorSelect(te)
                    || SPECTATOR_INTERACT_BLACKLIST.contains(event.world.getBlock(event.x, event.y, event.z))) {
                    event.setCanceled(true);
                }
            }
        }
    ...
    private void openInv(EntityPlayer player, Entity target) {
        ...
        // So we do things like make the game think the player is sneaking so it opens the inventory without having the
        // spectator sneak.
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.START) {
            /*
             * We need to create this so the client and server have separate maps, so the code below runs fully on the
             * client and server and does not desync
            ...
            if (isSpec) {
                // Manage these states while in spectator since we want these states to be forced to specific values in
                // spectator.
    ...
    protected void followEntity(EntityPlayer player, Entity entity) {
        ...
        player.setLocationAndAngles(
            entity.posX,
            (entity.posY - player.yOffset) + entity.getEyeHeight(),
            entity.posZ,
            yaw,
            pitch);
        ...
        if (isSpectator(event.entityPlayer)) {
            if (event.entityPlayer.worldObj.isRemote) {
                EntityClientPlayerMP player = FMLClientHandler.instance()
                    .getClient().thePlayer;
                if (event.entityPlayer == player && FMLClientHandler.instance()
                    .getClient().playerController != null) {
                    FMLClientHandler.instance()
                        .getClient().playerController.stepSoundTickCounter = -5;
                    FMLClientHandler.instance()
                        .getClient().playerController.isHittingBlock = false;
                }
            }
            event.newSpeed = 0;
        }
