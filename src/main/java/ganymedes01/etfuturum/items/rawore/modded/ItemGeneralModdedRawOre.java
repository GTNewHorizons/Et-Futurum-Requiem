...
/**
 * Modded raw ore class that doesn't add the raw ores to the Creative menu if there would be no ore for them to base off
 * of.
 */
    ...
    public void getSubItems(Item item, CreativeTabs tabs, List<ItemStack> list) {
        for (int i = 0; i < ores.length; i++) {
            ...
            if (Utils.listGeneralModdedRawOre(ores[i])
                || Utils.listGeneralModdedRawOre(ores[i].replace("Mythril", "Mithril"))) {
                list.add(stack);
            }
        }
    }
