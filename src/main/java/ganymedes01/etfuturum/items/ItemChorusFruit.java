...
public class ItemChorusFruit extends BaseFood {
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        for (int i = 0; i < 16; i++) {
            double xx = player.posX + (player.getRNG()
                .nextDouble() - 0.5D) * 64.0D;
            double yy = player.posY + (player.getRNG()
                .nextInt(64) - 32);
            double zz = player.posZ + (player.getRNG()
                .nextDouble() - 0.5D) * 64.0D;
    ...
    private static boolean teleportTo(EntityLivingBase entity, double xx, double yy, double zz) {
        ...
        if (entity.worldObj.blockExists(i, j, k)) {
            ...
            if (flag1) {
                ...
                if (entity.worldObj.getCollidingBoundingBoxes(entity, entity.boundingBox)
                    .isEmpty() && !entity.worldObj.isAnyLiquid(entity.boundingBox)) flag = true;
            }
        }
        ...
        for (int l = 0; l < short1; l++) {
            ...
            float f = (entity.getRNG()
                .nextFloat() - 0.5F) * 0.2F;
            float f1 = (entity.getRNG()
                .nextFloat() - 0.5F) * 0.2F;
            float f2 = (entity.getRNG()
                .nextFloat() - 0.5F) * 0.2F;
            double d7 = d3 + (entity.posX - d3) * d6
                + (entity.getRNG()
                    .nextDouble() - 0.5D) * entity.width * 2.0D;
            double d8 = d4 + (entity.posY - d4) * d6
                + entity.getRNG()
                    .nextDouble() * entity.height;
