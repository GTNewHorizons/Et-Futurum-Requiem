...
public class ItemLingeringPotion extends ItemPotion {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (!world.isRemote) world.spawnEntityInWorld(new EntityLingeringPotion(world, player, stack));
    ...
    public List<PotionEffect> getEffects(int meta) {
        ...
        if (effects2 != null && !effects2.isEmpty()) for (PotionEffect effect : effects2) {
            PotionEffect e;

            if (Potion.potionTypes[effect.getPotionID()].isInstant()) e = new PotionEffect(effect);
            else {
                e = new PotionEffect(
                    effect.getPotionID(),
    ...
    public List<PotionEffect> getEffects(ItemStack stack) {
        if (stack.hasTagCompound() && stack.getTagCompound()
            ...
            for (int i = 0; i < nbttaglist.tagCount(); i++) {
                ...
                if (potioneffect != null) list.add(potioneffect);
            }
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        if (stack.getItemDamage() == 0) return StatCollector.translateToLocal("item.emptyPotion.name")
            .trim();
        String s = StatCollector.translateToLocal("potion.prefix.lingering")
            .trim() + " ";
        ...
        if (list != null && !list.isEmpty()) {
            s1 = list.get(0)
                .getEffectName();
            s1 = s1 + ".postfix";
            return s + StatCollector.translateToLocal(s1)
                .trim();
        }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> list, boolean isComplex) {
        ...
        if (effects == null || effects.isEmpty()) {
        ...
        } else for (PotionEffect potioneffect : effects) {
            ...
            if (potioneffect.getAmplifier() > 0) s1 = s1 + " "
                + StatCollector.translateToLocal("potion.potency." + potioneffect.getAmplifier())
                    .trim();
            if (potioneffect.getDuration() > 20) s1 = s1 + " (" + Potion.getDurationString(potioneffect) + ")";
            ...
            for (Entry<String, AttributeModifier> entry1 : attributes.entries()) {
                ...
                if (d0 > 0.0D) list.add(
                    EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted(
                        ...
                        new Object[] { ItemStack.field_111284_a/* DECIMALFORMAT */.format(d1),
    ...
    public void getSubItems(Item item, CreativeTabs tab, List<ItemStack> list) {
        ...
        for (ItemStack potion : potions) if (!isSplash(potion.getItemDamage())) list.add(potion);
    }
