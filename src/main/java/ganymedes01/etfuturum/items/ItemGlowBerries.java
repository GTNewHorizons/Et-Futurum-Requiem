...
public class ItemGlowBerries extends BaseFood {
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side,
        float subValueX, float subValueY, float subValueZ) {
        ...
        if (!player.canPlayerEdit(x, y, z, side, itemStack)) {
            ...
            if (placeBlockAt(itemStack, player, world, x, y, z, side, subValueX, subValueY, subValueZ)) {
                world.playSoundEffect(
                    ...
                    y + 0.5F,
                    z + 0.5F,
                    ModBlocks.CAVE_VINE.get().stepSound.func_150496_b()/* getPlaceSound */,
                    (ModBlocks.CAVE_VINE.get().stepSound.getVolume() + 1.0F) / 2.0F,
                    ModBlocks.CAVE_VINE.get().stepSound.getPitch() * 0.8F);
                --itemStack.stackSize;
            }
