...
public class DebugTestItem extends BaseItem {
    ...
    private static final String langkey = "debug_test_item"; // Ignores IntellIJ's suggestion of giving it a lang key;
                                                             // The debug item doesn't use lang keys.

    public DebugTestItem() {
        setUnlocalizedName(langkey);// Placeholder lang key just to make it not null as a safeguard in case a mod grabs
                                    // the unlocalizedName field directly and does null-unsafe operations on it.
        setTextureName("ruby");
        setHasSubtypes(true);
    }
    ...
    public void onLeftClick(PlayerInteractEvent event) {
        if (event.entityPlayer != null && event.action == PlayerInteractEvent.Action.LEFT_CLICK_BLOCK
            && canUse(event.entityPlayer)) {
            if (event.entityPlayer.getHeldItem() != null && event.entityPlayer.getHeldItem()
                .getItem() == ModItems.DEBUGGING_TOOL.get()) {
                event.entityPlayer.getCurrentEquippedItem()
                    .setItemDamage(
                        (event.entityPlayer.getCurrentEquippedItem()
                            .getItemDamage() + 1) % TestTypes.values().length);
    ...
    private boolean canUse(EntityPlayer player) {
        if (player.worldObj.isRemote) {
            return FMLClientHandler.instance()
                .getClientPlayerEntity().capabilities.isCreativeMode;
    ...
    public String getItemStackDisplayName(ItemStack p_77653_1_) {
        return "Debugging Item"
            + TestTypes.values()[p_77653_1_.getItemDamage() % TestTypes.values().length].getSuffix();
    }
    ...
    private enum TestTypes {
        ...
        FOSSIL("Fossil", true) {
            ...
            @Override
            protected boolean runAction(ItemStack stack, EntityPlayer player, World world, int x, int y, int z,
        ...
        GEODE("Geode", true) {
            ...
            private final Supplier<WorldGenGeode> geodeSupplier = () -> new WorldGenGeode(
                ConfigWorld.amethystOuterBlock,
                ConfigWorld.amethystMiddleBlock) {
        ...
        protected boolean runAction(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
            float hitX, float hitY, float hitZ) {
            return false;
        }
