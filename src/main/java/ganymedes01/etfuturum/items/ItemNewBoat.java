...
public class ItemNewBoat extends BaseItem {
    ...
    public ItemNewBoat(EntityNewBoat.Type type, boolean isChest) {
        ...
        BOAT_INFO.put(
            "minecraft:" + type.getName()
                .toLowerCase() + (isChest ? "_chest" : ""),
            new BoatInfo(new ItemStack(this), () -> Item.getItemFromBlock(Blocks.planks), type.ordinal(), false));
        setMaxStackSize(1);
    }

    public ItemNewBoat(String domain, String name, Supplier<Item> plank, int plankMeta, boolean isChest,
        boolean isRaft) {
        ...
        this.isChest = isChest;
        BOAT_INFO.put(
            domain + ":" + name + (isChest ? "_chest" : ""),
            new BoatInfo(new ItemStack(this), plank, plankMeta, isRaft));
        setMaxStackSize(1);
    }
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer player) {
        ...
        if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            boolean isWater = worldIn.getBlock(i, j, k)
                .getMaterial() == Material.water;
            entityboat.setPositionAndRotation(
                movingobjectposition.hitVec.xCoord,
                movingobjectposition.hitVec.yCoord + (isWater ? -0.12 : 0),
            ...
            if (!worldIn.getCollidingBoundingBoxes(entityboat, entityboat.boundingBox.expand(-0.1D, -0.1D, -0.1D))
                .isEmpty()) {
                return itemStackIn;
            }
