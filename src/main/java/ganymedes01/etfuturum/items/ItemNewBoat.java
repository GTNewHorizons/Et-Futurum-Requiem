...
public class ItemNewBoat extends BaseItem {
	...
	public ItemNewBoat(String domain, String name, Supplier<Item> plank, int plankMeta, boolean isChest, boolean isRaft) {
		...
		this.domain = domain;
		this.isChest = isChest;
		setUnlocalizedName("etfuturum." + (domain.equals("witchery") ? "witchery_" : "") + (domain.equals("biomesoplenty") ? "bop_" : "") + name + (isChest ? "_chest" : "") + (isRaft ? "_raft" : "_boat"));
	...
	public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer player) {
		...
		double d1 = player.prevPosY + (player.posY - player.prevPosY) * (double) f + (double)(worldIn.isRemote ? player.getEyeHeight() - player.getDefaultEyeHeight() : player.getEyeHeight());
