        ...
        if (p_77648_7_ == 0) {
            ...
            .isSolid()) {
                ...
                if (!player.canPlayerEdit(x, y, z, p_77648_7_, itemStack)) {
                    ...
                    if (ConfigSounds.fixSilentPlacing) world.playSoundEffect(
                        (float) x + 0.5F,
                        (float) y + 0.5F,
                        (float) z + 0.5F,
                    ...
                    if (tileentitysign != null) {
                        if (player instanceof EntityPlayerMP) {
                            ...
                            // This is needed because the game crashes whenever we open the sign normally for some
                            // reason, so we have to use a packet.
                            // I'd like to get rid of this packet eventually because it feels wrong
                            EtFuturum.networkWrapper.sendTo(
                                new WoodSignOpenMessage(tileentitysign, Block.getIdFromBlock(block)),
