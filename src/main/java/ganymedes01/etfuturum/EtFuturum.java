...
@Mod(modid = MOD_ID, name = MOD_NAME, version = Tags.VERSION, dependencies = Reference.DEPENDENCIES
...
public class EtFuturum {
        ...
        public Item getTabIconItem() {
            return ModItems.RAW_ORE.isEnabled() ? ModItems.RAW_ORE.get()
                : ModItems.NETHERITE_SCRAP.isEnabled() ? ModItems.NETHERITE_SCRAP.get()
                    : ModItems.PRISMARINE_SHARD.isEnabled() ? ModItems.PRISMARINE_SHARD.get() : Items.magma_cream;
        }
    ...
    public static CreativeTabs creativeTabBlocks = new CreativeTabs(MOD_ID + ".blocks") {
        ...
        public void displayAllReleventItems(List<ItemStack> list) {
            ...
            // This allows them to be in the correct place instead of always at the bottom of the block ID list, since
            // item IDs are always above block IDs
            for (ModItems sign : ModItems.OLD_SIGN_ITEMS) {
                for (ItemStack stack : list) {
                    if (Item.getIdFromItem(stack.getItem())
                        > Block.getIdFromBlock(((ItemWoodSign) sign.get()).getSignBlock())) {
    ...
    public void onConstruction(FMLConstructionEvent event) {
        if (Reference.SNAPSHOT_BUILD && !Reference.DEV_ENVIRONMENT) {
            Logger.info(
                MOD_ID + " is in snapshot mode. Disabling update checker... Other features may also be different.");
        }
    ...
    public void preInit(FMLPreInitializationEvent event) {
        try {
            ...
            if (!((HashMap<String, ChestGenHooks>) chestInfo.get(null)).containsKey(NETHER_FORTRESS)) {
                fortressWeightedField = Class
                    .forName("net.minecraft.world.gen.structure.StructureNetherBridgePieces$Piece")
                    .getDeclaredField("field_111019_a");
                fortressWeightedField.setAccessible(true);
                ((HashMap<String, ChestGenHooks>) chestInfo.get(null)).put(
                    NETHER_FORTRESS,
                    new ChestGenHooks(
                        NETHER_FORTRESS,
                        (WeightedRandomChestContent[]) fortressWeightedField.get(null),
                        2,
                        5));
            }
        } catch (Exception e) {
        ...
        networkWrapper
            .registerMessage(ArmourStandInteractHandler.class, ArmourStandInteractMessage.class, 0, Side.SERVER);
        networkWrapper
            .registerMessage(BlackHeartParticlesHandler.class, BlackHeartParticlesMessage.class, 1, Side.CLIENT);
        ...
        networkWrapper
            .registerMessage(ChestBoatOpenInventoryHandler.class, ChestBoatOpenInventoryMessage.class, 5, Side.SERVER);
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        if (ConfigBlocksItems.enableOtherside) {
            ChestGenHooks.addItem(
                ...
                new WeightedRandomChestContent(ModItems.OTHERSIDE_RECORD.get(), 0, 1, 1, 1));
            ChestGenHooks.addItem(
        ...
        if (ConfigBlocksItems.enablePigstep) {
            ChestGenHooks
                .addItem(NETHER_FORTRESS, new WeightedRandomChestContent(ModItems.PIGSTEP_RECORD.get(), 0, 1, 1, 5));
            ...
            if (fortressWeightedField != null) {
                try {
                    ...
                    fortressContentList = (ArrayList<WeightedRandomChestContent>) contents
                        .get(ChestGenHooks.getInfo("netherFortress"));
                    if (!fortressContentList.isEmpty()) {
                        WeightedRandomChestContent[] fortressChest = new WeightedRandomChestContent[fortressContentList
                            .size()];
    ...
    public void onLoadComplete(FMLLoadCompleteEvent e) {
        ...
        for (Block block : (Iterable<Block>) Block.blockRegistry) {
            if (ConfigFunctions.enableHoeMining) {
                if (block instanceof BlockLeaves || block instanceof BlockHay
                    || block instanceof BlockSponge
                    || block instanceof BlockNetherWart
                    || block instanceof BlockSculk
                    || block instanceof BlockSculkCatalyst) {
    ...
    private void setupMultiBlockSoundRegistry() {
        ...
        MultiBlockSoundRegistry
            .addBasic(ExternalContent.Blocks.TCON_MULTIBRICK_FANCY.get(), ModSounds.soundNetherrack, 2);
        MultiBlockSoundRegistry
            .addBasic(ExternalContent.Blocks.TCON_MULTIBRICK_FANCY.get(), ModSounds.soundBoneBlock, 9);
        ...
        MultiBlockSoundRegistry
            .addBasic(ModBlocks.AMETHYST_CLUSTER_1.get(), ModSounds.soundAmethystBudSmall, 0, 1, 2, 3, 4, 5, 6);
        MultiBlockSoundRegistry
            .addBasic(ModBlocks.AMETHYST_CLUSTER_2.get(), ModSounds.soundAmethystBudLrg, 0, 1, 2, 3, 4, 5, 6);
        ...
        if (ModsList.IRON_CHEST.isLoaded() && ModsList.IRON_CHEST.isVersionNewerOrEqual("6.0.78")) { // Version
                                                                                                     // netherite chests
                                                                                                     // were added in
        ...
        if (ExternalContent.Blocks.TCON_METAL.get() != null) {
            {
                BasicMultiBlockSound mbs = new BasicMultiBlockSound() {
                    ...
                    @Override
                    public float getPitch(World world, int x, int y, int z, float pitch,
     ...
     * We need to use this event since unregistering specifically an ItemBlock from a block makes Forge mistakenly think
     * a save is corrupted.
     * I add the EFR name check at the beginning just as a safety precaution.
     * <p>
     * Forge does some bad checks on if the item is an ItemBlock before letting you run ignoreItemBlock, leading to
     * erroneous errors.
     * It doesn't look much different than what I do above but their code rarely spits out "Cannot skip an ItemBlock
     * that doesn't have a Block"
     * Which makes no sense since if the block != null then we're skipping an ItemBlock that DOES have a block, if my
     * block check != null then what else would it be?
     ...
     * I wrote this bad code to get around Forge's bad code, only to reveal EVEN MORE bad code in Forge I have to write
     * even worse code to avoid.
     */
    ...
    public SoundType getCustomStepSound(Block block, String namespace) {
        if (block.stepSound == Block.soundTypePiston || block.stepSound == Block.soundTypeStone) {
            if (namespace.contains("nether") && namespace.contains("brick")) {
                ...
                || (namespace.contains("nether") && (block instanceof BlockOre || namespace.contains("ore")))) {
                    ...
                    || (namespace.contains("nether") && namespace.contains("wart"))) {
    ...
    private static void getSounds(ADConfig config) {
        ...
        // Automatically register block sounds for AssetDirector, but only if they contain the MC version (which means
        ...
        for (ModSounds.CustomSound sound : ModSounds.getSounds()) {
            if (sound.getStepResourcePath()
                .startsWith(Reference.MCAssetVer)) { // Step sound
                config.addSoundEvent(
                    ver,
                    sound.getStepResourcePath()
                        .substring(Reference.MCAssetVer.length() + 1),
                    "neutral");
            }
            if (sound.func_150496_b/* getPlaceSound */()
                .startsWith(Reference.MCAssetVer)) { // Place sound
                config.addSoundEvent(
                    ver,
                    sound.func_150496_b/* getPlaceSound */()
                        .substring(Reference.MCAssetVer.length() + 1),
                    "block");
            }
            if (sound.getBreakSound()
                .startsWith(Reference.MCAssetVer)) { // Break sound
                config.addSoundEvent(
                    ver,
                    sound.getBreakSound()
                        .substring(Reference.MCAssetVer.length() + 1),
