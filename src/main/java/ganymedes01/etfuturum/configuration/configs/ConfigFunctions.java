...
public class ConfigFunctions extends ConfigBase {
    ...
    public static final String[] shulkerDefaultBans = new String[] { "adventurebackpack:adventureBackpack",
        "arsmagica2:essenceBag", "arsmagica2:runeBag", "betterstorage:backpack", "betterstorage:cardboardBox",
        "betterstorage:thaumcraftBackpack", "BiblioCraft:item.BigBook", "Botania:baubleBox", "Botania:flowerBag",
        "cardboardboxes:cbCardboardBox", "dendrology:fullDrawers1", "dendrology:fullDrawers2",
        "dendrology:fullDrawers4", "dendrology:halfDrawers2", "dendrology:halfDrawers4", "DQMIIINext:ItemMahounoTutu11",
        "DQMIIINext:ItemOokinaFukuro", "DQMIIINext:ItemOokinaFukuroB", "DQMIIINext:ItemOokinaFukuroG",
        "DQMIIINext:ItemOokinaFukuroR", "DQMIIINext:ItemOokinaFukuroY", "ExtraSimple:bedrockium",
        "ExtraSimple:bedrockiumBlock", "ExtraSimple:goldenBag", "ExtraSimple:lasso", "ExtraUtilities:bedrockiumIngot",
    ...
    public static boolean enableAttackedAtYawFix = true; // Servers should always send the packet, it's up to the client
                                                         // to disable handling of this feature
    ...
    protected void syncConfigOptions() {
        if (EtFuturumEarlyMixins.side == MixinEnvironment.Side.CLIENT) {
            enableAttackedAtYawFix = getBoolean(
                "enableAttackedAtYawFix",
                catChanges,
                true,
                "Adds a packet to send the attackedAtYaw field value to the client, which allows the screen to tilt based on where damage came from, and either left or right for direction-less sources like drowning or burning, instead of tilting to the left no matter what.");
        }

        // changes
        enableExtraBurnableBlocks = getBoolean(
            "enableExtraBurnableBlocks",
            catChanges,
            true,
            "Fences, gates and dead bushes burn");
        enableUpdatedHarvestLevels = getBoolean(
            "enableUpdatedHarvestLevels",
            catChanges,
            true,
            "Packed Ice, ladders and melons have preferred tools");
        enableSilkTouchingMushrooms = getBoolean(
            "enableSilkMushroom",
            catChanges,
            true,
            "Mushroom blocks can be silk-touched");
        ...
        enableStoneBrickRecipes = getBoolean(
            "enableStoneBrickRecipes",
            catChanges,
            true,
            "Makes mossy, cracked and chiseled stone brick craftable");
        enableFloatingTrapDoors = getBoolean("enableFloatingTrapDoors", catChanges, true, "");
        enableHayBaleFalls = getBoolean(
            "enableHayBaleFalls",
            catChanges,
            true,
            "If true, fall damage on a hay bale will be reduced");
        enableHoeMining = getBoolean(
            "enableHoeMining",
            catChanges,
            true,
            "Allows blocks like hay bales, leaves etc to mine faster with hoes");
        hayBaleReducePercent = getInt(
            "hayBaleReducePercent",
            catChanges,
            20,
            0,
            99,
            "If enableHayBaleFalls is true, what percent should we keep for the fall damage?");
        enableDoorRecipeBuffs = getBoolean(
            "enableDoorRecipeBuffs",
            catChanges,
            true,
            "Backports recipe buffs to doors (from 1 to 3)");
        mobSpawnerEgging = getBoolean(
            "mobSpawnerEgging",
            catChanges,
            true,
            "Click a mob spawner with a vanilla or EFR egg, and it'll change the mob inside. Doesn't support modded eggs.");
        fireworkRecipeFixes = getBoolean(
            "fireworkRecipeFixes",
            catChanges,
            true,
            "Fixes fireworks not being able to have an extra duration without a star, and they'll output 3 instead of 1. Note: This currently does NOT update NEI, it'll still show the old recipe outputs.");

        // settings
        enableNetheriteFlammable = getBoolean(
            "enableNetheriteFlammable",
            catSettings,
            false,
            "Set to true to disable the fireproof item entity Netherite/ancient debris etc uses");
        enableRecipeForPrismarine = getBoolean(
            "enablePrismarineRecipes",
            catSettings,
            true,
            "Recipe for prismarine if you want it without the temples, or want it craftable alongside temples.");
        enableAutoAddSmoker = getBoolean(
            "enableAutoAddSmoker",
            catSettings,
            true,
            "Seeks all available edible foods from the furnace and adds them to the smoker, if it's off it will only smelt things specified from CraftTweaker.");
        enableAutoAddBlastFurnace = getBoolean(
            "enableAutoAddBlastFurnace",
            catSettings,
            true,
            "Seeks all available smeltable ores, metals, etc (using OreDict tags like \"ore\", \"cluster\", \"ingot\", etc) from the furnace and adds them to the Blast Furnace, if it's off it will only smelt things specified from CraftTweaker.");
        enableAutoAddComposter = getBoolean(
            "enableAutoAddComposter",
            catSettings,
            true,
            "Seeks compostable items like plants and leaves, and automatically adds them to the composter. If this is off it will only compost things specified from CraftTweaker and nothing by default.");
        enableRecipeForTotem = getBoolean(
            "enableRecipeForTotem",
            catSettings,
            false,
            "Recipe for totems since there's no other way to get them currently.");
        shulkerBansString = getStringList(
            "shulkerBans",
            catSettings,
            shulkerDefaultBans,
        ...
        totemHealPercent = getInt(
            "totemHealPercent",
            catSettings,
            5,
            5,
            100,
            "Percentage of max health for totem to set you at if you die with it. (5% is 0.05, 20 * 0.05 = 1, 1 health is one half-heart)");
        registerRawItemAsOre = getBoolean(
            "registerRawItemAsOre",
            catSettings,
            true,
        ...
        extraDropRawOres = getStringList(
            "extraDropRawOres",
            catSettings,
            new String[] { "oreCopper", "oreTin" },
            "OreDictionary values for ore blocks that should drop extra items (2-3) instead of the usual one, before fortune.");
        elytraDataWatcherFlag = (byte) getInt(
        ...
        useStoneHardnessForDeepslate = getBoolean(
            "useStoneHardnessForDeepslate",
            catSettings,
            false,
            "Whether deepslate blocks should have the same hardness as their stone counterparts. This allows the asthetics of deepslate without the added hardness.");

        // client
        enableTransparentAmour = getBoolean("enableTransparentAmour", catClient, true, "Allow non-opaque armour");
        enableBowRendering = getBoolean(
            "enableBowRendering",
            catClient,
            true,
            "Bows render pulling animation in inventory");
        enableFancySkulls = getBoolean("enableFancySkulls", catClient, true, "Skulls render 3D in inventory");
        enablePlayerSkinOverlay = getBoolean(
            "enablePlayerSkinOverlay",
            catClient,
            false,
            "Allows use of 1.8 skin format, and Alex skins. Also includes some fixes from SkinPort. (Per SkinPort author's permission) Disable if skin is displaying oddly. Not compatible with OptiFine, use FastCraft instead.");
        enableExtraF3HTooltips = getBoolean(
            "enableExtraF3HTooltips",
            catClient,
            true,
            "Enables NBT tag count and item namespace label on F3 + H debug item labels");
        shulkerBoxTooltipLines = getInt(
            "shulkerBoxTooltipLines",
            catClient,
            5,
            0,
            Byte.MAX_VALUE,
            "The maximum amount of items a Shulker box can display on its tooltip. When the box has more stacks inside than this number, the rest of the stacks are displayed as \"And x more...\". Set to 0 to disable Shulker Box tooltips.");
        enableGamemodeSwitcher = getBoolean(
            "enableGamemodeSwitcher",
            catClient,
            true,
            "Enable the new F3+F4 gamemode switcher from 1.16+");
        enableNewF3Behavior = getBoolean(
            "enableNewF3Behavior",
            catClient,
            true,
            "Make F3 only show/hide info on release, and not if another key is pressed");
        enableNewTextures = getBoolean(
            "enableNewTextures",
            catClient,
            true,
            "Replace tall grass and sponge textures with modern version");
        enableLangReplacements = getBoolean(
            "enableLangReplacements",
            catClient,
            true,
            "Replaces some lang keys with a more modern version, such as calling some old wood items \"oak\", calling beds \"Red Bed\", and so on. Full list of replaced keys can be seen in the mod jar at resources/resourcepacks/vanilla_overrides/assets/minecraft/lang");
        inventoryBedModels = getBoolean(
            "inventoryBedModels",
            catClient,
            true,
            "Render beds with a 3D inventory model instead of a 2D sprite.");
    ...
    protected void initValues() {
        ...
        if (EtFuturumEarlyMixins.side == MixinEnvironment.Side.CLIENT && ConfigFunctions.enablePlayerSkinOverlay) {
            if (ModsList.EARS.isLoaded() || ModsList.FOAMFIX.isLoaded()
                || ModsList.SKINPORT.isLoaded()
                || ModsList.MOREPLAYERMODELS.isLoaded()
                || ModsList.DRAGON_BLOCK_C.isLoaded()
                ...
                Logger
                    .warn("Either Ears, FoamFix, MorePlayerModels, SkinPort, Dragon Block C or OptiFine is installed.");
