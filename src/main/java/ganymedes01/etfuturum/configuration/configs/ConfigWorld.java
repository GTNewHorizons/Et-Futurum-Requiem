...
public class ConfigWorld extends ConfigBase {
    ...
    protected void syncConfigOptions() {
        ...
        enableAirDebris = getBoolean(
            "enableAirDebris",
            catGeneration,
            false,
            "Can ancient debris generate next to air?");
        maxStonesPerCluster = getInt(
            "maxStonesPerCluster",
            catGeneration,
            32,
            0,
            64,
            "Max vein size for Granite/Andesite/Diorite blocks in a cluster");
        smallDebrisMax = getInt(
            "smallDebrisMax",
            catGeneration,
            2,
            0,
            64,
            "The max vein size for the first, typically smaller debris veins which generate from Y 8 to 119");
        debrisMax = getInt(
            "debrisMax",
            catGeneration,
            3,
            0,
            64,
            "The max vein size for the second, typically bigger debris veins, which generate from Y 8 to 22");
        maxNetherGoldPerCluster = getInt(
            "maxNetherGoldPerCluster",
            catGeneration,
            10,
            0,
            64,
            "Max vein size for nether gold ore blocks in a cluster");
        maxMagmaPerCluster = getInt(
            "maxMagmaPerCluster",
            catGeneration,
            33,
            0,
            64,
            "Max vein size for magma blocks in a cluster");
        maxCopperPerCluster = getInt(
            "copperClusterSize",
            catGeneration,
            8,
            0,
            64,
            "Max vein size for copper ore blocks in a cluster");
        maxDeepslatePerCluster = getInt(
            "deepslateClusterSize",
            catGeneration,
            64,
            0,
            128,
            "If deepslateGenerationMode is set to 1, this value is used to determine how big the clusters are. Otherwise this value is unused.");
        deepslateMaxY = getInt(
            "deepslateMaxY",
            catGeneration,
            22,
            0,
            255,
            "How high up deepslate and tuff goes. If deepslateGenerationMode is 0, all stone up to this y level (with a scattering effect a few blocks before then) are replaced with deepslate. If it's 1, the patches can generate to that Y level.");
        deepslateReplacesStones = getBoolean(
            "deepslateReplacesStones",
            catGeneration,
            true,
            "Whether or not Deepslate will overwrite granite, diorite, andesite (Only works when deepslate generation mode is set to 0)");
        deepslateReplacesDirt = getBoolean(
            "deepslateReplacesDirt",
            catGeneration,
            true,
            "Whether or not Deepslate will overwrite dirt (Only works when deepslate generation mode is set to 0)");
        deepslateGenerationMode = getInt(
            "deepslateGenerationMode",
            catGeneration,
            0,
            -1,
            1,
            "If 0, deepslate replaces all stone below the specified value, with a shattering effect near the top similar to bedrock. If 1, it generates in clusters using the deepslate cluster settings. -1 disables Et Futurum deepslate generation entirely.");
        enableOceanMonuments = getBoolean(
            "enableOceanMonuments",
            catGeneration,
            true,
            "Note: Ocean monuments currently do not have guardians");
        ...
        amethystOuterBlockID = getString(
            "amethystOuterBlockID",
            catGeneration,
        ...
        amethystMiddleBlockID = getString(
            "amethystMiddleBlockID",
            catGeneration,
            "etfuturum:calcite",
        ...
        fossilDimensionBlacklistAsWhitelist = getBoolean(
            "fossilDimensionBlacklistAsWhitelist",
            catGeneration,
            false,
            "Treat the fossil dimension blacklist as a whitelist instead, so fossils will ONLY generate in those dimensions, instead of excluding those dimensions from generation.");
        ...
        deepslateLayerDimensionBlacklistAsWhitelist = getBoolean(
            "deepslateLayerDimensionBlacklistAsWhitelist",
            catGeneration,
            false,
            "Treat the deepslate layer dimension blacklist as a whitelist instead, so it will ONLY generate in those dimensions, instead of excluding those dimensions from generation.");
        Property replaceAllStoneWithDeepslateDimensionWhitelistProp = get(
            catGeneration,
            "replaceAllStoneWithDeepslateDimensionWhitelist",
            new int[] {});
        ...
        replaceAllStoneWithDeepslateDimensionWhitelist = replaceAllStoneWithDeepslateDimensionWhitelistProp
            .getIntList();
        maxTuffPerCluster = getInt(
            "tuffClusterSize",
            catGeneration,
            32,
            0,
            64,
            "Max vein size for tuff blocks in a cluster");
        enableExtraMesaGold = getBoolean(
            "enableExtraMesaGold",
            catGeneration,
            true,
            "Generate 20 more veins of gold ore from Y 32 to Y 80 in any Mesa biome.");
        enableMesaMineshaft = getBoolean(
            "enableMesaMineshaft",
            catGeneration,
            true,
            "Generates extra mineshafts in mesa biomes up to y80. If fences are enabled, dark oak wood is used.");
        enableCoarseDirtReplacement = getBoolean(
            "enableCoarseDirtReplacement",
            catGeneration,
            true,
            "Replaces coarse dirt in biomes it (dirt:1) generates in such as shattered savannas or mesa plateaus.");
        enableAmethystGeodes = getBoolean("enableAmethystGeodes", catGeneration, true, "");
        buddingAmethystMode = getInt(
            "buddingAmethystMode",
            catGeneration,
            0,
            0,
        ...
        amethystDimensionBlacklistAsWhitelist = getBoolean(
            "amethystDimensionBlacklistAsWhitelist",
            catGeneration,
            false,
            "Treat the amethyst dimension blacklist as a whitelist instead, so geodes will ONLY generate in those dimensions, instead of excluding those dimensions from generation.");
        amethystRarity = getInt(
            "amethystRarity",
            catGeneration,
            53,
            1,
            Byte.MAX_VALUE,
            "How rare should amethyst geodes be? 1/x chance per chunk, 1 means a geode attempts to appear every chunk");
        amethystMaxY = getInt(
            "amethystMaxY",
            catGeneration,
            46,
            6,
            245,
            "Max Y level amethyst geodes should attempt to generate at");
        cherryTreeRarity = getInt(
            "cherryTreeRarity",
            catGeneration,
            72,
            0,
            Byte.MAX_VALUE,
            "How rare should cherry trees be? 1/x chance per chunk, 1 means a tree attempts to appear every chunk. 0 = no cherry trees. They will spawn in mountain-type biomes.");
        bambooWorldgen = getBoolean(
            "bambooWorldgen",
            catGeneration,
            true,
            "Whether bamboo should naturally spawn in the overworld. Turning this off allows you to use bamboo based blocks without bamboo world gen for mod compatability.");

        crimsonForestID = getInt(
            "crimsonForestID",
            catBiomes,
            200,
            -1,
            65536,
            "Set to -1 to disable the generation of Crimson Forests. To use an ID above 255, EndlessIDs is required.");
        warpedForestID = getInt(
            "warpedForestID",
            catBiomes,
            201,
            -1,
            65536,
            "Set to -1 to disable the generation of Warped Forests. To use an ID above 255, EndlessIDs is required.");
        soulSandValleyID = getInt(
            "soulSandValleyID",
            catBiomes,
            202,
            -1,
            65536,
            "Set to -1 to disable the generation of Soul Sand Valleys. To use an ID above 255, EndlessIDs is required.");
        basaltDeltasID = getInt(
            "basaltDeltasID",
            catBiomes,
            203,
            -1,
            65536,
            "Set to -1 to disable the generation of Basalt Deltas. To use an ID above 255, EndlessIDs is required.");

        netherWastesWeight = getInt(
            "netherWastesWeight",
            catBiomes,
            30,
            1,
            Byte.MAX_VALUE,
            "How likely the Nether Wastes biome should generate compared to other Nether biomes.");
        crimsonForestWeight = getInt(
            "crimsonForestWeight",
            catBiomes,
            10,
            1,
            Byte.MAX_VALUE,
            "How likely the Crimson Forest biome should generate compared to other Nether biomes.");
        warpedForestWeight = getInt(
            "warpedForestWeight",
            catBiomes,
            10,
            1,
            Byte.MAX_VALUE,
            "How likely the Warped Forest biome should generate compared to other Nether biomes.");
        soulSandValleyWeight = getInt(
            "soulSandValleyWeight",
            catBiomes,
            10,
            1,
            Byte.MAX_VALUE,
            "How likely the Soul Sand Valley biome should generate compared to other Nether biomes.");
        basaltDeltasWeight = getInt(
            "basaltDeltasWeight",
            catBiomes,
            10,
            1,
            Byte.MAX_VALUE,
            "How likely the Basalt Deltas biome should generate compared to other Nether biomes.");

        tileReplacementMode = getInt(
            "tileReplacementMode",
            catMisc,
            0,
            -1,
    ...
    protected void initValues() {
        if (enableFossils) {
            if (ModsList.NETHERLICIOUS.isLoaded() && fossilBlockID.equals("etfuturum:bone_block")
                && !ModBlocks.BONE.isEnabled()) {
                ...
                if ((fossilBlockArray.length == 2 || fossilBlockArray.length == 3)
                    && GameRegistry.findBlock(fossilBlockArray[0], fossilBlockArray[1]) != null) {
                    int meta = 0;
                    if (fossilBlockArray.length == 3) {
                        try {
                            ...
                            Logger.error(
                                "Specified bone block for fossils: " + fossilBlockID
                                    + " has invalid metadata specified! (Not an integer)");
                            Logger.error("Defaulting to 0.");
                        }
                        if (meta > 3 || meta < 0) {
                            Logger.error(
                                "Specified bone block for fossils: " + fossilBlockID
                                    + " has invalid metadata specified! (Value cannot be greater than 3 or lower than 0)");
                            Logger.error("Defaulting to 0.");
                            meta = 0;
                        }
                    }
                    fossilBlock = new RegistryMapping<>(
                        GameRegistry.findBlock(fossilBlockArray[0], fossilBlockArray[1]),
                        meta);
                } else {
                    Logger.error(
                        "Fossil block " + fossilBlockID
                            + " does not exist or is malformed, therefore fossils will not generate!");
                }
            }
        }

        if (enableAmethystGeodes && ModBlocks.AMETHYST_BLOCK.isEnabled()
            && ModBlocks.AMETHYST_CLUSTER_1.isEnabled()
            && ModBlocks.AMETHYST_CLUSTER_2.isEnabled()) {
            String[] amethystOuterBlockArray = amethystOuterBlockID.split(":");
            if ((amethystOuterBlockArray.length == 2 || amethystOuterBlockArray.length == 3)
                && GameRegistry.findBlock(amethystOuterBlockArray[0], amethystOuterBlockArray[1]) != null) {
                int meta = 0;
                if (amethystOuterBlockArray.length == 3) {
                    try {
                        ...
                        Logger.error(
                            "Specified block for amethystOuterLayer: " + amethystOuterBlockID
                                + " has invalid metadata specified! (Not an integer)");
                        Logger.error("Defaulting to 0.");
                    }
                    if (meta > 15 || meta < 0) {
                        Logger.error(
                            "Specified block for amethystOuterLayer: " + amethystOuterBlockID
                                + " has invalid metadata specified! (Value cannot be greater than 15 or lower than 0)");
                        Logger.error("Defaulting to 0.");
                        meta = 0;
                    }
                }
                amethystOuterBlock = new RegistryMapping<>(
                    GameRegistry.findBlock(amethystOuterBlockArray[0], amethystOuterBlockArray[1]),
                    meta);
            }

            String[] amethystMiddleBlockArray = amethystMiddleBlockID.split(":");
            if ((amethystMiddleBlockArray.length == 2 || amethystMiddleBlockArray.length == 3)
                && GameRegistry.findBlock(amethystMiddleBlockArray[0], amethystMiddleBlockArray[1]) != null) {
                int meta = 0;
                if (amethystMiddleBlockArray.length == 3) {
                    try {
                        ...
                        Logger.error(
                            "Specified block for amethystMiddleLayer: " + amethystMiddleBlockID
                                + " has invalid metadata specified! (Not an integer)");
                        Logger.error("Defaulting to 0.");
                    }
                    if (meta > 15 || meta < 0) {
                ...
                amethystMiddleBlock = new RegistryMapping<>(
                    GameRegistry.findBlock(amethystMiddleBlockArray[0], amethystMiddleBlockArray[1]),
                    meta);
            }
        }
    }
