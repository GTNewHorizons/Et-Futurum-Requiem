...
public class EtFuturumFXParticle extends EntityFX {
    ...
    protected ResourceLocation[] loadTextures(World world, double x, double y, double z, ResourceLocation texture) {
        this.usesSheet = texture != null && texture.toString()
            .equals("minecraft:textures/particle/particles.png");
        ...
        if (textures == 1 || usesSheet) {
            ...
            newRS[i] = new ResourceLocation(
                textureName.substring(0, length - 4) + "_" + i + textureName.substring(length - 4));
        }
        return newRS;
    }
    ...
    public void renderParticle(Tessellator tessellator, float partialTicks, float rx, float rxz, float rz, float ryz,
        ...
        Minecraft.getMinecraft()
            .getTextureManager()
            .bindTexture(resourceLocations[usesSheet ? 0 : currentTexture % textures]);
        ...
        if (particleAngle != 0) {
            Vec3 ipNormVec = Vec3.createVectorHelper(ipx, ipy, ipz)
                .normalize();
            double angle = Math
                .toDegrees(this.prevParticleAngle + (this.particleAngle - this.prevParticleAngle) * partialTicks);
