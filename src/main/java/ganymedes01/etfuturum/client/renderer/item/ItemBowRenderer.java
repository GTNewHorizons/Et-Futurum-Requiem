...
public class ItemBowRenderer implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack stack, Object... data) {
        ...
        if (usingItem != null && usingItem == stack) {
            ...
            if (charge >= 18) icon = bow.getItemIconForUseDuration(2);
            else if (charge > 13) icon = bow.getItemIconForUseDuration(1);
            else if (charge > 0) icon = bow.getItemIconForUseDuration(0);
        }
        if (icon == null) icon = ((TextureMap) textureManager.getTexture(resource)).getAtlasSprite("missingno");
        ...
        if (stack.hasEffect(0)) renderItem.renderEffect(textureManager, 0, 0);
