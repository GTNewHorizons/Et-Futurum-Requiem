...
public class TileEntityBannerRenderer extends TileEntitySpecialRenderer {
    ...
    private ResourceLocation getTexture(TileEntityBanner banner) {
        ...
        if (texture == null) {
            if (CANVAS_TEXTURES.size() >= 256) {
                ...
                Iterator<String> iterator = CANVAS_TEXTURES.keySet()
                    .iterator();
                ...
                while (iterator.hasNext()) {
                    ...
                    if (i - texture1.time > 60000L) {
                        Minecraft.getMinecraft()
                            .getTextureManager()
                            .deleteTexture(texture1.texture);
                        iterator.remove();
                    }
                }

                if (CANVAS_TEXTURES.size() >= 256) return null;
            }
            ...
            while (patters.hasNext()) arraylist.add(
                "textures/entity/banner/" + patters.next()
                    .getPatternName() + ".png");
            ...
            Minecraft.getMinecraft()
                .getTextureManager()
                .loadTexture(texture.texture, new LayeredColorMaskTexture(BASE_TEXTURE, arraylist, list));
    ...
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTicks) {
        ...
        long worldTime = banner.getWorldObj() != null ? banner.getWorldObj()
            .getTotalWorldTime() : 0;
        // 100 is one full revolution, so we can just mod by 100 without affecting the result
        int animationProgress100 = (((banner.xCoord % 100) * 7 + (banner.yCoord % 100) * 9 + (banner.zCoord % 100) * 13)
            + (int) (worldTime % 100)) % 100;
        float f3 = (float) animationProgress100 + partialTicks;
        bannerModel.bannerSlate.rotateAngleX = (-0.0125F + 0.01F * MathHelper.cos(f3 * 0.01F * 2F * (float) Math.PI))
            * (float) Math.PI;
