...
public class NewImageBufferDownload extends ImageBufferDownload {
    ...
    private void setAreaTransparent(int minX, int minY, int maxX, int maxY) {
        if (!hasTransparency(minX, minY, maxX, maxY)) for (int x = minX; x < maxX; x++)
            for (int y = minY; y < maxY; y++) imageData[x + y * imageWidth] &= 16777215;
    }

    private void setAreaOpaque(int minX, int minY, int maxX, int maxY) {
        for (int i = minX; i < maxX; i++) for (int j = minY; j < maxY; j++) imageData[i + j * imageWidth] |= -16777216;
    }
    ...
    private boolean hasTransparency(int minX, int minY, int maxX, int maxY) {
        for (int i = minX; i < maxX; i++) for (int j = minY; j < maxY; j++) {
            ...
            if ((colour >> 24 & 255) < 128) return true;
        }
