...
public class BeeFlySound extends MovingSound {
    ...
    public BeeFlySound(EntityBee beeIn) {
        ...
        soundHandler = FMLClientHandler.instance()
            .getClient()
            .getSoundHandler();
    }
    ...
    public void update() {
        ...
        if (!this.beeInstance.isDead && !this.hasSwitchedSound) {
            ...
            float f = MathHelper
                .sqrt_double(beeInstance.motionX * beeInstance.motionX + beeInstance.motionZ * beeInstance.motionZ);
            if (!beeInstance.onGround && (double) f >= 0.01D) {
                this.field_147663_c /* pitch */ = Utils.lerp(
                    MathHelper.clamp_float(f, this.getMinPitch(), this.getMaxPitch()),
                    this.getMinPitch(),
                    this.getMaxPitch());
