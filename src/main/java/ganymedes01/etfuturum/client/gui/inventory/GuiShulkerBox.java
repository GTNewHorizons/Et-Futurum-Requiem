...
public class GuiShulkerBox extends GuiContainer {
    ...
    private static final ResourceLocation[] backgrounds = Arrays.stream(ShulkerBoxType.VALUES)
        .map(
            t -> t.getGuiTextureName() == null ? new ResourceLocation("textures/gui/container/generic_54.png")
                : new ResourceLocation(
                    String.format("etfuturum:textures/gui/container/ironshulkerbox/%s.png", t.getGuiTextureName())))
    ...
    public GuiShulkerBox(IInventory playerInventory, IInventory chestInventory) {
        super(
            new ContainerChestGeneric(
                playerInventory,
                chestInventory,
                ((TileEntityShulkerBox) chestInventory).getRowSize(),
                chestInventory.getSizeInventory() != 27));
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        this.mc.getTextureManager()
            .bindTexture(backgrounds[type.ordinal()]);
