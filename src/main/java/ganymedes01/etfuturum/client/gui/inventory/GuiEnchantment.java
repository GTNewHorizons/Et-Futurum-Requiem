...
public class GuiEnchantment extends GuiContainer {
     ...
     */
    private static final ResourceLocation ENCHANTMENT_TABLE_BOOK_TEXTURE = Utils
        .getResource("textures/entity/enchanting_table_book.png");
    /**
    ...
    private final ArrayList<ItemStack> displayStacks; // Stores the stacks in an array so we can more easily pick random
                                                      // elements and stuff
    private final Random displayRand = new Random();
    private static final ItemStack LAPIS = new ItemStack(Items.dye, 1, 4); // This is used to match with lapis to put it
    ...
    public GuiEnchantment(InventoryPlayer p_i45502_1_, World worldIn, String p_i45502_3_) {
        ...
        displayStacks = new ArrayList<>(
            EnchantingFuelRegistry.getFuels()
                .keySet());
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        for (int i1 = 0; i1 < 3; i1++) {
            ...
            if (j1 == 0) drawTexturedModalRect(var14, l + 14 + 19 * i1, 0, 185, 108, 19);
                ...
                fontrenderer
                    .drawStringWithShadow(s1, var15 + 86 - fontrenderer.getStringWidth(s1), l + 16 + 19 * i1 + 7, k1);
                OpenGLHelper.colour(1, 1, 1);
            }
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int var6 = 0; var6 < 3; ++var6) {
            ...
            if (func_146978_c/* isPointInRegion */(60, 14 + 19 * var6, 108, 17, mouseX, mouseY) && var7 > 0
                && var8 >= 0) {
                ...
                if (ench != null) {
                    ...
                    var10.add(
                        EnumChatFormatting.WHITE.toString() + EnumChatFormatting.ITALIC
                            + I18n.format("container.enchant.clue", var11));
                }
                ...
                if (!var4) {
                    ...
                    if (mc.thePlayer.experienceLevel < var7) var10.add(
                        EnumChatFormatting.RED + I18n.format("container.enchant.level.required")
                            + ": "
                            + container.enchantLevels[var6]);
                    else {
                        if (displayStacks.size() > 1) {
                            if (viewingTicks > 40) {
                                viewingTicks = 0;
                                ...
                                // We don't want to display the same item name twice in a row (because then it looks
                                // like it's being displayed for a longer time)
                            }
                        }
                        if (!container.noFuel) {
                            String name;
                            ...
                            if (displayItem.isItemEqual(LAPIS)) {
                                name = StatCollector
                                    .translateToLocal("container.enchant.lapis." + (var9 > 1 ? "many" : "one"));
                            } else {
                                name = "%s "
                                    + StatCollector.translateToLocal(displayItem.getUnlocalizedName() + ".name");
                            }
                        ...
                        var11 = I18n
                            .format(var9 == 1 ? "container.enchant.level.one" : "container.enchant.level.many", var9);
    ...
    public void func_147068_g() {
        ItemStack var1 = inventorySlots.getSlot(0)
            .getStack();

        if (!ItemStack.areItemStacksEqual(var1, last)) {
            last = var1;

            do flipT += random.nextInt(4) - random.nextInt(4); while (flip <= flipT + 1.0F && flip >= flipT - 1.0F);
        }
        ...
        for (int var3 = 0; var3 < 3; ++var3) if (container.enchantLevels[var3] != 0) {
            var2 = true;
            break;
        }
