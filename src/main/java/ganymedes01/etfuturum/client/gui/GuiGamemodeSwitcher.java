...
public class GuiGamemodeSwitcher extends GuiScreen {
    ...
    private static WorldSettings.GameType nextGameType(WorldSettings.GameType type) {
        switch (type) {
            ...
            case ADVENTURE: {
                if (ConfigMixins.enableSpectatorMode) return SpectatorMode.SPECTATOR_GAMETYPE;
                else return WorldSettings.GameType.CREATIVE;
            }
            default:
    ...
    private WorldSettings.GameType getPreviousGameMode() {
        if (previousGameType == null) {
            previousGameType = Minecraft.getMinecraft().playerController.currentGameType
                == WorldSettings.GameType.CREATIVE ? WorldSettings.GameType.SURVIVAL : WorldSettings.GameType.CREATIVE;
        }
    ...
    public void initGui() {
        ...
        if (ConfigMixins.enableSpectatorMode) map.put(SpectatorMode.SPECTATOR_GAMETYPE, new ItemStack(Items.ender_eye));

        int i = 0;
        int totalWidth = map.size() * 31 - 5;
        for (Map.Entry<WorldSettings.GameType, ItemStack> entry : map.entrySet()) {
            this.gameModeButtons.add(
                new GuiGamemodeButton(
                    entry.getValue(),
                    entry.getKey(),
                    this.width / 2 - totalWidth / 2 + i * 31,
                    this.height / 2 - 31));
            i++;
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float delta) {
        ...
        if (this.gameMode != null) {
            drawCenteredString(
                mc.fontRenderer,
                I18n.format("gameMode." + this.gameMode.getName()),
                this.width / 2,
                this.height / 2 - 31 - 20,
                -1);
        }
        ...
        for (GuiGamemodeButton buttonWidget : this.gameModeButtons) {
            ...
            if (mouseX < buttonWidget.x || mouseY < buttonWidget.y
                || mouseX >= (buttonWidget.x + BUTTON_SIZE)
                || mouseY >= (buttonWidget.y + BUTTON_SIZE)) continue;
    ...
    class GuiGamemodeButton extends Gui {
        ...
        void render() {
            ...
            itemRender
                .renderItemAndEffectIntoGUI(mc.fontRenderer, mc.getTextureManager(), icon, this.x + 5, this.y + 5);
        }
    }
}
