...
/**
 * This resource pack will generate a grayscale version for any texture. The grayscale version of {name} is called
 * {name}_grayscale.
 */
public class GrayscaleWaterResourcePack implements IResourcePack {
    ...
    public InputStream getInputStream(ResourceLocation resLoc) throws IOException {
        InputStream original = Minecraft.getMinecraft()
            .getResourceManager()
            .getResource(toNonGrayscaleLocation(resLoc))
            .getInputStream();
        if (resLoc.getResourcePath()
            .endsWith(".png")) {
            BufferedImage image = ImageIO.read(original);
            String[] fileName = resLoc.getResourcePath()
                .split("/");
            image = convertImageToGrayscale(
                image,
                grayscaleTypes.get(fileName[fileName.length - 1].replace(".png", "")));
            byte[] data = null;
            ...
            for (int x = 0; x < image.getWidth(); x++) {
                ...
                switch (type) {
                    case TINT_INVERSE:
                        ...
                        float referenceRelativeLuma = Math.min(
                            1f,
                            ((referenceRelativeR * 0.299f) + (referenceRelativeG * 0.587f)
                                + (referenceRelativeB * 0.114f)));
    ...
    public boolean resourceExists(ResourceLocation resLoc) {
        if (resLoc.getResourcePath()
            .startsWith("textures/blocks")
            && resLoc.getResourcePath()
                .contains(GRAYSCALE_SUFFIX + ".png")) {
    ...
    private ResourceLocation toNonGrayscaleLocation(ResourceLocation resLoc) {
        return new ResourceLocation(
            ...
            resLoc.getResourcePath()
                .replaceFirst("_grayscale.png", ".png"));
    }
    ...
    public enum GrayscaleType {
        ...
        AVERAGE(),
        /**
         * Grayscaling based on the luminosity of the colors, weighing the RGB values based on the human eye's
         * sensitivity to the color.
         * (r * 0.299) + (g * 0.587) + (b * 0.114)
         * This is the default for grayscaling if no choice is made.
         */
        LUMINOSITY(),
        /**
         * Inverts the tinting operation (multiplying every pixel in the image by a constant (r, g, b) vector), which is
         * what vanilla uses
         * to tint textures. It should produce correct results if the texture has the same-ish hue in every pixel. The
         * grayscale texture
