 ...
 * Many code tweaks have been made to make non-sticky blocks and allowing honey blocks to work without sticking to slime
 * blocks.
 * However the original Back in Slime mod made this all possible, so go give its code a look:
 * https://github.com/DonBruce64/BackInSlime
 */
...
public class MixinBlockPistonBase extends Block {
     ...
     * <p>
     * Somehow this is 7 when initially placing a piston against power, and NO other time. Vanilla avoids a crash from
     * invalid directions by not continuing if the side is 7.
     * I have no idea how back in slime doesn't crash past this point, it doesn't do this check. Why doesn't DonBruce64
     * need it?
     * This could be why DonBruce's pistons have an arm going down when initially placed against power though.
     * It could be that Facing.offsetFor#Side somehow doesn't throw ArrayIndexOutOfBoundsException when 7 is passed in,
     * despite being an array, and instead returns down by default?
     *
     * @param world
     * @param x
     * @param y
    ...
    private void updatePistonState(World world, int x, int y, int z) {
        ...
        int oppositeSide = dir.getOpposite()
            .ordinal();
        Block block;
        TileEntity tileentity1;

        if (isPowered && !BlockPistonBase.isExtended(pistonMetadata)) {
            if (etfuturum$getPushableBlocks(
                world,
                x + xoffset,
                y + yoffset,
                z + zoffset,
    ...
    public boolean onBlockEventReceived(World world, int x, int y, int z, int extend, int side) {
        ...
        if (extend == 1 || extend == -1) {
            ...
            if (this.isSticky && extend == 1) {
                ...
                if (blockToPull.getMobilityFlag() != 1
                    && !PistonBehaviorRegistry.isNonStickyBlock(blockToPull, metaToPull)) {
                    if (etfuturum$getPushableBlocks(
                        world,
                        x + xoffset2,
                        y + yoffset2,
            ...
            if (etfuturum$pushBlocks(
                ...
                pushedBlockList,
                pushedBlockPosList)) {
                boolean flag1 = world.setBlock(
                    x + xoffset,
                    y + yoffset,
                    z + zoffset,
                    Blocks.piston_extension,
                    side | (this.isSticky ? 8 : 0),
                    4);
                world.setTileEntity(
                    x + xoffset,
                    y + yoffset,
                    z + zoffset,
                    BlockPistonMoving
                        .getTileEntity(Blocks.piston_head, side | (this.isSticky ? 8 : 0), side, true, false));
     ...
     * This includes all attached slime blocks. It populates the pushedBlockCoordinate list and pushingBlockCoordinate
     * list,
     ...
     */
    @Unique
    private int etfuturum$getPushableBlocks(World world, int x, int y, int z, int ignoreSide, final int side,
        final int pistonX, final int pistonY, final int pistonZ, List<Pair<Block, Integer>> pushedBlockList,
        List<BlockPos> pushedBlockPosList) {
        ...
        while (blocksPushed < 13) {
            if (pushedBlock.isAir(world, pushedBlockX, pushedBlockY, pushedBlockZ)) {
                ...
                return 13;
            } else if (etfuturum$canPushBlockNested(
                pushedBlock,
                world,
                pushedBlockX,
                pushedBlockY,
            ...
            for (BlockPos pushedBlockPos : pushedBlockPosList) {
                if (pushedBlockPos.equals(pushedBlockCoords)
                    && pushedBlockPos.getBlockMetadata(world) == pushedBlockMeta
                    && pushedBlockPos.getBlock(world) == pushedBlock) {
                    return blocksPushed;
                }
            }
                ...
                for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                    if (dir.ordinal() != side && dir.ordinal() != ignoreSide) {
                        ...
                        if (!(attachedX == pistonX && attachedY == pistonY && attachedZ == pistonZ)) {
                            if (etfuturum$canPushBlockNested(
                                attachedBlock,
                                world,
                                attachedX,
                                attachedY,
                                ...
                                if (attachedBlock.getMobilityFlag() != 1) {
                                    blocksPushed += etfuturum$getPushableBlocks(
                                        world,
                                        attachedX,
                                        attachedY,
                                        attachedZ,
     ...
     */
    @Unique
    private boolean etfuturum$pushBlocks(World world, int pistonX, int pistonY, int pistonZ, int side,
        boolean extending, List<Pair<Block, Integer>> pushedBlockList, List<BlockPos> pushedBlockPosList) {
        boolean needsPusher;
        ForgeDirection dir = ForgeDirection.getOrientation(side);
        int xoffset = dir.offsetX;
        int yoffset = dir.offsetY;
                    ...
                    if (destinationBlock.getMobilityFlag() == 1) {
                        ...
                        if (destinationBlock == Blocks.piston_head) {
                            ...
                            if (pistonHead instanceof BlockPistonExtension
                                && Facing.oppositeSide[getDirectionMeta(pistonHeadMeta)] == side) continue;
                        }
                    }
                    return false;
                }
        ...
        for (BlockPos obstruction : obstructionsCoordsList) {
            ...
            obstructionBlock.dropBlockAsItemWithChance(
                world,
                obstruction.getX(),
                obstruction.getY(),
                obstruction.getZ(),
        ...
        for (int i = 0; i < pushedBlockList.size(); ++i) {
            needsPusher = true;
            Block block = pushedBlockList.get(i)
                .getLeft();
            int blockMeta = pushedBlockList.get(i)
                .getRight();
            ...
            if (block.getMobilityFlag() == 1) {
                ...
                world.setTileEntity(
                    ...
                    blockY,
                    blockZ,
                    BlockPistonMoving.getTileEntity(block, blockMeta, side, true, false));
            ...
            if (extending && PistonBehaviorRegistry.bouncesEntities(block, blockMeta)) {
                for (Entity o : world.getEntitiesWithinAABBExcludingEntity(
                    null,
                    this.getCollisionBoundingBoxFromPool(world, blockX, blockY, blockZ))) {
    ...
    @Unique
    private boolean etfuturum$canPushBlockNested(Block pushedBlock, World world, int x, int y, int z, int pushedSide,
        int sideToPushTo) {
        ...
        if (sideToPushTo != pushedSide) {
            ...
            int xoffset = ForgeDirection.getOrientation(pushedSide)
                .getOpposite().offsetX;
            int yoffset = ForgeDirection.getOrientation(pushedSide)
                .getOpposite().offsetY;
            int zoffset = ForgeDirection.getOrientation(pushedSide)
                .getOpposite().offsetZ;
            ...
            if (PistonBehaviorRegistry.isStickyBlock(pushedBlock, pushedMeta)
                && PistonBehaviorRegistry.isStickyBlock(stuckToBlock, stuckToMeta)
