...
public class WorldEventHandler {
    ...
    private WorldEventHandler() {
        ...
        birchSaplingTrees.addEntry(new WorldGenForest(true, false), 0.0D); // It's the only entry in the list, it does
                                                                           // not need a weight.
        BEE_NEST_SAPLINGS.put(new RegistryMapping<>(Blocks.sapling, 2), birchSaplingTrees); // Also yes that is really
                                                                                            // the name of the birch
                                                                                            // tree gen class lol
        ...
        if (ModsList.NATURA.isLoaded()) {
            try {
                ...
                constructor = ReflectionHelper
                    .getClass(CommonProxy.class.getClassLoader(), "mods.natura.worldgen.SakuraTreeGen")
                    .getConstructor(boolean.class, int.class, int.class);
                sakuraSaplingTrees.addEntry((WorldGenerator) constructor.newInstance(true, 1, 0), 0.0D);
                BEE_NEST_SAPLINGS.put(
                    new RegistryMapping<>(GameRegistry.findBlock("Natura", "florasapling"), 3),
                    sakuraSaplingTrees);

                WeightedRandomList<WorldGenerator> mapleSaplingTrees = new WeightedRandomList<>();
                constructor = ReflectionHelper
                    .getClass(CommonProxy.class.getClassLoader(), "mods.natura.worldgen.RareTreeGen")
                    .getConstructor(boolean.class, int.class, int.class, int.class, int.class);
                mapleSaplingTrees.addEntry((WorldGenerator) constructor.newInstance(true, 4, 2, 0, 0), 0.0D);
                BEE_NEST_SAPLINGS
                    .put(new RegistryMapping<>(GameRegistry.findBlock("Natura", "Rare Sapling"), 0), mapleSaplingTrees);

                WeightedRandomList<WorldGenerator> willowSaplingTrees = new WeightedRandomList<>();
                constructor = ReflectionHelper
                    .getClass(CommonProxy.class.getClassLoader(), "mods.natura.worldgen.WillowGen")
                    .getConstructor(boolean.class);
                willowSaplingTrees.addEntry((WorldGenerator) constructor.newInstance(true), 0.0D);
                BEE_NEST_SAPLINGS.put(
                    new RegistryMapping<>(GameRegistry.findBlock("Natura", "Rare Sapling"), 4),
                    willowSaplingTrees);
            } catch (Exception e) {
    ...
    public void postInit() {
        ...
        BEE_NEST_BIOMES.put(BiomeGenBase.getBiome(BiomeGenBase.forest.biomeID + 128), .02F);// 2% chance to try to place
                                                                                            // a hive (Flower Forest)
    }
    ...
    public void onSaplingGrow(SaplingGrowTreeEvent event) {// 5% chance to run this logic.
        if (ModBlocks.BEE_NEST.isEnabled() && event.rand.nextFloat() <= 0.05F
            && isFlowerNearby(event.world, event.x, event.y, event.z)) {
            ...
            WeightedRandomList<WorldGenerator> treesForSapling = BEE_NEST_SAPLINGS
                .get(RegistryMapping.getKeyFor(sapling, saplingMeta % 8));
            if (treesForSapling != null) {
                event.world.setBlockToAir(event.x, event.y, event.z);
                if (treesForSapling.getRandom(event.rand)
                    .generate(event.world, event.rand, event.x, event.y, event.z)) {
    ...
    // Hives should always face south so we never want to generate them on the north face of a log, since the log would
    // block the hive opening.
    private static final ForgeDirection[] VALID_HIVE_DIRS = ArrayUtils
        .removeElements(Utils.FORGE_DIRECTIONS, ForgeDirection.NORTH);

    /**
     * Tries to place a bee nest at the tree at this location. Basically "walks" up the tree we started at until there's
     * a leaf block above the adjacent air from it.
     */
    public static void tryPlaceBeeNest(World world, int x, int y, int z, Random rand, int minBees) {
        ...
        if (targetLog.isWood(world, x, y, z)) {
            ...
            if (world.getBlock(hiveX, y, hiveZ) == targetLog
                && world.getBlockMetadata(hiveX, y, hiveZ) == targetLogMeta) {
                x += hiveDir.offsetX;
                z += hiveDir.offsetZ;
            } // If the block next to the log we started at is the same log, move the x or z we look for since this is
              // probably a 2x2 tree. Or could just be an adjacent tree of the same species. That's fine too.
            while (y < world.getHeight() - 2) { // Keep following the same log until there's a leaf block above the
                                                // adjacent block we're checking. If we are checking near world height
                                                // then give up, we're not finding leaves there lol
                if (world.getBlock(x, y, z) == targetLog && world.getBlockMetadata(x, y, z) == targetLogMeta) {
                    // If the beehive location is below leaves or the same species of log, we've found the spot for our
                    // buzzy friends to call home.
                    Block hiveHangingBlock = world.getBlock(hiveX, y + 1, hiveZ);
                    if (hiveHangingBlock.isLeaves(world, x, y + 1, z) || (hiveHangingBlock == targetLog
                        && world.getBlockMetadata(hiveX, y + 1, hiveZ) == targetLogMeta)) {
                        // We just gotta make sure we're not also replacing a block that already exists, and there's
                        // nothing in front of where our nest will be.
                        if (world.getBlock(hiveX, y, hiveZ)
                            .getMaterial() == Material.air
                            && world.getBlock(hiveX, y, hiveZ + 1)
                                .getMaterial() == Material.air) {
                            world
                                .setBlock(hiveX, y, hiveZ, ModBlocks.BEE_NEST.get(), ForgeDirection.SOUTH.ordinal(), 2);
                            TileEntityBeeHive hive = ((TileEntityBeeHive) world.getTileEntity(hiveX, y, hiveZ));
                            for (int i = MathHelper.getRandomIntegerInRange(rand, minBees, hive.maxHiveSize())
                                - minBees; i < hive.maxHiveSize(); i++) {
                        ...
                        return;
                        // TODO: Maybe instead of breaking here, storing if it's a valid position then choosing the
                        // highest possible valid position?
                    }
                } else {
                    return;
                }
            ...
            // Because mangrove trees never leave a log where the sapling actually grew, so we check for the first log
            // and then try this function again
