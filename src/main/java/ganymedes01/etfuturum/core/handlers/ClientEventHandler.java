...
public class ClientEventHandler {
    ...
    /**
     * Used by sound events to get the unlocalized name for the specific state of a block. This is handled on the item's
     * end of things.
     * So I use this "storage" stack to store the block I want the meta-name for, so I don't create new ItemStack
     * instances constantly.
     */
    private static final ItemStack STORAGE_STACK = new ItemStack((Item) null, 1, 0);

    private static final boolean forceHideSnapshotWarning = Boolean
        .parseBoolean(System.getProperty("etfuturum.hideSnapshotWarning"));
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (ConfigFunctions.enableNewF3Behavior) {
            if (Keyboard.getEventKey() == Keyboard.KEY_F3) {
                ...
                if (pressedF3) eligibleForDebugInfoSwap = true;
                ...
                if (key != Keyboard.KEY_LSHIFT && key != Keyboard.KEY_RSHIFT) eligibleForDebugInfoSwap = false;
            }
        }
    }
    ...
    public void onClientTick(ClientTickEvent event) {
        ...
        if (player.ticksExisted == 40) {
            if (!Reference.DEV_ENVIRONMENT && !ConfigExperiments.getEnabledElements()
                .isEmpty() && !showedDebugWarning) {
                if (!forceHideSnapshotWarning && !Reference.TESTING) {
                    ChatComponentText text = new ChatComponentText(
                        I18n.format("efr.warn.experimental", ConfigExperiments.buildLoadedExperimentsList(true)));
                    ...
                    text.getChatStyle()
                        .setChatClickEvent(
                            new ClickEvent(Action.OPEN_URL, "https://github.com/Roadhog360/Et-Futurum-Requiem/issues"));
                    player.addChatComponentMessage(text);
                } else {
                    Logger
                        .warn(I18n.format("efr.log.experimental", ConfigExperiments.buildLoadedExperimentsList(false)));
            ...
            if (ModItems.ELYTRA.isEnabled() && Utils.badBetterFPSAlgorithm()) {
                ChatComponentText text = new ChatComponentText(
                    I18n.format("efr.critical.badbetterfpsalgorithm1")
        ...
        if (ConfigSounds.netherAmbience && world.provider.dimensionId == -1
            && (world.provider.getClass() == WorldProviderHell.class
                || world.provider.getClass() == WorldProviderEFRNether.class)) {
            Chunk chunk = world
                .getChunkFromBlockCoords(MathHelper.floor_double(player.posX), MathHelper.floor_double(player.posZ));
            if (!chunk.isChunkLoaded) {
                if (netherAmbienceLoop != null && mc.getSoundHandler()
                    .isSoundPlaying(netherAmbienceLoop)) {
                    mc.getSoundHandler()
                        .stopSound(netherAmbienceLoop);
                }
        ...
        if (ConfigFunctions.enableGamemodeSwitcher && Keyboard.isCreated()
        ...
        if (ConfigFunctions.enableNewF3Behavior && mc.gameSettings.showDebugInfo != wasShowingDebugInfo
            && Keyboard.isKeyDown(Keyboard.KEY_F3)) {
    ...
    private final List<String> netherAmbienceLoopNames = ImmutableList
        .of("nether_wastes", "crimson_forest", "warped_forest", "soul_sand_valley", "basalt_deltas");
    ...
    private void handleBiomeParticles() {
        ...
        } else if (mc.gameSettings.particleSetting == 1) { // Fast particle logic (check biome at the player position
                                                           // once instead of at every particle
            byte b0 = 16;
    ...
    private void handleNetherAmbienceLoop() {
        if (netherAmbienceLoop == null || !mc.getSoundHandler()
            ...
            netherAmbienceLoop = netherAmbienceLoops.getOrDefault(currentBiome, defaultNetherAmbienceLoop)
                .createNew();
            netherAmbienceLoop.skipFadeIn();
            mc.getSoundHandler()
                .playSound(netherAmbienceLoop);
            return;
        }
    ...
    public void toolTipEvent(ItemTooltipEvent event) {
        ...
        // event.toolTip.add("Harvest: " + block.getHarvestTool(event.itemStack.getItemDamage()) + " " +
        ...
        if (ConfigFunctions.enableExtraF3HTooltips && event.showAdvancedItemTooltips) {
            ...
            if (event.itemStack.stackTagCompound != null && !event.itemStack.stackTagCompound.hasNoTags()) {
                event.toolTip.add(
                    "\u00a78NBT: " + event.itemStack.stackTagCompound.func_150296_c/* getKeySet */()
                        .size() + " Tag(s)");
            }
        }
    }
    ...
    public void onPlaySoundEvent(PlaySoundEvent17 event) {
        if (event.sound != null && event.name != null
            ...
            final boolean hitSound = block.stepSound.getStepResourcePath()
                .endsWith(event.name);
            final boolean breakSound = block.stepSound.getBreakSound()
                .endsWith(event.name);
            final boolean placeSound = block.stepSound.func_150496_b/* getPlaceSound */()
                .endsWith(event.name);
            ...
            if (MultiBlockSoundRegistry.multiBlockSounds.containsKey(block) && (hitSound || breakSound || placeSound)) {
                ...
                if (newSoundString != null || volume != -1 || pitch != -1) {
                    ...
                    event.result = new PositionedSoundRecord(
                        new ResourceLocation(newSoundString),
                        volume,
                        pitch,
                        soundX,
                        soundY,
            ...
            if (ConfigSounds.doorOpenClose && event.name.contains("random.door")) {
                event.result = new PositionedSoundRecord(
                    new ResourceLocation(getReplacementDoorSound(block, event.name)),
            ...
            if (ConfigSounds.chestOpenClose && event.name.contains("random.chest")) {
                ...
                if (blockID.contains("chest") && (event.name.contains("open") || event.name.contains("close"))) {
                    if ((blockID.contains("ender") && block.getMaterial()
                        .equals(Material.rock)))
                        s = Reference.MCAssetVer + ":"
                            + "block.ender_chest."
                            + (event.name.contains("close") ? "close" : "open");
                    else if (block.getMaterial()
                        .equals(Material.wood) && event.name.contains("close"))
            ...
            // We check what sound event to use by the pitch. These blocks fire 0.6F when turning on, and 0.5F when
            // turning off. We use > 0.5F instead of == 0.6F to account for floating point precision.w
            if (ConfigSounds.pressurePlateButton) {
                ...
                if (block instanceof BlockButton && event.name.equals("random.click")) {
                    ...
                    if (s != null) {
                        event.result = new PositionedSoundRecord(
                            new ResourceLocation(s + "_" + (event.sound.getPitch() > 0.5F ? "on" : "off")),
                            1,
                            1,
                            soundX,
                            soundY,
            ...
            if (ConfigSounds.noteBlockNotes
                && world.getBlock(
                    ...
                    MathHelper.floor_float(soundY),
                    MathHelper.floor_float(soundZ)) instanceof BlockNote
                && (event.name.equals("note.harp") || event.name.equals("note.snare")
                ...
                if (item != null && item.getHasSubtypes()) {
                    try {
                        ...
                        STORAGE_STACK.setItemDamage(
                            world.getBlockMetadata(
                                MathHelper.floor_float(soundX),
                                MathHelper.floor_float(soundY),
                                MathHelper.floor_float(soundZ)));
                        blockName = item.getUnlocalizedName(STORAGE_STACK)
                            .toLowerCase();
                    } catch (Exception e) {/*
                                            * In case a mod doesn't have a catch for invalid meta states and throws an
                                            * error, just ignore it and proceed
                                            */}
                }

                if (blockName.equals("")) {
                    blockName = blockBeneath.getUnlocalizedName()
                        .toLowerCase();
                }
                ...
                event.result = new PositionedSoundRecord(
                    new ResourceLocation(instrumentToPlay),
                    instrumentToPlay.equals(Reference.MCAssetVer + ":block.note_block.iron_xylophone") ? 1F
                        : event.sound.getVolume(),
                    event.sound.getPitch(),
                    soundX,
                    soundY,
            ...
            if (ConfigSounds.rainSounds && event.name.equals("ambient.weather.rain")) {
                event.result = new PositionedSoundRecord(
                    new ResourceLocation(
                        Reference.MCAssetVer + ":weather.rain" + (event.sound.getPitch() < 1.0F ? ".above" : "")),
                    event.sound.getVolume(),
                    event.sound.getPitch(),
                    x + 0.5F,
                    y + 0.5F,
            ...
            if (event.name.startsWith("music.game")) { // Just override overworld or Nether music, not other music types
                if (musicOverride == null || !mc.getSoundHandler()
                    .isSoundPlaying(musicOverride)) {
            ...
            if (event.name.equals("ambient.cave.cave")) {
                if (getAmbienceMood() != null) {
                    event.result = new PositionedSoundRecord(
                        new ResourceLocation(getAmbienceMood()),
                        event.sound.getVolume(),
                        event.sound.getPitch(),
                        x + 0.5F,
                        y + 0.5F,
    ...
    public void onPlaySoundAtEntityEvent(PlaySoundAtEntityEvent event) {
        ...
        /*
         * We have to do this because we don't want our new sound to be caught in the below logic, and get freeze in a
         * loop of repeatedly replacing itself and firing events forever.
         * And no, unlike the regular play sound event, the result isn't a PositionedSoundRecord so we can't just supply
         * a new one in the result. We can only override the name.
         * Because SOMEONE THOUGHT IT WOULD BE A GOOD IDEA TO MAKE IT SO YOU CAN ONLY CHANGE THE EVENT NAME AND NOT
         * VOLUME OR PITCH???
         * So as a solution I need to play a new sound instead of modifying the event one, and I do this to "mark" the
         * sound, making sure it doesn't get run through this logic.
         * Then we strip the ignore suffix and skip the custom sound logic. I don't like the way this works either but I
         * don't think I have a choice lol, blame Forge
         */
        ...
        if (entity instanceof EntityPlayer player && event.name.equals("random.drink")) {
            if (player.isUsingItem() && player.getHeldItem() != null
                && player.getHeldItem()
                    .getItem() instanceof ItemHoneyBottle) {
        ...
        if (ConfigSounds.heavyWaterSplashing && event.name.equals("game.player.swim.splash")) {
            ...
            float doWaterSplashEffect_f1 = (MathHelper.sqrt_double(
                ...
                * (entity.riddenByEntity == null ? 0.2F : 0.9F);
        ...
        if (MultiBlockSoundRegistry.multiBlockSounds.containsKey(block) && block.stepSound.getStepResourcePath()
            .equals(event.name)) {
            MultiBlockSoundContainer obj = MultiBlockSoundRegistry.multiBlockSounds.get(block);
            String newSoundString = obj
                .getSound(world, x, y, z, event.name, MultiBlockSoundRegistry.BlockSoundType.WALK);
        ...
        } else if (ConfigSounds.newBlockSounds && ModSounds.soundAmethystBlock.getStepResourcePath()
            .equals(event.name)) {
                ...
                if (entity.ticksExisted >= lastChimeAge + 20) {
                    field_26997 = (float) ((double) field_26997
                        * Math.pow(0.996999979019165D, entity.ticksExisted - lastChimeAge));
    ...
    public void onRenderTick(TickEvent.RenderTickEvent event) {
        ...
        if (((IElytraPlayer) player).etfu$isElytraFlying()) {
            if (event.phase == Phase.START) {
                ...
                if (mc.gameSettings.thirdPersonView == 0) player.yOffset = 3.02f;
            } else {
    ...
    public void onLivingUpdateEvent(LivingUpdateEvent event) {
        ...
        if (ConfigMixins.enableElytra && entity instanceof EntityPlayerSP) {
            ...
            if (e.etfu$isElytraFlying() && !e.etfu$lastElytraFlying()) {
                mc.getSoundHandler()
                    .playSound(new ElytraSound((EntityPlayerSP) entity));
            }
     ...
     * One: mc.thePlayer is NULL on first tick of world load even though sounds can play there, causing possible crashes
     * with mod sound handlers
     * Two: SoundHandler is pretty sensitive and likes to CRASH when you play a new sound alongside others sometimes.
     * This especially happens when playing multiple sounds at the same time
     * Somehow even though SoundHandler uses an iterator instance to iterate, which should be safe to modify, it gives a
     * ConcurrentModificationException error sometimes.
     * This can happen in various places during their iterators, even during iterator.remove() which should never throw
     * a comod error (?????)
     * And sometimes it just crashes when queueing multiple sounds. Of course when loading a world this will happen with
     * bees and other sounds I add like this. Geez, SoundHandler is messed up...
     * Three: Ear blasting. Not only do we have the handful of SoundHandler problems above, but you get an earful too!
     * Sometimes you can hear tickable sounds on the loading screen!
