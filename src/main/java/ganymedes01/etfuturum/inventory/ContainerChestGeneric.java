    ...
    public ContainerChestGeneric(IInventory p_i1806_1_, IInventory p_i1806_2_, int rowSize, boolean slim) {
        ...
        if (p_i1806_2_.getSizeInventory() % rowSize != 0) {
            throw new IllegalArgumentException(
                "Non-rectangular inventory. " + p_i1806_2_.getSizeInventory() + " slots, " + rowSize + " columns.");
        }

        int numRows = (int) Math.ceil(p_i1806_2_.getSizeInventory() / (float) rowSize);
        if (!(p_i1806_1_ instanceof InventoryPlayer)
            || !SpectatorMode.isSpectator(((InventoryPlayer) p_i1806_1_).player)) {
            ...
            for (k = 0; k < rowSize; ++k) {
                if (k + j * rowSize < chestInventory.getSizeInventory()) {
                    this.addSlotToContainer(
                        constructSlot(p_i1806_2_, k + j * rowSize, slotStartXOff + padL + k * 18, padT + j * 18));
                }
            }
        }
            ...
            for (k = 0; k < 9; ++k) {
                this.addSlotToContainer(
                    new Slot(
                        p_i1806_1_,
                        k + j * 9 + 9,
                        invSlotStartXOff + padL + k * 18,
                        padT + numRows * 18 + sep1H + j * 18));
            }
        }
        ...
        for (j = 0; j < 9; ++j) {
            this.addSlotToContainer(
                new Slot(
                    p_i1806_1_,
                    j,
                    invSlotStartXOff + padL + j * 18,
                    padT + numRows * 18 + sep1H + 3 * 18 + sep2H));
        }
    }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index < this.chestInventory.getSizeInventory()) {
                if (!this.mergeItemStack(
                    itemstack1,
                    this.chestInventory.getSizeInventory(),
