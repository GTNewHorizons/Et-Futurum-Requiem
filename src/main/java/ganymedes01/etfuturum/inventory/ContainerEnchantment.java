...
public class ContainerEnchantment extends Container {
    ...
    public ContainerEnchantment(InventoryPlayer inventory, World world, int x, int y, int z) {
        ...
        noFuel = EnchantingFuelRegistry.getFuels()
            .isEmpty();
    ...
    public void updateProgressBar(int p_75137_1_, int p_75137_2_) {
        if (p_75137_1_ >= 0 && p_75137_1_ <= 2) enchantLevels[p_75137_1_] = p_75137_2_;
        else if (p_75137_1_ == 3) enchantmentSeed = p_75137_2_;
        else if (p_75137_1_ >= 4 && p_75137_1_ <= 6) enchantmentIds[p_75137_1_ - 4] = p_75137_2_;
        else super.updateProgressBar(p_75137_1_, p_75137_2_);
    }
        ...
        if (p_75130_1_ == tableInventory) {
            ...
            if (var2 != null && var2.isItemEnchantable()) {
                if (!world.isRemote) {
                    ...
                    for (j = -1; j <= 1; ++j) for (int k = -1; k <= 1; ++k)
                        if ((j != 0 || k != 0) && world.isAirBlock(posX + k, posY, posZ + j)
                            && world.isAirBlock(posX + k, posY + 1, posZ + j)) {
                    ...
                    for (j = 0; j < 3; ++j) {
                        ...
                        if (enchantLevels[j] < j + 1) enchantLevels[j] = 0;
                    }

                    for (j = 0; j < 3; ++j) if (enchantLevels[j] > 0) {
            ...
            } else for (power = 0; power < 3; ++power) {
    ...
    public boolean enchantItem(EntityPlayer player, int id) {
        ...
        if ((slot1 == null || slot1.stackSize < var5) && !player.capabilities.isCreativeMode) {
            return false;
        } else if (enchantLevels[id] > 0 && slot0 != null
            && (player.experienceLevel >= var5 && player.experienceLevel >= enchantLevels[id]
                    ...
                    if (!world.isRemote) {
                        ...
                        if (var6 != null) {
                            ...
                            world.playSoundEffect(
                                ...
                                posY + 0.5F,
                                posZ + 0.5F,
                                Reference.MCAssetVer + ":block.enchantment_table.use",
                                1.0F,
                                world.rand.nextFloat() * 0.1F + 0.9F);

                            if (var7) slot0.func_150996_a(Items.enchanted_book); // setItem

                            for (EnchantmentData var9 : var6) {
                                if (var7) Items.enchanted_book.addEnchantment(slot0, var9);
                                else slot0.addEnchantment(var9.enchantmentobj, var9.enchantmentLevel);
                            }
                            ...
                            if (!player.capabilities.isCreativeMode && slot1 != null) {
                                ...
                                if (slot1.stackSize <= 0) {
                                    ItemStack containerItem = slot1.getItem()
                                        .getContainerItem(slot1);
                                    tableInventory.setInventorySlotContents(
                                        1,
                                        containerItem != null && (!slot1.getItem()
                                            .doesContainerItemLeaveCraftingGrid(slot1)
                                            || !player.inventory.addItemStackToInventory(containerItem)) ? containerItem
                                                : null);
                                }
                            }
    ...
    private List<EnchantmentData> getEnchantmentList(ItemStack stack, int seed, int level) {
        ...
        if (stack.getItem() == Items.book && list != null && list.size() > 1) list.remove(rand.nextInt(list.size()));
    ...
    public void onContainerClosed(EntityPlayer player) {
        ...
        if (!world.isRemote) for (int i = 0; i < tableInventory.getSizeInventory(); i++) {
    ...
    public boolean canInteractWith(EntityPlayer player) {
        return world.getBlock(posX, posY, posZ) == ModBlocks.ENCHANTMENT_TABLE.get()
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
        ...
        if (var4 != null && var4.getHasStack()) {
            ...
            if (index == 0) {
                if (!mergeItemStack(var5, 2, noFuel ? 37 : 38, true)) return null;
            } else if (!noFuel && index == 1) {
                if (!mergeItemStack(var5, 2, 38, true)) return null;
            } else if (!noFuel && EnchantingFuelRegistry.isFuel(var5)) {
                if (!mergeItemStack(var5, 1, 2, true)) return null;
            } else {
