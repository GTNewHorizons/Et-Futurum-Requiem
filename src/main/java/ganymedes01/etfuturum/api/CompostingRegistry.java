...
public class CompostingRegistry {
    ...
    public static void init() {
        ...
        registerCompostable(
            ImmutableList.of(new ItemStack(Blocks.cake), new ItemStack(Items.cake), new ItemStack(Items.pumpkin_pie)),
            100);
    }
     ...
     * In that case, the hundredth part of the number is how many layers will be filled, the last two digits = the %
     * chance to fill an additional layer.
    ...
    public static void registerCompostable(Object itemObj, int percent) {
        if (percent <= 0 || percent > BlockComposter.FULL_META * 100) {
            throw new IllegalArgumentException(
                "Tried to add a composter entry with percent value " + percent
                    + " which is not allowed, should be above 0 and equal to or below"
                    + (BlockComposter.FULL_META * 100)
                    + "!");
        }
        ...
        if (ModRecipes.validateItems(itemObj)) {
            if (itemObj instanceof ItemStack) {
                ...
                // OreDictionary.getOres((String) itemObj).forEach(itemStack ->
                // COMPOSTING_REGISTRY.put(itemStack.copy(), percent));//For some reason this line does nothing
                for (ItemStack oreStack : OreDictionary.getOres((String) itemObj)) { // This should be the same as the
                                                                                     // forEach above but for some
                                                                                     // reason the above just does
                                                                                     // nothing?
                ...
                COMPOSTING_REGISTRY.put(
                    new ItemStack(Item.getItemFromBlock((Block) itemObj), 1, OreDictionary.WILDCARD_VALUE),
                    percent);
            } else {
                throw new IllegalArgumentException(
                    "Tried to add " + itemObj + " as a compostable, which is not an Itemstack, item, block or string.");
            }
        }
    }
    ...
    public void printCompostables() {
        getComposts().forEach(
            (key, value) -> Logger.info(
                "Composter entry: " + Item.itemRegistry.getNameForObject(key.getItem())
                    + " Meta: "
                    + (key.getItemDamage() == OreDictionary.WILDCARD_VALUE ? "any" : key.getItemDamage())));
    }
