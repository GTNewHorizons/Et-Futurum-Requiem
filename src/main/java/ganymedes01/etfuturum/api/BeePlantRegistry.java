...
public class BeePlantRegistry {
    ...
    /**
     * When adding a BlockDoublePlant, note that any meta above 7 will be rejected. Bees are supposed to go to the top
     * half of the flower, but the bottom metas are unique.
     * Provide the bottom meta; when bees search for a flower in the world they'll go to the top half of your
     * BlockDoublePlant. No need to add the top half metas manually.
     *
     * @param block
     */
    public static void addFlower(Block block, int meta) {
        if (block instanceof BlockDoublePlant && BlockDoublePlant.func_149887_c(meta)
            ...
            throw new IllegalArgumentException(
                "BlockDoublePlant can't have meta using bit 8, it is for the top half. Bees will go to the top half if the bottom meta is valid.");
        }
