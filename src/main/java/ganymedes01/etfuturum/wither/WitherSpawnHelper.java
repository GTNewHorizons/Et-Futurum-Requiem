package ganymedes01.etfuturum.wither;
...
public class WitherSpawnHelper {
    // spotless:off
    ...
    private static boolean matchPattern(int[][] pattern, World w, int x, int y, int z, int offset, int rotation) {
    ...
    private static boolean matchWitherBody(int[][] pattern, World w, int x, int y, int z, int offset, int rotation) {
        int[] f = FORWARD[rotation / 4];
        int[] u = PERPENDICULAR[rotation / 4][rotation % 4];
    ...
    private static boolean isWitherSkull(World w, int x, int y, int z) {
    ...
    private static boolean isSoulSand(World w, int x, int y, int z) {
        return w.getBlock(x, y, z) == Blocks.soul_sand || w.getBlock(x, y, z) instanceof BlockSoulSoil;
    }

    private static void summon(World w, int ox, int oy, int oz, int[] u, int[] f) {
    ...
    public static boolean checkWitherPattern(World w, int x, int y, int z, TileEntitySkull skull) {
        if (w.isRemote || w.difficultySetting == EnumDifficulty.PEACEFUL || skull.func_145904_a() != 1) {
            return false;
        }
    ...
    public static boolean checkWitherSpawnPotential(World w, int x, int y, int z) {
        for (int rotation = 0; rotation < 12; rotation++) {
            for (int offset = 0; offset < 3; offset++) {
                if (matchWitherBody(PATTERN_BASE, w, x, y, z, offset, rotation)) {
                    return true;
                }
            }
        }
