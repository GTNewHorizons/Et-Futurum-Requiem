...
public class EntityShulkerBullet extends Entity {
    ...
    public EntityShulkerBullet(World worldIn, EntityLivingBase ownerIn, Entity targetIn, EnumFacing p_i46772_4_) {
        ...
        this.getDataWatcher()
            .updateObject(OWNER_ID, owner.getEntityId());
        this.getDataWatcher()
            .updateObject(TARGET_ID, target.getEntityId());
    }

    @Override
    protected void entityInit() {
        this.getDataWatcher()
            .addObject(OWNER_ID, -1);
        this.getDataWatcher()
            .addObject(TARGET_ID, -1);
    }
    ...
    private void selectNextMoveDirection(EnumFacing p_184569_1_) {
        ...
        if (p_184569_1_ != null && blockpos.getSquaredDistance(this.posX, this.posY, this.posZ, false) >= 4.0D) {
            ...
            if (p_184569_1_.getFrontOffsetX() == 0) {
                ...
                if (blockpos1.getX() < blockpos.getX() && this.worldObj.isAirBlock(
                    blockpos1.west()
                        .getX(),
                    blockpos1.getY(),
                    blockpos1.getZ())) {
                    list.add(EnumFacing.WEST);
                } else if (blockpos1.getX() > blockpos.getX() && this.worldObj.isAirBlock(
                    blockpos1.east()
                        .getX(),
                    blockpos1.getY(),
                    blockpos1.getZ())) {
            ...
            if (p_184569_1_.getFrontOffsetY() == 0) {
                if (blockpos1.getY() < blockpos.getY() && this.worldObj.isAirBlock(
                    blockpos1.getX(),
                    blockpos1.up()
                        .getY(),
                    blockpos1.getZ())) {
                    list.add(EnumFacing.UP);
                } else if (blockpos1.getY() > blockpos.getY() && this.worldObj.isAirBlock(
                    blockpos1.getX(),
                    blockpos1.down()
                        .getY(),
                    blockpos1.getZ())) {
            ...
            if (p_184569_1_.getFrontOffsetZ() == 0) {
                if (blockpos1.getZ() < blockpos.getZ() && this.worldObj.isAirBlock(
                    blockpos1.getX(),
                    blockpos1.getY(),
                    blockpos1.south()
                        .getZ())) {
                    list.add(EnumFacing.SOUTH);
                } else if (blockpos1.getZ() > blockpos.getZ() && this.worldObj.isAirBlock(
                    blockpos1.getX(),
                    blockpos1.getY(),
                    blockpos1.north()
                        .getZ())) {
            ...
            if (list.isEmpty()) {
                for (int i = 5; !this.worldObj.isAirBlock(
                    blockpos1.getX() + enumfacing.getFrontOffsetX(),
                    blockpos1.getY() + enumfacing.getFrontOffsetY(),
    ...
    public void onUpdate() {
        if (!this.worldObj.isRemote && this.worldObj.difficultySetting == EnumDifficulty.PEACEFUL) {
            ...
            if (!this.worldObj.isRemote) {
                if (this.target == null && this.targetUniqueId != null) {
                    for (EntityLivingBase entitylivingbase : this.worldObj.getEntitiesWithinAABB(
                        ...
                        BlockPos.getBB(this.targetBlockPos.add(-2, -2, -2), this.targetBlockPos.add(2, 2, 2)))) {
                ...
                if (this.owner == null && this.ownerUniqueId != null) {
                    for (EntityLivingBase entitylivingbase1 : this.worldObj.getEntitiesWithinAABB(
                        EntityLivingBase.class,
                        BlockPos.getBB(this.ownerBlockPos.add(-2, -2, -2), this.ownerBlockPos.add(2, 2, 2)))) {
            ...
            MovingObjectPosition raytraceresult = Utils
                .forwardsRaycast(this, true, false, ticksExisted <= 30 ? this.owner : null);
            ...
            if (this.worldObj.isRemote) {
                CustomParticles.spawnEndRodParticle(
                    worldObj,
                ...
                if (this.direction != null) {
                    ...
                    if (this.worldObj.isBlockNormalCubeDefault(
                        blockpos.getX() + direction.getFrontOffsetX(),
                        blockpos.getY() + direction.getFrontOffsetY(),
                        blockpos.getZ() + direction.getFrontOffsetZ(),
                        false)) {
                        this.selectNextMoveDirection(enumfacing$axis);
                    } else {
                        BlockPos blockpos1 = new BlockPos(this.target);

                        if ((enumfacing$axis.getFrontOffsetX() != 0 && blockpos.getX() == blockpos1.getX())
                            || (enumfacing$axis.getFrontOffsetY() != 0 && blockpos.getY() == blockpos1.getY())
                            || (enumfacing$axis.getFrontOffsetZ() != 0 && blockpos.getZ() == blockpos1.getZ())) {
                            this.selectNextMoveDirection(enumfacing$axis);
                        }
                    }
                }
    ...
    protected void bulletHit(MovingObjectPosition result) {
        if (result.entityHit == null) {
            ...
            boolean flag = result.entityHit
                .attackEntityFrom(new EntityDamageSourceIndirect("mob", this, this.owner), 4.0F);
