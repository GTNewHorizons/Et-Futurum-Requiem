...
public class EntityAICustomAvoidEntity extends EntityAIBase {
    ...
    public EntityAICustomAvoidEntity(EntityCreature mob, Class<? extends Entity> fleeFromType,
        Predicate<EntityLivingBase> extraInclusionSelector, float distance, double slowSpeed, double fastSpeed,
        Predicate<EntityLivingBase> inclusionSelector, boolean includeHidden) {
        this.mob = mob;
        ...
        this.setMutexBits(1);
        this.withinRangePredicate = new TargetPredicate().setBaseMaxDistance(distance)
    ...
    public EntityAICustomAvoidEntity(EntityCreature fleeingEntity, Class<? extends Entity> classToFleeFrom,
        float fleeDistance, double fleeSlowSpeed, double fleeFastSpeed, Predicate<EntityLivingBase> inclusionSelector,
    ...
    public boolean shouldExecute() {
        this.targetEntity = getClosestEntity(
            this.mob.worldObj.selectEntitiesWithinAABB(
                this.classToFleeFrom,
                this.mob.boundingBox.expand(this.fleeDistance, 3.0D, this.fleeDistance),
        ...
        if (this.targetEntity == null) {
            ...
            if (vec3d == null) {
                return false;
            } else if (this.targetEntity.getDistanceSq(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord)
                < this.targetEntity.getDistanceSqToEntity(this.mob)) {
                    return false;
                } else {
    ...
    private static Entity getClosestEntity(List<? extends Entity> entityList, TargetPredicate targetPredicate,
        ...
        while (true) {
            ...
            do {
                ...
                } while (!(livingEntity2 instanceof EntityLivingBase
                    && targetPredicate.test(entity, (EntityLivingBase) livingEntity2)));
    ...
    public void updateTask() {
        if (this.mob.getDistanceSqToEntity(this.targetEntity) < 49.0D) {
            this.mob.getNavigator()
                .setSpeed(this.fastSpeed);
        } else {
            this.mob.getNavigator()
                .setSpeed(this.slowSpeed);
        }
    }
}
