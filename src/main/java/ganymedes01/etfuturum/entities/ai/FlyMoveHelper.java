...
public class FlyMoveHelper extends ExtendedEntityMoveHelper {
    ...
    /**
     * Note: Since setNoGravity and moveVertical does not exist in 1.7.10 you must implement INoGravityEntity so this
     * class can toggle gravity and moveVertical on the entity when it needs to.
     */
    public FlyMoveHelper(EntityLiving p_i47418_1_, int angleLimit, boolean staysFloatingOnWait) {
        ...
        if (!(p_i47418_1_ instanceof INoGravityEntity)) {
            throw new IllegalArgumentException(
                "Entity using " + getClass().getName()
                    + " MUST implement "
                    + INoGravityEntity.class.getName()
                    + "! Got "
                    + p_i47418_1_
                    + " instead...");
        }
    ...
    public void onUpdateMoveHelper() {
        if (action == ExtendedEntityMoveHelper.Action.MOVE_TO) {
            ...
            if (entity.onGround) {
                f1 = (float) (speed * entity.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .getAttributeValue());
            } else {
                f1 = (float) (speed * entity.getEntityAttribute(EtFuturumEntityAttributes.flyingSpeed)
                    .getAttributeValue());
            }
