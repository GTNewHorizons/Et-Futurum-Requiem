...
public class ModEntityList {
    ...
    public static void registerEntity(Class<? extends Entity> entityClass, String entityName, int id, Object mod,
        int trackingRange, int updateFrequency, boolean sendsVelocityUpdates, int eggColour1, int eggColour2) {
        registerEntity(
            entityClass,
            entityName,
            id,
    ...
    private static void registerEntity(Class<? extends Entity> entityClass, String entityName, int id, Object mod,
        int trackingRange, int updateFrequency, boolean sendsVelocityUpdates, int eggColour1, int eggColour2,
        boolean hasEgg) {
        EntityRegistry
            .registerModEntity(entityClass, entityName, id, mod, trackingRange, updateFrequency, sendsVelocityUpdates);
        ...
        if (eggColour1 != -1) registerEntityEgg(entityClass, eggColour1, eggColour2);
    }
    ...
    public static boolean hasEntitiesWithEggs() {
        for (EntityData data : array) if (data != null && data.hasEgg) return true;
        return false;
    }

    public static Entity createEntityByID(int id, World world) {
        EntityData data = getData(id);
        if (data == null || !data.hasEgg) return null;

        try {
            Class<? extends Entity> cls = map.get(id);
            if (cls != null) return cls.getConstructor(World.class)
                .newInstance(world);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static EntityData[] getDatasWithEggs() {
        ...
        for (Integer id : map.keySet()) {
            EntityData data = getData(id);
            if (data != null && data.hasEgg) list.add(data);
        }
