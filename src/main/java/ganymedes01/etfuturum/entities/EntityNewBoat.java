...
public class EntityNewBoat extends Entity {
    ...
    // EntityDataManager.<Integer>createKey(EntityNewBoat.class, DataSerializers.VARINT);
    // private static final DataParameter<Integer> FORWARD_DIRECTION =
    // EntityDataManager.<Integer>createKey(EntityNewBoat.class, DataSerializers.VARINT);
    // private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityNewBoat.class,
    // DataSerializers.FLOAT);
    // private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityNewBoat.class,
    // DataSerializers.VARINT);
    // private static final DataParameter<Boolean>[] DATA_ID_PADDLE = new DataParameter[]
    // {EntityDataManager.createKey(EntityNewBoat.class, DataSerializers.BOOLEAN),
    // EntityDataManager.createKey(EntityNewBoat.class, DataSerializers.BOOLEAN)};
    ...
    public boolean canPassengerSteer() {
        return !worldObj.isRemote
            || getControllingPassenger() instanceof EntityPlayer && riddenByEntity == getControllingPassenger();
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isEntityInvulnerable()) {
            ...
            if (flag || this.getDamageTaken() > 40.0F) {
                if (!flag && this.worldObj.getGameRules()
    ...
    public void applyEntityCollision(Entity entityIn) {
        ...
        if (entityIn instanceof EntityNewBoatSeat || getPassengers().contains(entityIn)
            || entityIn.ridingEntity instanceof EntityNewBoat
            || entityIn.ridingEntity instanceof EntityNewBoatSeat) {} else
            if (entityIn.boundingBox.minY <= this.boundingBox.minY) {
                super.applyEntityCollision(entityIn);
            }
    }
    ...
    public void onUpdate() {
        ...
        if (this.status != EntityNewBoat.Status.UNDER_WATER
            && this.status != EntityNewBoat.Status.UNDER_FLOWING_WATER) {
        ...
        if (this.canPassengerSteer()) {
            ...
            this.updateMotion();
            if (this.getPassengers()
                .size() > 0 && getControllingPassenger() instanceof EntityPlayer) {
        ...
        for (int i = 0; i <= 1; ++i) {
            if (this.getPaddleState(i)) {
                if ((double) (this.paddlePositions[i] % ((float) Math.PI * 2F)) <= (Math.PI / 4D)
                    && ((double) this.paddlePositions[i] + 0.39269909262657166D) % (Math.PI * 2D) >= (Math.PI / 4D)) {
                    if (getPaddleSound() != null) playSound(getPaddleSound(), 1, 1);
                }
        ...
        if (this.worldObj.isRemote && canPassengerSteer()
            && getControllingPassenger() instanceof EntityClientPlayerMP) {
    ...
    private boolean canEntitySit(Entity entity) {
        ...
        for (int i = 0; i < ConfigBlocksItems.newBoatEntityBlacklist.length; i++) {
            ...
            if (blacklistEntry.startsWith("classpath:")) {
                if (entity.getClass()
                    .getName()
                    .contains(blacklistEntry.replace("classpath:", ""))) {
                    return ConfigBlocksItems.newBoatEntityBlacklistAsWhitelist;
                }
            }
    ...
    public float getRowingTime(int side, float limbSwing) {
        return this.getPaddleState(side) ? (float) MathHelper.denormalizeClamp(
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    ...
                    if (iblockstate.getMaterial() == Material.water
                        && d0 < (double) getLiquidHeight(this.worldObj, k1, l1, i2)) {
    ...
    public static float getBlockLiquidHeight(World world, int x, int y, int z) {
        ...
        return (i % 8) == 0 && world.getBlock(x, y + 1, z)
            .getMaterial() == Material.water ? 1.0F : 1.0F - BlockLiquid.getLiquidHeightPercent(i);
    }
    ...
    private void updateMotion() {
        ...
        if (this.previousStatus == EntityNewBoat.Status.IN_AIR && this.status != EntityNewBoat.Status.IN_AIR
            && this.status != EntityNewBoat.Status.ON_LAND) {
    ...
    private void controlBoat() {
        if (isBeingRidden()) {
            ...
            if (!worldObj.isRemote) this.setPaddleState(
                this.rightInputDown || this.forwardInputDown,
    ...
    public void updateRiderPosition() {
        if (riddenByEntity != null) updatePassenger(riddenByEntity);
    }
    ...
    protected void applyYawToEntity(Entity entityToUpdate) {
        int j = 0;
        if (entityToUpdate instanceof EntityAnimal && this.getPassengers()
            .size() > 1) {
            j = entityToUpdate.getEntityId() % 2 == 0 ? 90 : 270;
        }
        if (entityToUpdate instanceof EntityLivingBase) {
            float f = MathHelper
                .wrapAngleTo180_float(((EntityLivingBase) entityToUpdate).rotationYawHead - this.rotationYaw) + j;
    ...
    protected void readEntityFromNBT(NBTTagCompound compound) {
        if (compound.hasKey("Type", 8)) {
            this.setBoatType(
                compound.hasKey("Domain", 8) ? compound.getString("Domain") : "minecraft",
                compound.getString("Type"));
        } else {
            this.setBoatType("minecraft", "oak");
        }

        if (ConfigBlocksItems.newBoatPassengerSeat && compound.hasKey("Seat")
            && !worldObj.isRemote
            && shouldHaveSeat()) { // TODO add seat config
        ...
        if (compound.hasKey("CustomName", 9) && compound.getString("CustomName")
            .length() > 0) {
    ...
    protected void updateFallState(double y, boolean onGroundIn) {
        ...
        if (!this.isRiding()) {
            if (onGroundIn) {
                ...
                this.fallDistance = 0.0F;
            } else if (this.worldObj.getBlock((int) posX, (int) (posY - 1D), (int) posZ)
    ...
    public boolean getPaddleState(int p_184457_1_) {
        return dataWatcher.getWatchableObjectByte(DATA_ID_PADDLE[p_184457_1_]) == 1
    ...
    public void setBoatType(String domain, String boatType) {
        ...
        boatInfo = ItemNewBoat.BOAT_INFO.get(
            domain + ":"
                + boatType
                + (ConfigFunctions.dropVehiclesTogether && this instanceof EntityNewBoatWithChest ? "_chest" : ""));
        if (boatInfo == null) {
            boatInfo = ItemNewBoat.BOAT_INFO.get(
                "minecraft:oak"
                    + (ConfigFunctions.dropVehiclesTogether && this instanceof EntityNewBoatWithChest ? "_chest" : ""));
        ...
        getDataWatcher()
            .updateObject(DATA_ID_RESOURCELOCATION, resourceDomain + ":textures/entity/boat/" + getBoatType() + ".png");
    }
    ...
    /**
     * Provided through the entity as a way for modders to be able to easily make modded boats withing needing a base
     * class to attach a renderer to
     *
     * @return
     */
    public ResourceLocation getResourceLocation() {
        if (resourceLocation == null) {
            resourceLocation = new ResourceLocation(
                getDataWatcher().getWatchableObjectString(DATA_ID_RESOURCELOCATION));
        }
