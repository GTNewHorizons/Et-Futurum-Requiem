...
public class EntityRabbit extends EntityAnimal {
    ...
    public void setJumping(boolean jump, EntityRabbit.EnumMoveType moveTypeIn) {
        ...
        if (!jump) {
            if (moveType == EntityRabbit.EnumMoveType.ATTACK) moveType = EntityRabbit.EnumMoveType.HOP;
        } else {
    ...
    public void updateAITasks() {
        ...
        if (getMoveHelper().getSpeed() > 0.8D) setMoveType(EntityRabbit.EnumMoveType.SPRINT);
        else if (moveType != EntityRabbit.EnumMoveType.ATTACK) setMoveType(EntityRabbit.EnumMoveType.HOP);

        if (currentMoveTypeDuration > 0) currentMoveTypeDuration--;

        if (carrotTicks > 0) {
            carrotTicks -= rand.nextInt(3);

            if (carrotTicks < 0) carrotTicks = 0;
        }
            ...
            if (jumpHelper instanceof RabbitJumpHelper rabbitJumpHelper) {
                if (!rabbitJumpHelper.getIsJumping()) {
                    if (!getNavigator().noPath() && currentMoveTypeDuration == 0) {
                        ...
                        Vec3 vec3 = Vec3
                            .createVectorHelper(getMoveHelper().posX, getMoveHelper().posY, getMoveHelper().posZ);
    ...
    public void onLivingUpdate() {
        ...
        if (jumpTicks != jumpDuration) {
            if (jumpTicks == 0 && !worldObj.isRemote) worldObj.setEntityState(this, (byte) 1);
    ...
    protected void dropFewItems(boolean hitRencently, int fortune) {
        ...
        for (int i = 0; i < j; i++) dropItem(ModItems.RABBIT_HIDE.get(), 1);

        j = rand.nextInt(2);

        for (int i = 0; i < j; i++) if (isBurning()) dropItem(ModItems.RABBIT_COOKED.get(), 1);
        else dropItem(ModItems.RABBIT_RAW.get(), 1);

        if (rand.nextInt(100) <= 10 + fortune) entityDropItem(new ItemStack(ModItems.RABBIT_FOOT.get()), 0.0F);
    }
    ...
    protected void createRunningParticles() {
        ...
        // this.getEntityBoundingBox().minY + 0.1D, posZ + (rand.nextFloat() - 0.5D) * width, -motionX * 4.0D, 1.5D,
        // -motionZ * 4.0D, new int[] { Block.getStateId(iblockstate) });
    }
    ...
    class RabbitMoveHelper extends EntityMoveHelper {
        ...
        public void onUpdateMoveHelper() {
            if (theEntity.onGround && !field_175536_bo) theEntity.setMovementSpeed(0.0D);
