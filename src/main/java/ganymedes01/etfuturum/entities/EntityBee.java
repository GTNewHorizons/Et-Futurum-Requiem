...
/**
 * Credit to thedarkcolour for permission for thedarkcolour to see how FutureMC bees work in 1.12 to see what makes bees
 * work without 1.15 mechanics present, and what part of 1.15 mechanics need to be changed or cut.
...
public class EntityBee extends EntityAnimal implements INoGravityEntity {
    ...
    public EntityBee(World worldIn) {
        ...
        navigator = new FlyingPathNavigator(this, worldIn) {
            ...
            @Override
            public boolean isSafeToStandAt(int p_75483_1_, int p_75483_2_, int p_75483_3_, int p_75483_4_,
                int p_75483_5_, int p_75483_6_, Vec3 p_75483_7_, double p_75483_8_, double p_75483_10_) {
    ...
    protected void entityInit() {
        super.entityInit();
        this.getDataWatcher()
            .addObject(DATA_FLAGS_ID, (byte) 0);
        this.getDataWatcher()
            .addObject(ANGER_TIME, 0);
        this.getDataWatcher()
            .addObject(NO_GRAVITY, (byte) 1);
    }
    ...
    public float getBlockPathWeight(int x, int y, int z) {
        ...
        if (block.getMaterial()
            .isLiquid() || block instanceof BlockFire
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        boolean flag = entityIn.attackEntityFrom(
            new EntityDamageSource("sting", this),
            (float) ((int) this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                .getAttributeValue()));
        if (flag) {
    ...
    public void onLivingUpdate() {
        ...
        if (worldObj.isRemote && this.hasNectar()
            && this.getCropsGrownSincePollination() < 10
            && this.rand.nextFloat() < 0.05F) {
            for (int i = 0; i < this.rand.nextInt(2) + 1; ++i) {
                addParticle(
                    this.worldObj,
                    this.posX - (double) 0.3F,
                    this.posX + (double) 0.3F,
                    this.posZ - (double) 0.3F,
                    posZ + (double) 0.3F,
                    posY + this.height * 0.5F);
            }
        }
        updateBodyPitch();
    }
    ...
    private void addParticle(World worldIn, double p_226397_2_, double p_226397_4_, double p_226397_6_,
        double p_226397_8_, double posY) {
        CustomParticles.spawnBeeNectarParticle(
            worldIn,
            Utils.lerp(worldIn.rand.nextDouble(), p_226397_2_, p_226397_4_),
            posY,
            Utils.lerp(worldIn.rand.nextDouble(), p_226397_6_, p_226397_8_));
    }
    ...
    // Both flowers. For breeding the ItemBlock will have different metas, this is an issue for double plants so we
    ...
    private boolean canEnterHive() {
        if (this.stayOutOfHiveCountdown <= 0 && !this.pollinateGoal.isRunning() && !this.hasStung()) {
            boolean flag = this.failedPollinatingTooLong() || this.worldObj.isRaining()
    ...
    protected void updateAITick() {
        ...
        if (hasStung()) {
            ...
            if (this.timeSinceSting % 5 == 0
                && this.rand.nextInt(MathHelper.clamp_int(1200 - this.timeSinceSting, 1, 1200)) == 0) {
                this.attackEntityFrom(DamageSource.generic, this.getHealth()); // Kills the bee after it's been a while
    ...
    private int getAnger() {
        return this.getDataWatcher()
            .getWatchableObjectInt(ANGER_TIME);
    }

    private void setAnger(int angerTime) {
        this.getDataWatcher()
            .updateObject(ANGER_TIME, angerTime);
    }
    ...
    private void setBeeFlag(int flagId, boolean p_226404_2_) {
        if (p_226404_2_) {
            this.getDataWatcher()
                .updateObject(
                    DATA_FLAGS_ID,
                    (byte) (this.getDataWatcher()
                        .getWatchableObjectByte(DATA_FLAGS_ID) | flagId));
        } else {
            this.getDataWatcher()
                .updateObject(
                    DATA_FLAGS_ID,
    ...
    private boolean getBeeFlag(int flagId) {
        return (this.getDataWatcher()
            .getWatchableObjectByte(DATA_FLAGS_ID) & flagId) != 0;
    }
    ...
    private boolean isWithinDistance(BlockPos pos, int distance) {
        return Utils.getVec3FromEntity(this, 1.0F)
            .squareDistanceTo(pos.getX(), pos.getY(), pos.getZ()) <= distance * distance;
    }
    ...
    class AngerGoal extends EntityAIHurtByTarget {
        ...
        protected boolean isSuitableTarget(EntityLivingBase targetIn, boolean p_75296_2_) {
            if (targetIn != null && this.taskOwner.canEntityBeSeen(targetIn)
                && (!(targetIn instanceof EntityPlayer) || !((EntityPlayer) targetIn).capabilities.isCreativeMode)
                && ((EntityBee) taskOwner).setBeeAttacker(targetIn)) {
    ...
    private void startMovingTo(BlockPos pos) {
        ...
        if (vec3d1 != null) {
            ...
            this.getNavigator()
                .tryMoveToXYZ(vec3d1.xCoord, vec3d1.yCoord, vec3d1.zCoord, 1.0D);
        }
    }
    ...
    class EnterBeehiveGoal extends EntityBee.PassiveGoal {
        ...
        public boolean canBeeStart() {
            if (EntityBee.this.hasHive() && EntityBee.this.canEnterHive()
                && EntityBee.this.hivePos.isWithinDistance(new BlockPos(EntityBee.this), 2.0D)) {
    ...
    public class FindBeehiveGoal extends EntityBee.PassiveGoal {
        ...
        @Override
        public boolean canBeeStart() {
            return EntityBee.this.hivePos != null && !EntityBee.this.hasHome()
                && EntityBee.this.canEnterHive()
                && !this.isCloseEnough(EntityBee.this.hivePos)
                && getHivePos().getBlock(worldObj) instanceof BlockBeeHive;
        }
            ...
            if (EntityBee.this.hivePos != null) {
                ...
                if (this.ticks > 600) {
                    ...
                    if (!EntityBee.this.isWithinDistance(EntityBee.this.hivePos, 16)) {
                        ...
                        if (!flag) {
                            this.makeChosenHivePossibleHive();
                        } else if (this.path != null && EntityBee.this.getNavigator()
                            .getPath()
                            .isSamePath(this.path)) {
                                this.reset();
                            } else {
                                this.path = EntityBee.this.getNavigator()
                                    .getPath();
                            }
        ...
        private boolean startMovingToFar(BlockPos pos) {
            ...
            EntityBee.this.getNavigator()
                .tryMoveToXYZ(pos.getX(), pos.getY(), pos.getZ(), 1.0D);
            return EntityBee.this.getNavigator()
                .getPath() != null /* && EntityBee.this.getNavigator().getPath().reachesTarget() */;
        }
        ...
        private boolean isCloseEnough(BlockPos pos) {
            if (EntityBee.this.isWithinDistance(pos, 2)) {
                ...
                PathEntity path = EntityBee.this.getNavigator()
                    .getPath();
                return path != null && path.isDestinationSame(
                    Vec3.createVectorHelper(pos.getX(), pos.getY(), pos.getZ()))/* && path.reachesTarget() */
                    && path.isFinished();
            }
        }
    }
    ...
    public class FindFlowerGoal extends EntityBee.PassiveGoal {
        ...
        @Override
        public boolean canBeeStart() {
            return EntityBee.this.savedFlowerPos != null && !EntityBee.this.hasHome()
                && this.shouldMoveToFlower()
                && isValidFlower(worldObj, savedFlowerPos.getX(), savedFlowerPos.getY(), savedFlowerPos.getZ())
        ...
        public void updateTask() {
            if (EntityBee.this.rand.nextInt(30) == 0) {
                for (int i = 1; i <= 2; ++i) {
                    ...
                    int z = (int) posZ;
                    Block block = worldObj.getBlock(x, y, z);
                    if (BeePlantRegistry.isCrop(block)
                        && ((IGrowable) block).func_149851_a/* canFertilize */(worldObj, x, y, z, false)
                        && ((IGrowable) block).func_149852_a/* shouldFertilize */(worldObj, worldObj.rand, x, y, z)) {
                        // BlockCrops, BlockStem and BlockBerryBush should use the next meta for growth stage. We can
                        // change this later if incrementing the meta doesn't work with mod crops.
                        // For now we'll just increment instead of using the IGrowable grow event, since that often adds
    ...
    class PollinateGoal extends EntityBee.PassiveGoal {
        ...
        public boolean canBeeStart() {
            if (EntityBee.this.remainingCooldownBeforeLocatingNewFlower > 0) {
                ...
                if (optional.isPresent()) {
                    ...
                    EntityBee.this.getNavigator()
                        .tryMoveToXYZ(
                            (double) EntityBee.this.savedFlowerPos.getX() + 0.5D,
                            (double) EntityBee.this.savedFlowerPos.getY() + 0.5D,
                            (double) EntityBee.this.savedFlowerPos.getZ() + 0.5D,
                            1.2F);
                    return true;
                } else {
                    return false;
        ...
        public boolean canBeeContinue() {
            ...
            } else if (EntityBee.this.ticksExisted % 20 == 0
                && !isValidFlower(worldObj, savedFlowerPos.getX(), savedFlowerPos.getY(), savedFlowerPos.getZ())) {
                    EntityBee.this.savedFlowerPos = null;
                    return false;
                } else {
        ...
        public void updateTask() {
            ...
            if (this.ticks > 600) {
                ...
                Vec3 vec3d = EntityBee.this.savedFlowerPos.add(0.5D, 0.6F, 0.5D)
                    .newVec3();
                if (vec3d.squareDistanceTo(posX, posY, posZ) > 1.0D) {
                    ...
                    if (!flag && this.ticks > 600) {
                        ...
                        if (flag) {
                            ...
                            if (flag2) {
                                this.nextTarget = Vec3.createVectorHelper(
                                    vec3d.xCoord + (double) this.getRandomOffset(),
                                    vec3d.yCoord,
                                    vec3d.zCoord + (double) this.getRandomOffset());
                                EntityBee.this.getNavigator()
                                    .clearPathEntity();
                            } else {
                                flag1 = false;
                        ...
                        if (EntityBee.this.rand.nextFloat() < 0.05F
        ...
        private void moveToNextTarget() {
            EntityBee.this.getMoveHelper()
                .setMoveTo(this.nextTarget.xCoord, this.nextTarget.yCoord, this.nextTarget.zCoord, 0.35F);
        }
        ...
        private Optional<BlockPos> findFlower(int distance) {
            List<BlockPos> list = getBlocksInRange(
                pos -> isValidFlower(worldObj, pos.getX(), pos.getY(), pos.getZ()),
                distance,
                true);
    ...
    class UpdateBeehiveGoal extends EntityBee.PassiveGoal {
        ...
        @Override
        public boolean canBeeStart() {
            return EntityBee.this.remainingCooldownBeforeLocatingNewHive == 0 && !EntityBee.this.hasHive()
                && EntityBee.this.canEnterHive();
        }
    ...
    class WanderGoal extends EntityAIBase {
        ...
        public boolean shouldExecute() {
            return EntityBee.this.getNavigator()
                .noPath() && EntityBee.this.rand.nextInt(10) == 0;
        }
        ...
        public void startExecuting() {
            ...
            if (vec3d != null) {
                EntityBee.this.getNavigator()
                    .setPath(
                        EntityBee.this.getNavigator()
                            .getPathToXYZ(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord),
                        1.0D);
            }
        }
        ...
        private Vec3 getRandomLocation() {
            ...
            if (isHiveValid() && !isWithinDistance(hivePos, 40)) {
                ...
                vec3d = vec3d1.subtract(Utils.getVec3FromEntity(EntityBee.this, 1.0F))
                    .normalize();
            } else {
                vec3d = getLook(0.0F);
            }
            ...
            return vec3d2 != null ? vec3d2
                : EntityVectorUtils.findGroundTarget(EntityBee.this, 8, 4, -2, vec3d, (float) Math.PI / 2F);
        }
    }
    ...
    class TemptBeeWithFlowerGoal extends EntityAITempt {
        ...
        public boolean shouldExecute() {
            if (this.delayTemptCounter > 0) {
                ...
                this.temptingPlayer = this.temptedEntity.worldObj.getClosestPlayerToEntity(
                    this.temptedEntity,
                    getEntityAttribute(EtFuturumEntityAttributes.flyingSpeed).getAttributeValue());
                ...
                if (this.temptingPlayer == null) {
                    ...
                    return itemstack != null
                        && isBreedingFlower(Block.getBlockFromItem(itemstack.getItem()), itemstack.getItemDamage());
                }
            }
        }
    }
