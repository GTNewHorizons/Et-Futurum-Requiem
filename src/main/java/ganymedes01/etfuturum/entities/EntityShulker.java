...
public class EntityShulker extends EntityGolem implements IMob {
    ...
    public void setLocationAndAngles(double x, double y, double z, float yaw, float pitch) {
        ...
        if (getAge() == 0) {
            ...
            for (EnumFacing enumfacing1 : Utils.ENUM_FACING_VALUES) {
                if (this.worldObj.isBlockNormalCubeDefault(
                    xPos + enumfacing1.getFrontOffsetX(),
                    yPos + enumfacing1.getFrontOffsetY(),
                    zPos + enumfacing1.getFrontOffsetZ(),
                    false)) {
                    this.getDataWatcher()
                        .updateObject(ATTACHED_FACE, (byte) enumfacing1.ordinal());
                    break;
                }
            }
        }
    ...
    protected void entityInit() {
        super.entityInit();
        this.getDataWatcher()
            .addObject(ATTACHED_FACE, (byte) 0);
        this.getDataWatcher()
            .addObject(PEEK_TICK, (byte) 0);
        this.getDataWatcher()
            .addObject(ATTACHED_X, 0);
        this.getDataWatcher()
            .addObject(ATTACHED_Y, -1);
        this.getDataWatcher()
            .addObject(ATTACHED_Z, 0);
        this.getDataWatcher()
            .addObject(COLOR, (byte) 16);
    }
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        this.getDataWatcher()
            .updateObject(
                ATTACHED_FACE,
                (byte) EnumFacing.getFront(compound.getByte("AttachFace"))
                    .ordinal());
        this.getDataWatcher()
            .updateObject(PEEK_TICK, compound.getByte("Peek"));

        this.setAttachmentPos(
            new BlockPos(compound.getInteger("APX"), compound.getInteger("APY"), compound.getInteger("APZ")));
    ...
    public void writeEntityToNBT(NBTTagCompound compound) {
        ...
        compound.setByte(
            "AttachFace",
            this.getDataWatcher()
                .getWatchableObjectByte(ATTACHED_FACE));
        compound.setByte(
            "Peek",
            this.getDataWatcher()
                .getWatchableObjectByte(PEEK_TICK));

        compound.setInteger(
            "APX",
            this.getDataWatcher()
                .getWatchableObjectInt(ATTACHED_X));
        compound.setInteger(
            "APY",
            this.getDataWatcher()
                .getWatchableObjectInt(ATTACHED_Y));
        compound.setInteger(
            "APZ",
            this.getDataWatcher()
                .getWatchableObjectInt(ATTACHED_Z));
        compound.setByte("Color", getDataWatcher().getWatchableObjectByte(COLOR));
    }

    public BlockPos getAttachmentPos() {
        return new BlockPos(
            getDataWatcher().getWatchableObjectInt(ATTACHED_X),
            getDataWatcher().getWatchableObjectInt(ATTACHED_Y),
            getDataWatcher().getWatchableObjectInt(ATTACHED_Z));
    }
    ...
    public void onUpdate() {
        ...
        if (getColor() != 16 && worldObj.getBlock(blockpos.getX(), blockpos.getY(), blockpos.getZ())
        ...
        if (blockpos.getY() == -1 && !this.worldObj.isRemote) {
            ...
            this.getDataWatcher()
                .updateObject(ATTACHED_X, blockpos.getX());
            this.getDataWatcher()
                .updateObject(ATTACHED_Y, blockpos.getY());
            this.getDataWatcher()
                .updateObject(ATTACHED_Z, blockpos.getZ());
        }
        ...
        if (this.isRiding()) {
            ...
            if (iblockstate != Blocks.air) {
                ...
                if (iblockstate == Blocks.piston_extension) {
                    EnumFacing enumfacing = EnumFacing
                        .getFront(worldObj.getBlockMetadata(blockpos.getX(), blockpos.getY(), blockpos.getZ()));
                    blockpos = blockpos.offset(enumfacing);
                    this.setAttachmentPos(blockpos);
                } else if (iblockstate == Blocks.piston_head) {
            ...
            if (!this.worldObj.isBlockNormalCubeDefault(blockpos1.getX(), blockpos1.getY(), blockpos1.getZ(), false)) {
                ...
                for (EnumFacing enumfacing1 : Utils.ENUM_FACING_VALUES) {
                    if (this.worldObj.isBlockNormalCubeDefault(
                        blockpos.getX() + enumfacing1.getFrontOffsetX(),
                        blockpos.getY() + enumfacing1.getFrontOffsetY(),
                        blockpos.getZ() + enumfacing1.getFrontOffsetZ(),
                        false)) {
                        this.getDataWatcher()
                            .updateObject(ATTACHED_FACE, (byte) enumfacing1.ordinal());
                        flag = true;
    ...
    public void setPosition(double x, double y, double z) {
        ...
        if (this.getDataWatcher() != null) {
            ...
            if (this.getDataWatcher()
                .getWatchableObjectInt(ATTACHED_X) != xpos
                || this.getDataWatcher()
                    .getWatchableObjectInt(ATTACHED_Y) != ypos
                || this.getDataWatcher()
                    .getWatchableObjectInt(ATTACHED_Z) != zpos) {
                this.setAttachmentPos(new BlockPos(xpos, ypos, zpos));
                if (!this.isRiding()) this.getDataWatcher()
                    .updateObject(PEEK_TICK, (byte) 0);
        ...
        if (this.isAIEnabled() && this.isEntityAlive()) {
            for (int i = 0; i < 5; ++i) {
                ...
                if (newy > 0 && this.worldObj.isAirBlock(newx, newy, newz)
                    && /* this.worldObj.isInsideBorder(this.worldObj.getWorldBorder(), this) && */this.worldObj
                        .getCollidingBoundingBoxes(
                            this,
                            AxisAlignedBB.getBoundingBox(newx, newy, newz, newx + 1, newy + 1, newz + 1))
                        .isEmpty()) {
                    boolean flag = false;
                    ...
                    for (EnumFacing enumfacing : Utils.ENUM_FACING_VALUES) {
                        if (this.worldObj.isBlockNormalCubeDefault(
                            ...
                            false)) {
                            this.getDataWatcher()
                                .updateObject(ATTACHED_FACE, (byte) enumfacing.ordinal());
                            flag = true;
                            break;
                        }
                    }
    ...
    public void func_145781_i(int key) {
        if ((key == ATTACHED_X || key == ATTACHED_Y || key == ATTACHED_Z) && this.worldObj.isRemote
            && !this.isRiding()) {
    ...
    public void setAttachmentPos(BlockPos pos) {
        this.getDataWatcher()
            .updateObject(ATTACHED_X, pos.getX());
        this.getDataWatcher()
            .updateObject(ATTACHED_Y, pos.getY());
        this.getDataWatcher()
            .updateObject(ATTACHED_Z, pos.getZ());
    }
    ...
    public void updateArmorModifier(int p_184691_1_) {
        ...
        this.getDataWatcher()
            .updateObject(PEEK_TICK, (byte) p_184691_1_);
    }
    ...
    public boolean isAttachedToBlock() {
        return this.currentAttachmentPosition != null && this.getAttachmentPos()
            .getY() != -1;
    }
    ...
    public boolean interact(EntityPlayer p_70085_1_) {
        ...
        if (ConfigTweaks.dyableShulkers && stack != null) {
            if (stack.getItem() instanceof ItemBucket && ((ItemBucket) stack.getItem()).isFull == Blocks.flowing_water
                && getColor() != 16) {
                setColor((byte) 16);
                return true;
            }
            for (int oreID : OreDictionary.getOreIDs(stack)) {
                ...
                if (ArrayUtils.contains(ModRecipes.ore_dyes, OreDictionary.getOreName(oreID)) && getColor() != color) {
                    setColor(color);
                    if (!p_70085_1_.capabilities.isCreativeMode) --stack.stackSize;
                    return true;
                }
            }
        }
    ...
    class AIAttack extends EntityAIBase {
        ...
        public boolean shouldExecute() {
            ...
            return entitylivingbase != null && entitylivingbase.isEntityAlive()
                && EntityShulker.this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL;
        }
    ...
    class AIAttackNearest extends EntityAINearestAttackableTarget {
        ...
        protected AxisAlignedBB getTargetableArea(double targetDistance) {
            ...
            return enumfacing.getFrontOffsetX() != 0
                ? this.taskOwner.boundingBox.expand(4.0D, targetDistance, targetDistance)
                : (enumfacing.getFrontOffsetZ() != 0
                    ? this.taskOwner.boundingBox.expand(targetDistance, targetDistance, 4.0D)
                    : this.taskOwner.boundingBox.expand(targetDistance, 4.0D, targetDistance));
        }
    }
