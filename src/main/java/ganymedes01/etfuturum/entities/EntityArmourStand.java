...
public class EntityArmourStand extends EntityLiving {
    ...
    private void writePoseToNBT(NBTTagCompound tagCompound) {
        ...
        if (nbttaglist.tagCount() > 0) setHeadRotation(new Rotations(nbttaglist));
        else setHeadRotation(DEFAULT_HEAD_ROTATION);

        NBTTagList nbttaglist1 = tagCompound.getTagList("Body", 5);

        if (nbttaglist1.tagCount() > 0) setBodyRotation(new Rotations(nbttaglist1));
        else setBodyRotation(DEFAULT_BODY_ROTATION);

        NBTTagList nbttaglist2 = tagCompound.getTagList("LeftArm", 5);

        if (nbttaglist2.tagCount() > 0) setLeftArmRotation(new Rotations(nbttaglist2));
        else setLeftArmRotation(DEFAULT_LEFTARM_ROTATION);

        NBTTagList nbttaglist3 = tagCompound.getTagList("RightArm", 5);

        if (nbttaglist3.tagCount() > 0) setRightArmRotation(new Rotations(nbttaglist3));
        else setRightArmRotation(DEFAULT_RIGHTARM_ROTATION);

        NBTTagList nbttaglist4 = tagCompound.getTagList("LeftLeg", 5);

        if (nbttaglist4.tagCount() > 0) setLeftLegRotation(new Rotations(nbttaglist4));
        else setLeftLegRotation(DEFAULT_LEFTLEG_ROTATION);

        NBTTagList nbttaglist5 = tagCompound.getTagList("RightLeg", 5);

        if (nbttaglist5.tagCount() > 0) setRightLegRotation(new Rotations(nbttaglist5));
        else setRightLegRotation(DEFAULT_RIGHTLEG_ROTATION);
    }

    private NBTTagCompound readPoseFromNBT() {
        NBTTagCompound nbt = new NBTTagCompound();

        if (!DEFAULT_HEAD_ROTATION.equals(headRotation)) nbt.setTag("Head", headRotation.writeToNBT());
        if (!DEFAULT_BODY_ROTATION.equals(bodyRotation)) nbt.setTag("Body", bodyRotation.writeToNBT());
        if (!DEFAULT_LEFTARM_ROTATION.equals(leftArmRotation)) nbt.setTag("LeftArm", leftArmRotation.writeToNBT());
        if (!DEFAULT_RIGHTARM_ROTATION.equals(rightArmRotation)) nbt.setTag("RightArm", rightArmRotation.writeToNBT());
        if (!DEFAULT_LEFTLEG_ROTATION.equals(leftLegRotation)) nbt.setTag("LeftLeg", leftLegRotation.writeToNBT());
        if (!DEFAULT_RIGHTLEG_ROTATION.equals(rightLegRotation)) nbt.setTag("RightLeg", rightLegRotation.writeToNBT());
    ...
    public boolean interact(EntityPlayer player) {
        if (worldObj.isRemote) {
            EtFuturum.networkWrapper
                .sendToServer(new ArmourStandInteractMessage(worldObj.provider.dimensionId, this, player));
            return true;
        }
        return false;
    }
        ...
        if (!worldObj.isRemote) {
            ...
            if (itemstack != null) {
                if (itemstack.getItem() instanceof ItemArmor itemarmor) {
                    ...
                    if (itemarmor.armorType == 3) b0 = 1;
                    else if (itemarmor.armorType == 2) b0 = 2;
                    else if (itemarmor.armorType == 1) b0 = 3;
                    else if (itemarmor.armorType == 0) b0 = 4;
                }
            ...
            if (d3 >= 0.1D && d3 < 0.1D + (isSmall ? 0.8D : 0.45D) && getEquipmentInSlot(1) != null) b1 = 1;
            else if (d3 >= 0.9D + (isSmall ? 0.3D : 0.0D) && d3 < 0.9D + (isSmall ? 1.0D : 0.7D)
                && getEquipmentInSlot(3) != null) b1 = 3;
            else if (d3 >= 0.4D && d3 < 0.4D + (isSmall ? 1.0D : 0.8D) && getEquipmentInSlot(2) != null) b1 = 2;
            else if (d3 >= 1.6D && getEquipmentInSlot(4) != null) b1 = 4;

            boolean flag2 = getEquipmentInSlot(b1) != null;

            if (itemstack != null && b0 == 0 && !getShowArms()) return true;
            if (itemstack != null) func_175422_a(player, b0);
            else if (flag2) func_175422_a(player, b1);
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (!worldObj.isRemote && !canInteract) {
            if (DamageSource.outOfWorld.equals(source)) {
                ...
                if (source.getSourceOfDamage() instanceof EntityArrow) source.getSourceOfDamage()
                    .setDead();

                if (source.getEntity() instanceof EntityPlayer
                    && !((EntityPlayer) source.getEntity()).capabilities.allowEdit) return false;
                    ...
                    && ((EntityPlayer) source.getEntity()).capabilities.isCreativeMode) {
                        ...
                        if (i - punchCooldown > 5L && !flag) punchCooldown = i;
                        else {
                            dropBlock();
                            playParticles();
                            setDead();
    ...
    private void playParticles() {
        ...
        // ((WorldServer) worldObj).spawnParticle(EnumParticleTypes.BLOCK_DUST, posX, posY + height / 1.5D, posZ, 10,
        // (double) (width / 4.0F), (double) (height / 4.0F), (double) (width / 4.0F), 0.05D, new int[] {
    ...
    private void dropequipment() {
        for (int i = 0; i < 5; i++) if (getEquipmentInSlot(i) != null && getEquipmentInSlot(i).stackSize > 0)
    ...
    public void moveEntityWithHeading(float p_70612_1_, float p_70612_2_) {
        if (!hasNoGravity()) super.moveEntityWithHeading(p_70612_1_, p_70612_2_);
    }
    ...
    public void onUpdate() {
        ...
        if (!headRotation.equals(rotations)) setHeadRotation(rotations);

        Rotations rotations1 = getRotations(15);
        if (!bodyRotation.equals(rotations1)) setBodyRotation(rotations1);

        Rotations rotations2 = getRotations(18);
        if (!leftArmRotation.equals(rotations2)) setLeftArmRotation(rotations2);

        Rotations rotations3 = getRotations(21);
        if (!rightArmRotation.equals(rotations3)) setRightArmRotation(rotations3);

        Rotations rotations4 = getRotations(24);
        if (!leftLegRotation.equals(rotations4)) setLeftLegRotation(rotations4);

        Rotations rotations5 = getRotations(27);
        if (!rightLegRotation.equals(rotations5)) setRightLegRotation(rotations5);
    }

    private Rotations getRotations(int index) {
        return new Rotations(
            dataWatcher.getWatchableObjectFloat(index),
            dataWatcher.getWatchableObjectFloat(index + 1),
