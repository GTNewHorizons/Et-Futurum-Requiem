...
public class ModEnchantments {
    ...
    public static void init() {
        if (ConfigEnchantsPotions.enableFrostWalker) frostWalker = new FrostWalker();
        if (ConfigEnchantsPotions.enableMending) mending = new Mending();
        if (ConfigEnchantsPotions.enableSwiftSneak) swiftSneak = new SwiftSneak();
    }

    // Frost Walker logic
    public static void onLivingUpdate(EntityLivingBase entity) {
        if (entity.worldObj.isRemote) return;
        if (!ConfigEnchantsPotions.enableFrostWalker) return;
        ...
        if ((level = EnchantmentHelper.getEnchantmentLevel(frostWalker.effectId, boots)) > 0 && entity.onGround) {
            ...
            if (prevCoords == null
                || (Math.abs(prevCoords[0] - entity.posX) > 0.003D && Math.abs(prevCoords[1] - entity.posZ) > 0.003D)) {
                ...
                for (int i = -radius; i <= radius; i++) {
                    for (int j = -radius; j <= radius; j++) {
                        if (i * i + j * j <= radius * radius) {
                            ...
                            if (!blockUp.isNormalCube() && blockUp.getMaterial() != Material.water
                                && (block == Blocks.water || block == Blocks.flowing_water)) {
                                if (entity.worldObj
                                    .getEntitiesWithinAABBExcludingEntity(
                                        entity,
                                        AxisAlignedBB.getBoundingBox(x + i, y - 1, z + j, x + i + 1, y, z + j + 1))
                                    .isEmpty()) {
    ...
    public static void onPlayerPickupXP(PlayerPickupXpEvent event) {
        ...
        if (!ConfigEnchantsPotions.enableMending) return;
        ...
        for (ItemStack stack : stacks) if (stack != null && stack.getItemDamage() > 0
            && EnchantmentHelper.getEnchantmentLevel(mending.effectId, stack) > 0) {
