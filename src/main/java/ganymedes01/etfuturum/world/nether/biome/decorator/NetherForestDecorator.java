...
public class NetherForestDecorator extends NetherBiomeDecorator {
    ...
    private NetherForestDecorator(boolean crimson) {
        if (crimson) {
            hugeFungusGenerator = new WorldGenHugeFungus(
                false,
                0,
                0,
                ModBlocks.CRIMSON_STEM.get(),
                ModBlocks.NETHER_WART.get());
            grassGenerator = new WorldGenNetherGrass(true);
            vineGenerator = new WorldGenWeepingVines();
            wartSplatterGenerator = new WorldGenBlockSplatter(
                ModBlocks.NETHER_WART.get(),
                0,
                ModBlocks.NYLIUM.get(),
                0,
                4);
    ...
    public void decorate(World world, Random rand, int chunkX, int chunkZ) {
        ...
        for (int attempt = 0; attempt < 12; attempt++) {
            ...
            if (world.getBlock(x, y, z) instanceof BlockNylium
                && world.getBlockMetadata(x, y, z) == (hugeFungusGenerator.log == ModBlocks.CRIMSON_STEM.get() ? 0 : 1)
