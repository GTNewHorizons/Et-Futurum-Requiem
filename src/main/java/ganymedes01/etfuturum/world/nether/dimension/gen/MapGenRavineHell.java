...
public class MapGenRavineHell extends MapGenRavine {
    ...
    /**
     * We have to replace the whole function because we need to change some loops inside to do 128 height instead of 256
     * height.
     */
    @Override
    ...
    protected void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
        ...
        if (block == Blocks.netherrack || block instanceof BlockNylium
            || block == Blocks.soul_sand
            || block == ModBlocks.SOUL_SOIL.get()
            || block == ModBlocks.BLACKSTONE.get()
            || block == ModBlocks.BASALT.get()) {
            // if (y < 10)
            // {
