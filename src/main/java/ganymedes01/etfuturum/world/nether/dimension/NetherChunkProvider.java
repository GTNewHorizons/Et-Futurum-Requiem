...
public class NetherChunkProvider implements IChunkProvider {
        ...
        for (int i1 = 0; i1 < b0; ++i1) {
            for (int j1 = 0; j1 < b0; ++j1) {
                for (int k1 = 0; k1 < 16; ++k1) {
                    ...
                    // BiomeGenBase biome = worldObj.getBiomeGenForCoords(i1 + par1 * 16, j1 + par2 * 16); // unused
                    // variable
     ...
     */
    public void replaceBlocksForBiome(int x, int z, Block[] blocks, byte[] metas,
        ...
        slowsandNoise = slowsandGravelNoiseGen
            .generateNoiseOctaves(slowsandNoise, x * 16, z * 16, 0, 16, 16, 1, d0, d0, 1.0D);
        gravelNoise = slowsandGravelNoiseGen
            .generateNoiseOctaves(gravelNoise, x * 16, 109, z * 16, 16, 1, 16, d0, 1.0D, d0);
        ...
        soulSoilNoise = soulSoilNoiseGen
            .generateNoiseOctaves(soulSoilNoise, x * 16, z * 16, 16, 16, 0.0625D, 0.0625D, 1.0D);

        for (int k = 0; k < 16; ++k) {
            for (int l = 0; l < 16; ++l) {
                BiomeGenBase biomegenbase = par4ArrayOfBiomeGenBase[l + k * 16];

                boolean soulSandFlag = biomegenbase == BiomeGenBase.hell
                    && slowsandNoise[k + l * 16] + hellRNG.nextDouble() * 0.2D > 0.0D;
                boolean gravelFlag = biomegenbase == BiomeGenBase.hell
                    && gravelNoise[k + l * 16] + hellRNG.nextDouble() * 0.2D > 0.0D;
    ...
    public Chunk provideChunk(int par1, int par2) {
        ...
        netherRavineGenerator.func_151539_a(this, worldObj, par1, par2, blocks); // generate TODO: Make this available
                                                                                 // for the vanilla Nether as a separate
                                                                                 // mixin
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
        ...
        if (doGen) {
            for (j1 = 0; j1 < i1; ++j1) {
                ...
                (biome == NetherBiomeManager.soulSandValley && ConfigMixins.soulFire ? soulFireGen : fireGen)
                    .generate(worldObj, hellRNG, k1, l1, i2);
            }
        }
        ...
        if (doGen) {
            ...
            // Not sure why this needs two identical glowstone classes, I'll use them in non-deltas instances just to be
            // safe.
            // Probably doesn't make a difference for seed parity but the classes are already there so might as well use
            // them.
