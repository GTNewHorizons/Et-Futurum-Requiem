...
public class OceanMonument {
    ...
    private static final List<BiomeGenBase> validBiomes = Arrays.asList(
        BiomeGenBase.ocean,
    ...
    public static void buildTemple(World world, int x, int y, int z) {
        ...
        for (Entry<WorldCoord, Integer> entry : OceanMonument.getMap()
            .entrySet()) {
            ...
            if (block != null) world.setBlock(pos.x + x, pos.y + y, pos.z + z, block, meta, 2);
        }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (i != 3) generatePillar(world, x + 5 * i + 4 * i, y, z + 54, ModBlocks.PRISMARINE_BLOCK.get(), 1);
        }
    }

    private static void generatePillar(World world, int x, int y, int z, Block block, int meta) {
        for (int i = 1; i <= 5; i++) generatePillarSection(world, x, y - i, z, block, meta);
        y -= 5;

        for (; y >= 0; y--) {
            generatePillarSection(world, x, y, z, block, meta);
            for (int i = 0; i < 4; i++) for (int k = 0; k < 4; k++) if (world.getBlock(x + i, y, z)
                .getMaterial() != Material.water && y > 3) {
    ...
    private static void generatePillarSection(World world, int x, int y, int z, Block block, int meta) {
        for (int i = 0; i < 4; i++) for (int k = 0; k < 4; k++) if (world.getBlock(x + i, y, z)
            .getBlockHardness(world, x + i, y, z + k) > 0) world.setBlock(x + i, y, z + k, block, meta, 2);
    }
    ...
    public static void generateFile(World world, int x, int y, int z, String path) {
        try {
            ...
            for (int i = 0; i < 58; i++) for (int j = 0; j < 31 - 9; j++) for (int k = 0; k < 58; k++) {
                Block b = world.getBlock(x + i, y + j, z + k);
                int meta = world.getBlockMetadata(x + i, y + j, z + k);

                String s = "(" + i + ", " + j + ", " + k + ") - ";
                if (b == ModBlocks.PRISMARINE_BLOCK.get()) s += meta;
                else if (b == ModBlocks.SEA_LANTERN.get()) s += 3;
                else if (b == Blocks.gold_block) s += 4;
                else if (b == Blocks.sponge) s += 5;
                else if (b == Blocks.stained_glass) s += 6;
                else s = null;
    ...
    public static boolean canSpawnAt(World worldObj, int chunkX, int chunkZ) {
        ...
        if (xx == i1 && zz == j1) {
            if (worldObj.getWorldChunkManager()
                .getBiomeGenAt(xx * 16 + 8, zz * 16 + 8) != BiomeGenBase.deepOcean) return false;
            return worldObj.getWorldChunkManager()
                .areBiomesViable(xx * 16 + 8, zz * 16 + 8, 29, validBiomes);
        }
