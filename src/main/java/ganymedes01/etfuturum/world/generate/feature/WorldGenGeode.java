...
public class WorldGenGeode extends WorldGenerator {
    ...
    private final double filling, innerLayer, middleLayer, outerLayer;// geodeLayerThicknessConfig filling innerLayer
    ...
    public WorldGenGeode(RegistryMapping<Block> outerBlock, RegistryMapping<Block> middleBlock, Block innerBlock,
        Block innerBuddingBlock, Block bud1, Block bud2) {
        this(
            -16,
            16,
            1,
    ...
    public WorldGenGeode(RegistryMapping<Block> outerBlock, RegistryMapping<Block> middleBlock) {
        this(
            ...
            ModBlocks.BUDDING_AMETHYST.get(),
            ModBlocks.AMETHYST_CLUSTER_1.get(),
            ModBlocks.AMETHYST_CLUSTER_2.get());
    }

    private WorldGenGeode(int minOffset, int maxOffset, int invalidMax, int[] distPoints, int[] outerWallDist,
        double fill, double inner, double middle, double outer, int[] pointOff, double crackChance, double baseCrack,
        int crackPointOff, double noiseAmp, double budChance, double potentialPlaceChance,
        RegistryMapping<Block> outerBlock, RegistryMapping<Block> middleBlock, Block innerBlock,
        Block innerBuddingBlock, Block bud1, Block bud2) {
    ...
    /**
     * This is used when generating amethyst so it doesn't generate in the middle of the air, ocean, hanging in trees,
     * etc.
     */
    ...
    /**
     * Geode code from 1.17, ported to 1.7.10 by Roadhog360, with help of embeddedt to port the noise samplers by using
     * Trove instead of FastUtil where applicable.
     * Note: Original variable locations are left as comments above the respective variable to make it easier to
     * backtrack through the vanilla 1.17 code.
     * Some of them use a number provider to do .get to get a number in the range. If this would get two numbers I used
     * nextBoolean() instead to be faster.
     */
    @Override
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        DoublePerlinNoiseSampler doublePerlinNoiseSampler = DoublePerlinNoiseSampler.create(random, -4, 1.0D);// Somehow
                                                                                                              // this
                                                                                                              // was
                                                                                                              // (double[])(1.0D)
                                                                                                              // which
        ...
        double crackSize = 1.0D
            / Math.sqrt(baseCrackSize + random.nextDouble() / 2.0D + (distPoint > 3 ? outerWallMaxDiv : 0.0D));
        ...
        Iterator<BlockPos> var48 = BlockPos
            .iterate(
                blockPos.add(minGenOffset, minGenOffset, minGenOffset),
                blockPos.add(maxGenOffset, maxGenOffset, maxGenOffset))
            .iterator();
        ...
        while (true) {
            ...
            do {
                        ...
                        for (EnumFacing budFacing : directions) {
                            ...
                            if (world.isAirBlock(budPos.getX(), budPos.getY(), budPos.getZ())) {
                                world.setBlock(
                                    ...
                                    budPos.getZ(),
                                    budBlock,
                                    (random.nextBoolean() ? 0 : 6)/* picks a random bud size */ + budFacing.ordinal(),
                                    2);
                                break;
                            }
                        }
                ...
                double t = doublePerlinNoiseSampler.sample(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ())
                    * noiseMultiplier;
                ...
                for (var40 = list.iterator(); var40.hasNext(); currentLayerSqrt += Utils
                    .invSqrt(blockPos3.getSquaredDistance(pair.getLeft()) + (double) pair.getRight()) + t) {
                    pair = var40.next();
                } // Almost deleted this code for being unused, but the variable in the for loop is vital to later parts
                  // of the code.

                BlockPos blockPos4;
                Iterator<BlockPos> var41;
                for (var41 = list2.iterator(); var41
                    .hasNext(); v += Utils.invSqrt(blockPos3.getSquaredDistance(blockPos4) + (double) crackPointOffset)
                        + t) {
            ...
            if (world.getBlock(blockPos3.getX(), blockPos3.getY(), blockPos3.getZ())
                .getBlockHardness(world, blockPos3.getX(), blockPos3.getY(), blockPos3.getZ()) != -1) {
                if (generateCrack && v >= crackSize && currentLayerSqrt < fillingSqrt) {
                    ...
                    if ((/* !geodeFeatureConfig.placementsRequireLayer0Alternate || */bl2)
                        && (double) random.nextFloat() < usePotentialPlacementsChance) {
                    ...
                    world.setBlock(
                        ...
                        blockPos3.getY(),
                        blockPos3.getZ(),
                        middleBlock.getObject(),
                        middleBlock.getMeta(),
                        2);// MiddleLayerProvider also TODO I need to make this layer configurable
                } else if (currentLayerSqrt >= outerLayerSqrt) {
                    world.setBlock(
                        blockPos3.getX(),
                        blockPos3.getY(),
                        blockPos3.getZ(),
                        outerBlock.getObject(),
                        outerBlock.getMeta(),
                        2);// OuterLayerProvider
                }
            }
        }
    }
