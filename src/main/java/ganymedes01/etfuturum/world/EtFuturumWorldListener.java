...
public class EtFuturumWorldListener implements IWorldAccess {
    ...
    public void markBlockForUpdate(int x, int y, int z) {
        ...
        if (replacements.isEmpty()
            || !world.blockExists(x, y, z) /* || !world.getChunkFromBlockCoords(x, z).isChunkLoaded */) return;
        ...
        if ((replacement = replacements.get(world.getBlock(x, y, z))) == null) return;
    ...
    private void handleBasaltFromLava(int x, int y, int z) { // Don't support Netherlicious blocks, this can be handled
                                                             // on their end
    ...
    /**
     * This is only needed to create the initial bubble column above the magma or soul sand or custom column above
     * another block.
    ...
    private void handleBubbleColumnCreation(int x, int y, int z) {
        if (!bubbleColumnMap.isEmpty() && world.blockExists(x, y, z)) {
            if (BlockBubbleColumn
                .isFullVanillaWater(world.getBlock(x, y + 1, z), world.getBlockMetadata(x, y + 1, z))) {
