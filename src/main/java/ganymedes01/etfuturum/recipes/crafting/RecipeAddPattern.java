...
public class RecipeAddPattern implements IRecipe {
    ...
    public boolean matches(InventoryCrafting grid, World world) {
        ...
        for (int i = 0; i < grid.getSizeInventory(); i++) {
            ...
            if (slot != null && slot.getItem() == Item.getItemFromBlock(ModBlocks.BANNER.get())) {
                ...
                if (TileEntityBanner.getPatterns(slot) >= 6) return false;
                flag = true;
            }
        }
    ...
    private boolean isDye(ItemStack stack) {
        for (String ore : Utils.getOreNames(stack))
            for (EnumColor colour : EnumColor.VALUES) if (ore.equals(colour.getOreName())) return true;
    ...
    private int getDyeIndex(ItemStack stack) {
        for (String ore : Utils.getOreNames(stack))
            for (EnumColor colour : EnumColor.VALUES) if (ore.equals(colour.getOreName())) return colour.getDamage();

        return -1;
    }

    private EnumBannerPattern getPattern(InventoryCrafting grid) {
        for (EnumBannerPattern pattern : EnumBannerPattern.VALUES) if (pattern.hasValidCrafting()) {
            boolean flag = true;
            ...
            if (pattern.hasCraftingStack()) {
                ...
                if (!flag1) flag = false;
            } else if (grid.getSizeInventory()
                != pattern.getCraftingLayers().length * pattern.getCraftingLayers()[0].length()) flag = false;
            else {
                int k = -1;
