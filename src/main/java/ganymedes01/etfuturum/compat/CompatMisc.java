...
public class CompatMisc {
    ...
    public static void doLwjgl3ifyCompat() {
        try {
            ...
            if (t instanceof NoClassDefFoundError) {
                Logger.trace(
                    "Failed to apply lwjgl3ify compat: " + t + ". This is not an error unless lwjgl3ify is present.");
            } else {
        ...
        if (ConfigBlocksItems.enableSmoothStone && ModsList.BLUEPOWER.isLoaded()) {
            ...
            if (stoneTile != null) {
                ...
                FurnaceRecipes.smelting()
                    ...
                    .entrySet()
                    .removeIf(
                        recipe -> recipe.getValue() != null && recipe.getValue()
                            .getItem() == stoneTile
                            && recipe.getKey() != null
                            && recipe.getKey()
                                .getItem() == stoneItem);
    ...
    public static void runModHooksLoadComplete() {
        if (ModBlocks.SPONGE.isEnabled() && ModsList.BIOMES_O_PLENTY.isLoaded()) {
            try {
                Field featureMapField = Class.forName("biomesoplenty.common.world.generation.WorldGenFieldAssociation")
                    .getDeclaredField("featureMap");
                @SuppressWarnings("unchecked")
                Map<String, ?> featureMap = (Map<String, ?>) featureMapField.get(null); // Get the list of BOP world
                                                                                        // generators
                Object feature = featureMap.get("generateSponge"); // Find the one for sponges; this is a container
                                                                   // class, we get the value we want below

                Field worldGeneratorField = feature.getClass()
                    .getDeclaredField("worldGenerator"); // Get the field from the container class
                worldGeneratorField.setAccessible(true);
                WorldGenerator worldGenerator = (WorldGenerator) worldGeneratorField.get(feature); // Pull the value to
                                                                                                   // get the
                                                                                                   // WorldGenerator we
                                                                                                   // want

                Class<?> splotches = Class.forName("biomesoplenty.common.world.features.WorldGenSplotches"); // Get the
                                                                                                             // fields
                                                                                                             // for the
                                                                                                             // block
                                                                                                             // info in
                ...
                splotchBlockField.set(worldGenerator, ModBlocks.SPONGE.get()); // Change the info to generate the EFR
                                                                               // sponge
        ...
        if (ConfigSounds.newBlockSounds) {
            // Because NP+ uses its own (worse) step sounds for this and it causes the check in EtFuturum.java that
            // replaces these block sounds to fail.
