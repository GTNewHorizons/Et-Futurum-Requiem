...
public class ComposterHandler extends TemplateRecipeHandler implements ICraftingHandler, IUsageHandler {
    ...
    public final PositionedStack composter = new PositionedStack(
        new ItemStack(ModBlocks.COMPOSTER.get()),
        75,
        78,
        false);
    ...
    private void composeRecipeMap(ItemStack predicate) {
        List<ItemStack> inputs = new ArrayList<>(
            CompostingRegistry.getComposts()
                .keySet());
        WeightedRandomChestContent[] tables = EtFuturumLootTables.COMPOSTER_LOOT
            .getItems(Minecraft.getMinecraft().thePlayer.worldObj.rand);
        ...
        for (WeightedRandomChestContent table : tables) {
            ...
            this.props.put(
                table.theItemId,
                new TableProps(
                    table.theMinimumChanceToGenerateItem,
                    table.theMaximumChanceToGenerateItem,
                    table.itemWeight));
            x += 18;
            i++;
            if (i == 9) {
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (WeightedRandomChestContent item : EtFuturumLootTables.COMPOSTER_LOOT
            .getItems(Minecraft.getMinecraft().thePlayer.worldObj.rand)) {
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (itemCheck(
            ...
            CompostingRegistry.getComposts()
                .keySet())
            || this.composter.contains(ingredient)) {
    ...
    public void drawExtras(int recipe) {
        ...
        for (PositionedStack stack : cached.getIngredients()) {
            ...
            GuiDraw.drawString(
                str,
                (stack.relx + 16 - (fontRenderer.getStringWidth(str) >> 1)) << 1,
                (stack.rely + 12) << 1,
                0xFFFFFF,
                true);
        }
    ...
    public List<String> handleItemTooltip(GuiRecipe gui, ItemStack stack, List<String> currenttip, int recipe) {
        if (stack != null && this.props.containsKey(stack)) {
            ...
            currenttip.add(
                currenttip.size() - 1,
                "§9" + I18n.format("efr.nei.amount") + ":§6 " + props.minSize + "§7~§6" + props.maxSize);
