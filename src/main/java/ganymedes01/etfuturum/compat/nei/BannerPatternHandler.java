...
public class BannerPatternHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        ...
        for (EnumBannerPattern pattern : EnumBannerPattern.VALUES) {
            ...
            if (pattern.hasCraftingStack()) {
                arecipes.add(
                    new CachedPatternRecipe(
                        pattern,
                        new String[] { "   ", "xy ", " z " },
                        Arrays.asList(
                            'x',
                            pattern.getCraftingStack(),
                            'y',
                            "dye",
                            'z',
                            ModBlocks.BANNER.newItemStack(1, OreDictionary.WILDCARD_VALUE))).setRandomPermutations());
                arecipes.add(
                    new CachedPatternRecipe(
                        pattern,
                        new String[] { "   ", "xy ", "   " },
                ...
                arecipes.add(
                    new CachedPatternRecipe(
                        ...
                        layersCopy,
                        Arrays
                            .asList('#', "dye", 'x', ModBlocks.BANNER.newItemStack(1, OreDictionary.WILDCARD_VALUE))));
            }
        }
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        if (result.getItem() != Item.getItemFromBlock(ModBlocks.BANNER.get())) return;
        ...
        if (nbt != null && nbt.hasKey("Patterns", 9)) {
            ...
            for (int i = 0; i < patterns.tagCount(); i++) {
                ...
                EnumColor colour = patternNBT.hasKey("Color") ? EnumColor.fromDamage(patternNBT.getInteger("Color"))
                    : null;
                if (!pattern.hasValidCrafting()) continue;
                if (pattern.hasCraftingStack()) {
                    if (colour != null) arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            new String[] { "   ", "xy ", " z " },
                            Arrays.asList('x', pattern.getCraftingStack(), 'y', colour.getOreName(), 'z', copy)));
                    else arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            new String[] { "   ", "xy ", "   " },
                            Arrays.asList('x', pattern.getCraftingStack(), 'y', copy)));
                } else {
                    String[] layers = pattern.getCraftingLayers();
                    String[] layersCopy = new String[] { layers[0], layers[1], layers[2] };
                    for (int j = 0; j < 3; j++) {
                    ...
                    arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            layersCopy,
                            Arrays.asList('#', colour == null ? "dye" : colour.getOreName(), 'x', copy)));
                }
            }
        }
    }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (ingredient.getItem() == Item.getItemFromBlock(ModBlocks.BANNER.get())
            && TileEntityBanner.getPatterns(ingredient) < 6)
            for (EnumBannerPattern pattern : EnumBannerPattern.VALUES) {
                if (!pattern.hasValidCrafting()) continue;
                    ...
                    arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            new String[] { "   ", "xy ", " z " },
                            Arrays.asList('x', pattern.getCraftingStack(), 'y', "dye", 'z', banner)));
                    arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            new String[] { "   ", "xy ", "   " },
                            Arrays.asList('x', pattern.getCraftingStack(), 'y', banner)));
                } else {
                    String[] layers = pattern.getCraftingLayers();
                    String[] layersCopy = new String[] { layers[0], layers[1], layers[2] };
                    for (int i = 0; i < 3; i++) {
        ...
        else for (EnumBannerPattern pattern : EnumBannerPattern.VALUES) if (!pattern.hasValidCrafting()) continue;
        else if (pattern.hasCraftingStack()
            && OreDictionary.itemMatches(pattern.getCraftingStack(), ingredient, false)) {
                ...
                arecipes.add(
                    new CachedPatternRecipe(
                        ...
                        new String[] { "   ", "xy ", " z " },
                        Arrays.asList('x', pattern.getCraftingStack(), 'y', "dye", 'z', banner))
                            .setRandomPermutations());
                arecipes.add(
                    new CachedPatternRecipe(
                        pattern,
                        new String[] { "   ", "xy ", "   " },
                        Arrays.asList('x', pattern.getCraftingStack(), 'y', banner)).setRandomPermutations());
            } else for (int oreID : OreDictionary.getOreIDs(ingredient)) {
                String oreName = OreDictionary.getOreName(oreID);
                if (isNameDye(oreName)) if (pattern.hasCraftingStack()) {
                    ItemStack banner = ModBlocks.BANNER.newItemStack(1, OreDictionary.WILDCARD_VALUE);
                    banner.stackSize = 1;
                    arecipes.add(
                        new CachedPatternRecipe(
                            pattern,
                            new String[] { "   ", "xy ", " z " },
                            Arrays.asList('x', pattern.getCraftingStack(), 'y', oreName, 'z', banner)));
                } else {
                    String[] layers = pattern.getCraftingLayers();
                    String[] layersCopy = new String[] { layers[0], layers[1], layers[2] };
                    for (int i = 0; i < 3; i++) {
                    ...
                    banner.stackSize = 1;
                    arecipes
                        .add(new CachedPatternRecipe(pattern, layersCopy, Arrays.asList('#', oreName, 'x', banner)));
                }
            }
    }
    ...
    EnumColor getEnumColour(ItemStack stack) {
        for (String ore : Utils.getOreNames(stack))
            for (EnumColor colour : EnumColor.VALUES) if (ore.equals(colour.getOreName())) return colour;
        return null;
    }

    private boolean isNameDye(String name) {
        for (String dye : ModRecipes.ore_dyes) if (dye.equals(name)) return true;
        return false;
    }
    ...
    public class CachedPatternRecipe extends CachedRecipe {
        ...
        public CachedPatternRecipe(EnumBannerPattern pattern, String[] grid, List<Object> inputs) {
            ...
            for (int y = 0; y < 3; y++) for (int x = 0; x < 3; x++) {
                ...
                if (c != ' ') {
                    ...
                    if (input instanceof String) input = OreDictionary.getOres((String) input);
        ...
        public List<PositionedStack> getIngredients() {
            if (randomPermutations) return getCycledIngredients(cycleticks / 20, ingredients);

            for (PositionedStack stack : ingredients)
                if (stack.items.length > 1) stack.setPermutationToRender(cycleticks / 20 % stack.items.length);
            return ingredients;
        }
        ...
        public PositionedStack getResult() {
            ...
            for (PositionedStack stack : getIngredients()) {
                if (stack.item.getItem() == Item.getItemFromBlock(ModBlocks.BANNER.get())) banner = stack.item.copy();
                if (colour == null) colour = getEnumColour(stack.item);
            }
            if (banner == null) banner = new ItemStack(ModBlocks.BANNER.get());

            NBTTagCompound nbt = ItemBlockBanner.getSubTag(banner, "BlockEntityTag", true);
            NBTTagList nbttaglist;
            if (nbt.hasKey("Patterns", 9)) nbttaglist = nbt.getTagList("Patterns", 10);
            else {
            ...
            if (colour != null) nbttagcompound.setInteger("Color", colour.getDamage());
